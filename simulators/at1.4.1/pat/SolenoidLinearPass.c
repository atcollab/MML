...
{	int c;
	...
	if(ks!=0)
	#pragma omp parallel for default(shared) shared(r_in, num_particles) private(c, r6,p_norm,x,xpr,y,ypr,H,S,C) 	    
    ...
    #pragma omp parallel for default(shared) shared(r_in, num_particles) private(c, r6) 	    
        for(c = 0;c<num_particles;c++)
		{	
			r6= r_in+c*6;
			if(!isnan(r6[0]))	{
