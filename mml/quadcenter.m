function [QMS1, QMS2] = quadcenter(QuadFamily, QuadDev, XYPlane, FigureHandle)
...
% Extra delay can be written over by the QMS.ExtraDelay field.  If this does not exist, then the value below is used.
ExtraDelay = 2;    % Was zero 6/28/2014

ExtraCorrectorDelay = 4;
HysteresisLoopDwellTime = 1;
...
if isstruct(QuadFamily)
    ...
    if QMS.QuadPlane == 1
        QMS_Horizontal = QMS;
        QMS_Vertical   = quadcenterinit(QMS.QuadFamily, QMS.QuadDev, 2);
        QMS_Vertical.CorrectOrbit = QMS.CorrectOrbit;
    elseif QMS.QuadPlane == 2
        QMS_Horizontal = quadcenterinit(QMS.QuadFamily, QMS.QuadDev, 1);
    ...
    if size(QuadDev,1) > 1
        for i = 1:size(QuadDev,1)
            if XYPlane == 0
                [Q1, Q2] = quadcenter(QuadFamily, QuadDev(i,:), XYPlane, FigureHandle);
                QMS1(i,1) = Q1.Center;
                QMS2(i,1) = Q2.Center;
            else
                [Q1] = quadcenter(QuadFamily, QuadDev(i,:), XYPlane, FigureHandle);
                QMS1(i,1) = Q1.Center;
            end
        end
        return
    ...
    % Get QMS structure
    QMS_Horizontal = quadcenterinit(QuadFamily, QuadDev, 1);
    QMS_Vertical   = quadcenterinit(QuadFamily, QuadDev, 2);
end
...
% Correct orbit to the old offsets
% Commented so that orbit correction is only in the opposite plane
...
if XYPlane==0 || XYPlane==1    
    ...
    if strcmpi(QMS_Horizontal.ModulationMethod, 'bipolar')
        ...
        setquad(QMS_Horizontal, DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
        setquad(QMS_Horizontal,-DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
        setquad(QMS_Horizontal, DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
        setquad(QMS_Horizontal,-DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
        setquad(QMS_Horizontal,         QUAD0, -1);  sleep(HysteresisLoopDwellTime);
    elseif strcmpi(QMS_Horizontal.ModulationMethod, 'unipolar')
        ...
        setquad(QMS_Horizontal,         QUAD0, -1);  sleep(HysteresisLoopDwellTime);
    elseif strcmpi(QMS_Horizontal.ModulationMethod, 'sweep')
        % This is done only the because the cycle may have overshoot
        setquad(QMS_Horizontal, DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
        StartAt = 1;
    end
    ...
    fprintf('   Setting HCM %s(%d,%d)  (%s)\n', HCMFamily, HCMDev(1,:), datestr(now,'yyyy-mm-dd HH:MM:SS.FFF')); drawnow;
    ...
    for i = 1:N
        ...
        % Add an extra delay for eddy current settling
        sleep(ExtraCorrectorDelay);
        
        if strcmpi(QMS_Horizontal.ModulationMethod, 'sweep')
            % One directional sweep of the quadrupole
            sleep(ExtraDelay);
            fprintf('   Get BPM point #1      (%s)\n', datestr(now,'yyyy-mm-dd HH:MM:SS.FFF')); drawnow;
            ...
            setquad(QMS_Horizontal, (i+StartAt)*DelQuad+QUAD0, WaitFlag);
            sleep(ExtraDelay);
    ...
    fprintf('   Reset all correctors & quads (%s)\n', datestr(now,'yyyy-mm-dd HH:MM:SS.FFF')); drawnow;
...
if XYPlane==0 || XYPlane==2    
    ...
    if XYPlane == 2 || strcmpi(QMS_Horizontal.ModulationMethod, 'sweep')
        if strcmpi(QMS_Vertical.ModulationMethod, 'bipolar')
            fprintf('   Establishing a hysteresis loop on the quadrupole (bi-polar case)\n'); pause(0);
            setquad(QMS_Vertical, DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
            setquad(QMS_Vertical,-DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
            setquad(QMS_Vertical, DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
            setquad(QMS_Vertical,-DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
            setquad(QMS_Vertical,         QUAD0, -1);  sleep(HysteresisLoopDwellTime);
        elseif strcmpi(QMS_Vertical.ModulationMethod, 'unipolar')
            fprintf('   Establishing a hysteresis loop on the quadrupole (uni-polar case)\n'); pause(0);
            setquad(QMS_Vertical, DelQuad+QUAD0, -1);  sleep(HysteresisLoopDwellTime);
            setquad(QMS_Vertical,         QUAD0, -1);  sleep(HysteresisLoopDwellTime);
    ...
    % Start the corrector a little lower first for hysteresis reasons 
    fprintf('   Setting VCM %s(%d,%d)  (%s)\n', VCMFamily, VCMDev(1,:), datestr(now,'yyyy-mm-dd HH:MM:SS.FFF')); drawnow;
...
setsp(VCMFamily, VCM00, VCMDev, 0);
setquad(QMS_Horizontal, QUAD0, 0);
fprintf('   Final reset of all correctors & quads (%s)\n\n', datestr(now,'yyyy-mm-dd HH:MM:SS.FFF')); drawnow;
