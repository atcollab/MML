function copychrorespfile(DataFileName, ToFileName)
%COPYCHRORESPFILE - Copies a chromaticity response matrix file to the golden file 
%  copychrorespfile(FromFileName, ToFileName)
%
%  INPUTS
%  1. FromFileName - File name to get data from
%  2. ToFileName - File name to write data to (Default: Golden Filename) 
%
%  See also copybpmrespfile, copybpmsigmafile, copydispersionfile,
...
if isempty(DataFileName)
    DataDirectoryName = getfamilydata('Directory','ChroResponse');
    if isempty(DataDirectoryName)
        DataDirectoryName = getfamilydata('Directory','DataRoot');
    end
    [DataFileName, DataDirectoryName, FilterIndex] = uigetfile('*.mat','Select the Chromaticity Response Matrix File to Copy', DataDirectoryName);
    if FilterIndex == 0 
        fprintf('   File not copied (copychrorespfile)\n');
        return;
    end
else
...
if strcmpi(ToFileName, 'Golden')
    FileName = [getfamilydata('OpsData','ChroRespFile'),'.mat'];
    DirectoryName = getfamilydata('Directory','OpsData');

    if exist([DirectoryName FileName],'file')
        AnswerString = questdlg(strvcat(strvcat(strvcat('Are you sure you want to overwrite the default chromaticity response matrix file?',sprintf('%s',[DirectoryName FileName])),'With file:'),[DataDirectoryName, DataFileName]),'Default Chromaticity Response','Yes','No','No');
    else
        AnswerString = 'Yes';
    end
