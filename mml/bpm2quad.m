function [QUADFamilyOutput, QUADDevOutput, DeltaSpos, PhaseAdvanceX, PhaseAdvanceY] = bpm2quad(BPMFamily, BPMDev, LocationFlag)
...
for k = 1:size(BPMDev,1)
    ...
    if nargout >= 4
        ...
        i = findrowindex(ATIndex(1), (1:length(PhiQx))');
        PhiQx = (PhiQx(i) + PhiQx(i+1))/2;

        PhaseAdvanceX = PhiQx - PhiX;
    end
    if nargout >= 5
        PhiQy = (PhiQy(i) + PhiQy(i+1))/2;
        PhaseAdvanceY = PhiQy - PhiY;
    end
end
