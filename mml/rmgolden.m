function Data = rmgolden(varargin)
%RMGOLDEN - Remove the golden values for data set
...
%
%  See also getgolden, rmoffset, getoffset
...
elseif iscell(varargin{1})
    ...
    for i = 1:length(varargin{1})
        if length(varargin) == 1
            Data{i} = rmgolden(varargin{1}{i}, UnitsFlagCell{:});
        elseif length(varargin) == 2
            Data{i} = rmgolden(varargin{1}{i}, varargin{2}{i}, UnitsFlagCell{:});
        elseif length(varargin) == 3
            Data{i} = rmgolden(varargin{1}{i}, varargin{2}{i}, varargin{3}{i}, UnitsFlagCell{:});
        else
            Data{i} = rmgolden(varargin{1}{i}, varargin{2}{i}, varargin{3}{i}, varargin{4}{i}, UnitsFlagCell{:});
        end
    end
    return
end
...
Golden = getgolden(Family, Field, DeviceList, UnitsFlagCell{:});

for i = 1:size(Data,1)
    Data(i,:) = Data(i,:) - Golden(i,:); 
end
...
if StructFlag
    if isempty(StructOut)
        ...
        StructOut.DataDescriptor = 'rmgolden';
    end
    StructOut.Data = Data;
    Data = StructOut;
end
