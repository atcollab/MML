...
function Data =  getpvlnls(ChannelNames)


global lnls1;

msg = struct;
modChannelNames = cell(length(ChannelNames(:,1)),1);
for i=1:length(ChannelNames(:,1))
    cName = upper(deblank(ChannelNames(i,:)));
    cName = strrep(cName, ':AM','_AM');
    cName = strrep(cName, ':SP','_SP');
    modChannelNames{i} = cName;
    msg.(cName) = 0;
end
lnls1_comm_read(msg);
Data = zeros(length(modChannelNames),1);
for i=1:length(modChannelNames)
    Data(i,1) = lnls1.ring.state.(modChannelNames{i});
end
