function DataOut = real2raw(varargin)
%REAL2RAW - Converts "real" data (calibrated values) to control system data
%  RawData = real2raw(Family, Field, RawData, DeviceList)
%  RawData = real2raw(DataStruct)
%
%  INPUTS
...
%
%  OUTPUTS
%  1. RawData - Raw control system values
...
%
%  See also raw2real, physics2hw, hw2physics, getoffset, getgain, getgolden
...
for i = length(varargin):-1:1
    ...
    elseif strcmpi(varargin{i},'physics')
        fprintf('   WARNING: ''Physics'' flag ignored by real2raw\n');
        varargin(i) = [];
    elseif strcmpi(varargin{i},'hardware')
        fprintf('   WARNING: ''Hardware'' flag ignored by real2raw\n');
        varargin(i) = [];
    end
end
...
if iscell(Family)
    ...
    for i = 1:length(Family)
        if length(varargin) == 1
            DataOut{i} = real2raw(Family{i}, InputFlags{:});
        elseif length(varargin) == 2
            DataOut{i} = real2raw(Family{i}, Field{i}, InputFlags{:});
        elseif length(varargin) == 3
            DataOut{i} = real2raw(Family{i}, Field{i}, DataIn{i}, InputFlags{:});
        else
            DataOut{i} = real2raw(Family{i}, Field{i}, DataIn{i}, DeviceList{i}, InputFlags{:});
        end
    end
    return    
end  
        ...
        for k = 1:size(DataOut,2)
            ...
            if isfield(DataOut(j,k),'Monitor') && isfield(DataOut(j,k),'Actuator')
                ...
                if strcmpi(IncrementalFlag, 'Absolute') && j==1 && k==1
                    fprintf('   WARNING:  ''Absolute'' flag passed to real2raw for a response matrix.  Flag ignored!');
                end
                
                % Incrementally scale rows by Monitor and Columns by Actuators
                DataOut(j,k).Data = real2raw(DataOut(j,k).Monitor.FamilyName, DataOut(j,k).Monitor.Field, DataOut(j,k).Data, DataOut(j,k).Monitor.DeviceList, 'Incremental');
                DataOut(j,k).Data = real2raw(DataOut(j,k).Actuator.FamilyName, DataOut(j,k).Actuator.Field, DataOut(j,k).Data', DataOut(j,k).Actuator.DeviceList, 'Incremental')';
                
                DataOut(j,k).Monitor  = real2raw(DataOut(j,k).Monitor,  'Absolute'); 
                DataOut(j,k).Actuator = real2raw(DataOut(j,k).Actuator, 'Absolute');
                if isfield(DataOut(j,k), 'ActuatorDelta')
                    DataOut(j,k).ActuatorDelta = real2raw(DataOut(j,k).Actuator.FamilyName, DataOut(j,k).Actuator.Field, DataOut(j,k).ActuatorDelta, [1 1], 'Incremental');
                end
                                
            elseif isfield(DataOut(j,k),'FamilyName') && isfield(DataOut(j,k),'Field')
                % Data structure
                DataOut(j,k).Data = real2raw(DataOut(j,k).FamilyName, DataOut(j,k).Field, DataOut(j,k).Data, DataOut(j,k).DeviceList, IncrementalFlag{:});
            ...
            if isfield(DataOut(j,k), 'CreatedBy')
                DataOut(j,k).CreatedBy = ['real2raw,', DataOut(j,k).CreatedBy];
            end
        end
    end
...
FunctionHandle = getfamilydata(Family, Field, 'Real2RawFcn');
if ~isempty(FunctionHandle)
    ...
    for i = 1:size(DataIn,2)
        DataOut(:,i) = (DataIn(:,i) ./ Gain) + Offset;
    end
