function copydisprespfile(DataFileName, ToFileName)
%COPYDISPRESPFILE - Copies a dispersion response matrix file to the golden file 
...
%           copybpmrespfile, copymachineconfigfile, copytunerespfile
...
if isempty(DataFileName)
    DataDirectoryName = getfamilydata('Directory','DispResponse');
    if isempty(DataDirectoryName)
        DataDirectoryName = getfamilydata('Directory','DataRoot');
    end
    [DataFileName, DataDirectoryName, FilterIndex] = uigetfile('*.mat','Select the Dispersion Response Matrix File to Copy', DataDirectoryName);
    if FilterIndex == 0 
        fprintf('   File not copied (copydisprespfile)\n');
        return;
    end
else
...
if strcmpi(ToFileName, 'Golden')
    FileName = [getfamilydata('OpsData','DispRespFile'),'.mat'];
    DirectoryName = getfamilydata('Directory','OpsData');
    
    if exist([DirectoryName FileName],'file')
        AnswerString = questdlg(strvcat(strvcat(strvcat('Are you sure you want to overwrite the default dispersion response matrix file?',sprintf('%s',[DirectoryName FileName])),'With file:'),[DataDirectoryName, DataFileName]),'Default Dispersion Response','Yes','No','No');
    else
        AnswerString = 'Yes';
    end
