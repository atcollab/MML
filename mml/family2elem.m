function ElemList = family2elem(Family, varargin)
%FAMILY2ELEM - Return the device list for a family
...
%                Empty if not found
%
%  See also family2dev, family2common, family2status, family2tol, family2units
...
if iscell(Family)
    for i = 1:length(Family)
        if iscell(StatusFlag) && iscell(PowerSupplyFlag)
            ElemList{i} = family2dev(Family{i}, StatusFlag{i}, PowerSupplyFlag{i});
        elseif iscell(StatusFlag)
            ElemList{i} = family2dev(Family{i}, StatusFlag{i}, PowerSupplyFlag);
        elseif iscell(PowerSupplyFlag)
            ElemList{i} = family2dev(Family{i}, StatusFlag, PowerSupplyFlag{i});
        else
            ElemList{i} = family2dev(Family{i}, StatusFlag, PowerSupplyFlag);
        end
    end
    return
end
...
ElemList = dev2elem(Family, DeviceList);
