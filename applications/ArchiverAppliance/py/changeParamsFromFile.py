...
def parseImportArgs():
	parser = ArchArgumentParser(description="Change sampling period and/or method for PVs", fromfile_prefix_chars='@')
	...
	parser.add_argument('-t', '--time', type=float, help="Sampling interval in seconds", default=60)
	parser.add_argument('-m', '--method', help="Sampling method, either SCAN or MONITOR", default="MONITOR")
	parser.add_argument('-l', '--path', help="Path to files")
...
if __name__ == '__main__':
	...
	for fname in args.files:
		...
		for pvname in f:
			...
			if len(pvname) == 0:
				continue
			logging.debug('Changing pvname "%s"' % pvname)
			response = client.changeArchivalParams(pvname, args.time,
					args.method)
			if response is None:
				logging.critical('Failed to submit new pvs')
				sys.exit()

			logging.info('pv %s has been submitted.  Response:' % pvname)
			print response
