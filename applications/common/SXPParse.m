function [freq, data, freq_noise, data_noise, Zo, raw_data] = SXPParse(DataFileName, fid_log)

% reads .sxp file data in MDIF (a.k.a. Touchstone / HPEEsof format)
%
% EXAMPLE :
% [freq, data, freq_noise, data_noise, Zo, raw_data] = SXPParse(DataFileName, fid_log);
%
...
if NoOfPorts == 1
    raw_data_A(1,1,:) = raw_data(:,1); 
    raw_data_B(1,1,:) = raw_data(:,2);
elseif NoOfPorts == 2
    raw_data_A(1,1,:) = raw_data(:,1); 
    raw_data_B(1,1,:) = raw_data(:,2);
    raw_data_A(2,1,:) = raw_data(:,3); 
    raw_data_B(2,1,:) = raw_data(:,4);
    raw_data_A(1,2,:) = raw_data(:,5); 
    raw_data_B(1,2,:) = raw_data(:,6);
    raw_data_A(2,2,:) = raw_data(:,7); 
    raw_data_B(2,2,:) = raw_data(:,8);
...
switch opt.format
    ...
    case 'db'
        t_mag = 10.^(raw_data_A/20); 
        t_ang = raw_data_B*pi/180;
