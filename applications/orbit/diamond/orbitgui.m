...
SYS.machine='Diamond';
SYS.restorefile='DiamondRestore';
SYS.mode='SIMULATOR';
SYS.bpmode='Libera';
SYS.plane=1;
...
SYS.units='Hardware';   %default to hardware units
...
Diamondelementicons;            %initialize element icons (append fields to THERING)
...
%Load Gain and offsets from Physdata - Alba routine is OK for Diamond
[BPM,COR,BL]=orbgui('LoadALBAPhysData',BPM,COR,BL);
...
BPM(1).phi=(20/Circumference)*BPM(1).z(:);        %...phase advance
BPM(2).phi= (10/Circumference)*BPM(2).z(:);
COR(1).phi=(20/Circumference)*COR(1).z(:);
COR(2).phi= (10/Circumference)*COR(2).z(:);
