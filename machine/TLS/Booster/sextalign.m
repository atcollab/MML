...
for ii=1:length(QuadList),
    ...
    tol=1e-4; %tolerance for spacing between magnets
...
V = zeros(1,6);
for i = 1:length(ATindx)
   V(1) = -mx(i);
   V(3) = -my(i);
   THERING{ATindx(i,1)}.T1 =  V;
   THERING{ATindx(i,1)}.T2 = -V;
   THERING{ATindx(i,2)}.T1 = -V;
   THERING{ATindx(i,2)}.T2 =  V;
end
