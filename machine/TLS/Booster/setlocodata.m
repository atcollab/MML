...
if any(strcmpi(CommandInput, 'Nominal'))
    ...
    AO.SFQ.Gain = ones(size(family2dev('SFQ',0),1),1);
    AO.SDQ.Gain = ones(size(family2dev('SDQ',0),1),1);
%     AO.S1.Gain = ones(size(family2dev('S1',0),1),1);
%     AO.S2.Gain = ones(size(family2dev('S2',0),1),1);
