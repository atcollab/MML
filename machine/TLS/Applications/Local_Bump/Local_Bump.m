function varargout = Local_Bump(varargin)
% Local_Bump M-file for Local_Bump.fig
%      Local_Bump, by itself, creates a new Local_Bump or raises the existing
%      singleton*.
%
%      H = Local_Bump returns the handle to a new Local_Bump or the handle to
%      the existing singleton*.
%
%      Local_Bump('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in Local_Bump.M with the given input arguments.
%
%      Local_Bump('Property','Value',...) creates a new Local_Bump or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Local_Bump_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Local_Bump_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Local_Bump

% Last Modified by GUIDE v2.5 16-Sep-2011 10:41:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Local_Bump_OpeningFcn, ...
                   'gui_OutputFcn',  @Local_Bump_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Local_Bump is made visible.
function Local_Bump_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Local_Bump (see VARARGIN)

% Choose default command line output for Local_Bump
...
function varargout = Local_Bump_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in HCM.
function HCM_Callback(hObject, eventdata, handles)
% hObject    handle to HCM (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.Device, 'String', []);
set(handles.Device, 'Value', []);
H = get(handles.HCM, 'Value');
HCM = getfamilydata('HCM');
if H == 1
    set(handles.Device, 'String', num2str(HCM.DeviceList(HCM.ElementList,:),'[ %2d, %2d ]'));
else
    set(handles.Device, 'String', []);
end
getbpm_BBA('BPMx',[],handles);
...
% Hint: get(hObject,'Value') returns toggle state of HCM
...
Current = get(handles.uipanel5,'UserData');
Option = get(handles.Method,'Value')-1;
BPM = get(handles.BPM,'UserData');
if length(DeviceList) == 3
    if strcmpi(get(handles.F1,'Checked'),'on')
        if strcmpi(Family,'HCM')
            Position = str2num(get(handles.Position,'String'));
            P1 = getpv('BPMx','Monitor',BPM);
            theta = localbump_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),0.00001,handles);
            getbpm_BBA('BPMx',BPM,handles);
            getcm_BBA(Family,DeviceList(1,:),handles);
            P2 = getpv('BPMx','Monitor',BPM);
            delta_P = P2-P1;
            while abs(Position - getpv('BPMx','Monitor',BPM)) > 0.0001
                delta = 0.00001/delta_P*(Position-getpv('BPMx','Monitor',BPM));
                theta = localbump_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),delta,handles);
                getbpm_BBA('BPMx',BPM,handles);
                getcm_BBA(Family,DeviceList(1,:),handles);
            end
        elseif strcmpi(Family,'VCM')
            Position = str2num(get(handles.Position,'String'));
            P1 = getpv('BPMy','Monitor',BPM);
            theta = localbump_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),0.00001,handles);
            getbpm_BBA('BPMy',BPM,handles);
            getcm_BBA(Family,DeviceList(1,:),handles);
            P2 = getpv('BPMy','Monitor',BPM);
            delta_P = P2-P1;
            while abs(Position - getpv('BPMy','Monitor',BPM)) > 0.0001
                delta = 0.00001/delta_P*(Position-getpv('BPMy','Monitor',BPM));
                theta = localbump_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),delta,handles);
                getbpm_BBA('BPMy',BPM,handles);
                getcm_BBA(Family,DeviceList(1,:),handles);
            end
        end
    elseif strcmpi(get(handles.F2,'Checked'),'on')
        theta = localbump(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),handles);
    end
elseif length(DeviceList) == 4
    if strcmpi(get(handles.F1,'Checked'),'on')
        BPM = get(handles.BPM,'UserData');
        if strcmpi(Family,'HCM')
            if get(handles.Method,'Value') == 2
                Position = str2num(get(handles.Position,'String'));
                P1 = getpv('BPMx','Monitor',BPM);
                theta = localbump4_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,0.00001,handles);
                getbpm_BBA('BPMx',BPM,handles);
                getcm_BBA(Family,DeviceList(1,:),handles);
                P2 = getpv('BPMx','Monitor',BPM);
                delta_P = P2-P1;
                while abs(Position - getpv('BPMx','Monitor',BPM)) > 0.0001
                    delta = 0.00001/delta_P*(Position-getpv('BPMx','Monitor',BPM));
                    theta = localbump4_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,delta,handles);
                    getbpm_BBA('BPMx',BPM,handles);
                    getcm_BBA(Family,DeviceList(1,:),handles);
                end
            elseif get(handles.Method,'Value') == 3
                Angle = str2num(get(handles.Position,'String'));
                theta1 = localbump4_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,-0.00001,handles);
                set(handles.RPS2,'String',theta1);
                getbpm_BBA('BPMx',BPM,handles);
                getcm_BBA(Family,DeviceList(1,:),handles);
                theta2 = localbump4_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,0.00001,handles);
                set(handles.RPS2,'String',theta2);
                getbpm_BBA('BPMx',BPM,handles);
                getcm_BBA(Family,DeviceList(1,:),handles);
                delta_theta = theta2-theta1;
                while abs(Angle - theta2) > 1e-6
                    delta = 0.00001/delta_theta*(Angle-theta2);
                    theta2 = localbump4_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,delta,handles);
                    set(handles.RPS2,'String',theta2);
                    getbpm_BBA('BPMx',BPM,handles);
                    getcm_BBA(Family,DeviceList(1,:),handles);
                end
    ...
    elseif strcmpi(get(handles.F2,'Checked'),'on')
        BPM = get(handles.Reference,'UserData');
        theta = localbump4(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,handles);
    end
end
if strcmpi(Family,'HCM')
    getbpm_BBA('BPMx',BPM,handles);
...
set(handles.Device, 'UserData', Data);
QF5S = getfamilydata('QF5S');
set(handles.Device, 'String', num2str(QF5S.DeviceList(QF5S.ElementList,:),'[ %2d, %2d ]'));
set(handles.Device, 'Value', Data(:,8));
...
DeviceList = get(handles.Device,'UserData');
Current = get(handles.uipanel5,'UserData');
setsp(Family,Current,DeviceList);
getbpm_BBA('BPMx',[],handles);
getcm_BBA('HCM',[],handles);
set(handles.HCM, 'Value', 0);
set(handles.VCM, 'Value', 0);
set(handles.Device, 'String', []);
...
set(handles.Reference,'Value',1);
set(handles.Reference,'String','Reference BPM');
set(handles.Position,'String',[]);
set(handles.slider3, 'Value', 0);
set(handles.text68,'String','Position');
set(handles.text66,'String','mm');
% set(handles.Angle,'Visible','off');
set(handles.slider3,'Max',10);
set(handles.slider3,'Min',-10);
set(handles.BPM,'String',' ');
set(handles.BPM,'Value',1);
set(handles.R1,'String',' ');
set(handles.R2,'String',' ');
set(handles.R3,'String',' ');
set(handles.R4,'String',' ');
set(handles.RPS1,'String',' ');
set(handles.RPS2,'String',' ');
set(handles.BPM,'UserData',[]);
set(handles.Reference,'UserData',[]);
...
% --- Executes on button press in VCM.
function VCM_Callback(hObject, eventdata, handles)
% hObject    handle to VCM (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.Device, 'String', []);
set(handles.Device, 'Value', []);
V = get(handles.VCM, 'Value');
VCM = getfamilydata('VCM');
if V == 1
    set(handles.Device, 'String', num2str(VCM.DeviceList(VCM.ElementList,:),'[ %2d, %2d ]'));
else
    set(handles.Device, 'String', []);
end
...
% Hint: get(hObject,'Value') returns toggle state of VCM
...
set(handles.Device, 'String', []);
set(handles.Device, 'Value', []);
Q6 = get(handles.Q6, 'Value');
QD3S = getfamilydata('QD3S');
if Q6 == 1
    set(handles.Device, 'String', num2str(QD3S.DeviceList(QD3S.ElementList,:),'[ %2d, %2d ]'));
else
    set(handles.Device, 'String', []);
end
...
set(handles.Device, 'String', []);
set(handles.Device, 'Value', []);
Q7 = get(handles.Q7, 'Value');
QD4S = getfamilydata('QD4S');
if Q7 == 1
    set(handles.Device, 'String', num2str(QD4S.DeviceList(QD4S.ElementList,:),'[ %2d, %2d ]'));
else
    set(handles.Device, 'String', []);
end
...
% if get(handles.HCM, 'Value') == 1
%     Data1 = get(handles.Device, 'Value');
% else
%     set(handles.Device, 'Value', []);
% end
...
elseif get(handles.VCM, 'Value') == 1
    Family = 'VCM';
...
function theta = localbump(Family,D1,D2,D3,handles)
switch2physics;
C1 = hw2physics(Family, 'Setpoint',str2num(get(handles.Current,'String')),D1);
C2 = getsp(Family,D2);
C3 = getsp(Family,D3);
Index1 = family2atindex(Family,D1);
Index2 = family2atindex(Family,D2);
Index3 = family2atindex(Family,D3);
Optics = gettwiss;
if strcmpi(Family,'HCM')
    beta1 = Optics.betax(Index1);
    phi1 = Optics.phix(Index1);
    beta2 = Optics.betax(Index2);
    phi2 = Optics.phix(Index2);
    beta3 = Optics.betax(Index3);
    phi3 = Optics.phix(Index3);    
    alfa3 = Optics.alfax(Index3);
elseif strcmpi(Family,'VCM')
    beta1 = Optics.betay(Index1);
    phi1 = Optics.phiy(Index1);
    beta2 = Optics.betay(Index2);
    phi2 = Optics.phiy(Index2);
    beta3 = Optics.betay(Index3);
    phi3 = Optics.phiy(Index3);  
    alfa3 = Optics.alfay(Index3);
end
C2 = C1*(-1)*sqrt(beta1/beta2)*sin((phi3-phi1)*2*pi)/sin((phi3-phi2)*2*pi);
C3 = C1*sqrt(beta1/beta3)*sin((phi2-phi1)*2*pi)/sin((phi3-phi2)*2*pi);
theta = C1*sqrt(beta1/beta3)*(cos((phi3-phi1)*2*pi)-alfa3*sin((phi3-phi1)*2*pi))+C2*sqrt(beta2/beta3)*(cos((phi3-phi2)*2*pi)-alfa3*sin((phi3-phi2)*2*pi))+C3;
position = C1*sqrt(beta1*beta3)*sin((phi3-phi1)*2*pi)+C2*sqrt(beta2*beta3)*sin((phi3-phi2)*2*pi);
set(handles.RPS1,'String',num2str(position,'%5.3e'));
set(handles.RPS2,'String',num2str(theta,'%5.3e'));
set(handles.R1,'String',1);
set(handles.R2,'String',C2/C1);
set(handles.R3,'String',C3/C1);
setsp(Family,C1,D1);
setsp(Family,C2,D2);
setsp(Family,C3,D3);
theta = [C1;C2;C3];
...
function theta = localbump_2(Family,D1,D2,D3,delta,handles)
switch2physics;
C1 = getsp(Family,D1)+delta;
C2 = getsp(Family,D2);
C3 = getsp(Family,D3);
Index1 = family2atindex(Family,D1);
Index2 = family2atindex(Family,D2);
...
%         set(handles.Angle,'Visible','off');
    elseif get(handles.Method,'Value') == 3
        set(handles.text68,'String','Angle');
        set(handles.text66,'String','rad');
%         set(handles.Angle,'Visible','on');
        theta = localbump4_2(Family,DeviceList(1,:),DeviceList(2,:),DeviceList(3,:),DeviceList(4,:),BPM,Option,0,handles);
%         set(handles.Angle,'String',num2str(theta,'%6.3e'));
        set(handles.slider3,'Max',30);
        set(handles.slider3,'Min',-30);
    else
%         set(handles.Angle,'Visible','off');
        set(handles.text68,'String','Position');
        set(handles.text66,'String','mm');
...
function theta = localbump4(Family,D1,D2,D3,D4,BPM,Option,handles)
switch2physics;
C1 = hw2physics(Family, 'Setpoint',str2num(get(handles.Current,'String')),D1);
C2 = getsp(Family,D2);
C3 = getsp(Family,D3);
...
Index2 = family2atindex(Family,D2);
Index3 = family2atindex(Family,D3);
Index4 = family2atindex(Family,D4);
Optics = gettwiss;
if strcmpi(Family,'HCM')
    BPMindex = family2atindex('BPMx',BPM);
    alfaBPM = Optics.alfax(BPMindex);
    phiBPM = Optics.phix(BPMindex);
    betaBPM = Optics.betax(BPMindex);
    beta1 = Optics.betax(Index1);
    phi1 = Optics.phix(Index1);
    beta2 = Optics.betax(Index2);
    phi2 = Optics.phix(Index2);
    beta3 = Optics.betax(Index3);
    phi3 = Optics.phix(Index3);    
    beta4 = Optics.betax(Index4);
    phi4 = Optics.phix(Index4); 
    alfa4 = Optics.alfax(Index4);
elseif strcmpi(Family,'VCM')
    BPMindex = family2atindex('BPMy',BPM);
    alfaBPM = Optics.alfay(BPMindex);
    phiBPM = Optics.phiy(BPMindex);
    betaBPM = Optics.betay(BPMindex);
    beta1 = Optics.betay(Index1);
    phi1 = Optics.phiy(Index1);
    beta2 = Optics.betay(Index2);
    phi2 = Optics.phiy(Index2);
    beta3 = Optics.betay(Index3);
    phi3 = Optics.phiy(Index3);  
    beta4 = Optics.betay(Index4);
    phi4 = Optics.phiy(Index4); 
    alfa4 = Optics.alfay(Index4);
end
if Option == 1
    C2 = C1*(-1)*sqrt(beta1/beta2)*(cos((phiBPM-phi1)*2*pi)-alfaBPM*sin((phiBPM-phi1)*2*pi))/(cos((phiBPM-phi2)*2*pi)-alfaBPM*sin((phiBPM-phi2)*2*pi));
    C3 = C1*(-1)*sqrt(beta1/beta3)*sin((phi4-phi1)*2*pi)/sin((phi4-phi3)*2*pi)-C2*sqrt(beta2/beta3)*sin((phi4-phi2)*2*pi)/sin((phi4-phi3)*2*pi);
    C4 = C1*(-1)*sqrt(beta1/beta4)*(cos((phi4-phi1)*2*pi)-alfa4*sin((phi4-phi1)*2*pi))-C2*sqrt(beta2/beta4)*(cos((phi4-phi2)*2*pi)-alfa4*sin((phi4-phi2)*2*pi))-C3*sqrt(beta3/beta4)*(cos((phi4-phi3)*2*pi)-alfa4*sin((phi4-phi3)*2*pi));
elseif Option == 2
    C2 = C1*(-1)*sqrt(beta1/beta2)*sin((phiBPM-phi1)*2*pi)/sin((phiBPM-phi2)*2*pi);
    C3 = C1*(-1)*sqrt(beta1/beta3)*sin((phi4-phi1)*2*pi)/sin((phi4-phi3)*2*pi)-C2*sqrt(beta2/beta3)*sin((phi4-phi2)*2*pi)/sin((phi4-phi3)*2*pi);
    C4 = C1*(-1)*sqrt(beta1/beta4)*(cos((phi4-phi1)*2*pi)-alfa4*sin((phi4-phi1)*2*pi))-C2*sqrt(beta2/beta4)*(cos((phi4-phi2)*2*pi)-alfa4*sin((phi4-phi2)*2*pi))-C3*sqrt(beta3/beta4)*(cos((phi4-phi3)*2*pi)-alfa4*sin((phi4-phi3)*2*pi));
end
if C1 ~= 0
    ...
    set(handles.R2,'String',C2/C1);
    set(handles.R3,'String',C3/C1);
    set(handles.R4,'String',C4/C1);
end
theta = C1*sqrt(beta1/betaBPM)*(cos((phiBPM-phi1)*2*pi)-alfaBPM*sin((phiBPM-phi1)*2*pi))+C2*sqrt(beta2/betaBPM)*(cos((phiBPM-phi2)*2*pi)-alfaBPM*sin((phiBPM-phi2)*2*pi));
position = C1*sqrt(beta1*betaBPM)*sin((phiBPM-phi1)*2*pi)+C2*sqrt(beta2*betaBPM)*sin((phiBPM-phi2)*2*pi);
set(handles.RPS1,'String',num2str(position,'%5.3e'));
set(handles.RPS2,'String',num2str(theta,'%5.3e'));
setsp(Family,C1,D1);
setsp(Family,C2,D2);
...
function theta = localbump4_2(Family,D1,D2,D3,D4,BPM,Option,delta,handles)
switch2physics;
C1 = getsp(Family,D1)+delta;
C2 = getsp(Family,D2);
C3 = getsp(Family,D3);
