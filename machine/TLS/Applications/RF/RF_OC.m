function varargout = RF_OC(varargin)
% RF_OC MATLAB code for RF_OC.fig
%      RF_OC, by itself, creates a new RF_OC or raises the existing
%      singleton*.
%
%      H = RF_OC returns the handle to a new RF_OC or the handle to
%      the existing singleton*.
%
%      RF_OC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RF_OC.M with the given input arguments.
%
%      RF_OC('Property','Value',...) creates a new RF_OC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before RF_OC_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to RF_OC_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help RF_OC

% Last Modified by GUIDE v2.5 21-Nov-2011 16:51:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @RF_OC_OpeningFcn, ...
                   'gui_OutputFcn',  @RF_OC_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before RF_OC is made visible.
function RF_OC_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to RF_OC (see VARARGIN)

% Choose default command line output for RF_OC
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
OCdefault('FitRF','NoSetPV',handles);
% UIWAIT makes RF_OC wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = RF_OC_OutputFcn(hObject, eventdata, handles) 
...
function OCdefault(Evectors,varargin)
...
% Corrector orbit
OC('Golden', BPM, CM, Iters, Evectors, DisplayFlag, varargin, handles);




function [OCS, OCS0, V, S, ErrorFlag] = OC(varargin)
...
if OCS.Incremental
    ...
    % The incremental flag is always zero on exit because the orbits are absolute at this point
    OCS.Incremental = 0;
end
...
%         [FigHandles, AxesHandles, OCS] = PreCorrectionDisplay(FigHandles, OCS0, OCS, Smat, S, U, V, NumberOfTrys, MorePlotsFlag, InputFlags, handles);
%     else
%         SVDquestion = 'Correct Orbit';
%     end
    if get(handles.SVindex,'UserData') == 0
        SVindex = {sprintf('[%d:%d]',OCS.SVDIndex(1),OCS.SVDIndex(end))};
        set(handles.SVindex,'String',SVindex);
    end
    SVindex = get(handles.SVindex,'String');
    tmp = str2num(SVindex{1});
...
for iloop = 1:NumberOfTrys
    ...
    if OCS.Display && OCS.NIter && SetSPFlag
        PostCorrectionDisplay(AxesHandles, OCS0, OCS, Smat, S, U, V, iloop, NumberOfTrys, MorePlotsFlag, InputFlags, handles);
    end
...
set(handles.RF1,'String',sprintf('%6.8E',getrf));
set(handles.RF2,'String',sprintf('%6.8E',getrf-OCS0.RF.Data));
set(handles.FQ,'UserData',OCS0.RF.Data);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [H, Haxis, OCS] = PreCorrectionDisplay(H, OCS0, OCS, Smat, S, U, V, NumberOfTrys, MorePlotsFlag, InputFlags, handles)
...
%     figure(H(1));
%     clf reset
    Haxis(1) = handles.axes1;
    Haxis(2) = handles.axes2;
    Haxis(3) = handles.axes3;
%     yaxesposition(1.05);
    orient portrait
else
    Haxis = H;
...
set(handles.RF1,'String',sprintf('%6.8E',getrf));
set(handles.RF2,'String',sprintf('%6.8E',getrf-OCS.RF.Data));
...
for i4 = 1:length(OCS.CM)
    ...
    % Absolute corrector change
    plot(CMpos, CM + OCS.CM{i4}.Delta(isort), '.-','Color', ColorOrderMat(mod(i4,size(ColorOrderMat,1)),:));
    LabelCell4{i4} = sprintf('%s [%s]', OCS.CM{i4}.FamilyName, OCS.CM{i4}.UnitsString);
    hold on;
end
hold off;
if length(OCS.CM) == 1
    %ylabel(sprintf('{\\Delta}%s [%%Max]', OCS.CM{1}.FamilyName));
    ylabel(sprintf('%s [%s]', OCS.CM{1}.FamilyName, OCS.CM{1}.UnitsString));
else
    %ylabel('\Delta Corrector [%Max]');
    ylabel('Corrector Strength');
    h_legend = legend(LabelCell4);
    set(h_legend, 'FontSize', 8);
    set(h_legend, 'UserData', 'Axes1');
end
...
set(handles.X1,'String', sprintf('%6.3E',mean(OCS.BPM{1}.Data)));
set(handles.X2,'String', sprintf('%6.3E',std(OCS.BPM{1}.Data)));
set(handles.Y1,'String', sprintf('%6.3E',mean(OCS.BPM{2}.Data)));
set(handles.Y2,'String', sprintf('%6.3E',std(OCS.BPM{2}.Data)));
hold off
...
if OCS.FitRF
    ...
    if length(LabelCellCM) == 1
        ...
        %h_legend = legend(Haxis(1), LabelCellCM, 0);
        %set(h_legend, 'FontSize', 8);
...
if length(LabelCell2) == 1
    ...
    %h_legend = legend(Haxis(2), LabelCell2, 0);
    %set(h_legend, 'FontSize', 8);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function PostCorrectionDisplay(H, OCS0, OCS, Smat, S, U, V, Iteration, NumberOfTrys, MorePlotsFlag, InputFlags, handles)
...
set(handles.X1,'String', sprintf('%6.3E',mean(BPMx)));
set(handles.X2,'String', sprintf('%6.3E',std(BPMx)));
set(handles.Y1,'String', sprintf('%6.3E',mean(BPMy)));
set(handles.Y2,'String', sprintf('%6.3E',std(BPMy)));
...
function SVindex_Callback(hObject, eventdata, handles)
% hObject    handle to SVindex (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.SVindex,'UserData',1);
OCdefault('FitRF', 'NoSetPV', handles);

% Hints: get(hObject,'String') returns contents of SVindex as text
%        str2double(get(hObject,'String')) returns contents of SVindex as a double


% --- Executes during object creation, after setting all properties.
function SVindex_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SVindex (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
% Hints: get(hObject,'String') returns contents of RF1 as text
%        str2double(get(hObject,'String')) returns contents of RF1 as a double


% --- Executes during object creation, after setting all properties.
function RF1_CreateFcn(hObject, eventdata, handles)
% hObject    handle to RF1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% Hints: get(hObject,'String') returns contents of RF2 as text
%        str2double(get(hObject,'String')) returns contents of RF2 as a double


% --- Executes during object creation, after setting all properties.
function RF2_CreateFcn(hObject, eventdata, handles)
% hObject    handle to RF2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in CP1.
function CP1_Callback(hObject, eventdata, handles)
% hObject    handle to CP1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns CP1 contents as cell array
%        contents{get(hObject,'Value')} returns selected item from CP1
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.CP1,'Value',1);
RF = get(handles.FQ,'UserData');
setsp('RF',RF);
OCdefault('FitRF','NoSetPV',handles);
