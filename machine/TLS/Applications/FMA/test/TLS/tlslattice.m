...
R4D12A = drift('R4D12A', 0.0695, 'DriftPass'); % 02/10/2010
R4D12B = drift('R4D12B', 0.0705, 'DriftPass'); % 02/10/2010
R4D13A = drift('R4D13A', 0.243, 'DriftPass'); % 02/10/2010
R4D13B = drift('R4D13B', 0.138, 'DriftPass'); % 02/10/2010
R4D14A = drift('R4D14A', 0.17, 'DriftPass'); % 02/10/2010
R4D14B = drift('R4D14B', 0.231, 'DriftPass'); % 02/10/2010
...
% Super-Conducting Multi-Pole Wiggler IASWB (16 pole, anti-symmetry)
...
% R4-IASW6 In-Arc Super-conducting Wiggler (IASWC)
IASWCHC1 = marker('IASWCHC1', 'IdentityPass'); % 02/10/2010
IASWCHC2 = marker('IASWCHC2', 'IdentityPass'); % 02/10/2010
IASWCD = drift('IASWCD', 0.488, 'DriftPass'); % 02/10/2010
% Super-Conducting Multi-Pole Wiggler IASWC (16 pole, anti-symmetry)
IASWC.device = {'IASWCMPS'}; % setting
...
IASWC.PeakField = [0 0.5013 0.5577 0.5768 0.5998 0.6234 0.6461 0.6553 0.6812 ...
                   0.695 0.708 0.7214 1.0981 1.6089 2.5348 2.9927 3.1 3.22 ...
		   3.3397];
IASWC.NumberOfPoles = 32;
IASWC.NumberOfEndPoles = 1;
IASWC.Mode = 0;
IASWC.Type = 'AGID';
IASWC.Symmetry = 0;
IASWC.PoleWidth = 0.0305;
IASWC.PeriodLength = 2*IASWC.PoleWidth;
% Driving current
%IASWCMPS = 255; % Amp
IASWCMPS = 0; % Amp
if (IASWCMPS < IASWC.Parameter(1)) | (IASWCMPS > IASWC.Parameter(end))
    error('Current is out of rang [minimum(%f) maximum(%f)].', ...
    IASWC.Parameter(1), IASWC.Parameter(end));
end
IASWC.Field = interp1(IASWC.Parameter,IASWC.PeakField,IASWCMPS,'linear');
IASWCB0 = IASWC.Field;
...
% slices of poles
%------------------------------------------------------------------------------
IASWCPEA = rbend('IASWC', 0.0061,  IASWCEP(1), IASWCEP(1)/2, IASWCEP(1)/2, 0, 'BendLinearPass');
IASWCPEB = rbend('IASWC', 0.0061,  IASWCEP(2), IASWCEP(2)/2, IASWCEP(2)/2, 0, 'BendLinearPass');
IASWCPEC = rbend('IASWC', 0.00305, IASWCEP(3), IASWCEP(3)/2, IASWCEP(3)/2, 0, 'BendLinearPass');
IASWCNEA = rbend('IASWC', 0.0061,  IASWCEN(1), IASWCEN(1)/2, IASWCEN(1)/2, 0, 'BendLinearPass');
IASWCNEB = rbend('IASWC', 0.0061,  IASWCEN(2), IASWCEN(2)/2, IASWCEN(2)/2, 0, 'BendLinearPass');
IASWCNEC = rbend('IASWC', 0.00305, IASWCEN(3), IASWCEN(3)/2, IASWCEN(3)/2, 0, 'BendLinearPass');
IASWCP2A = rbend('IASWC', 0.0061,  IASWC2P(1), IASWC2P(1)/2, IASWC2P(1)/2, 0, 'BendLinearPass');
IASWCP2B = rbend('IASWC', 0.0061,  IASWC2P(2), IASWC2P(2)/2, IASWC2P(2)/2, 0, 'BendLinearPass');
IASWCP2C = rbend('IASWC', 0.00305, IASWC2P(3), IASWC2P(3)/2, IASWC2P(3)/2, 0, 'BendLinearPass');
IASWCN2A = rbend('IASWC', 0.0061,  IASWC2N(1), IASWC2N(1)/2, IASWC2N(1)/2, 0, 'BendLinearPass');
IASWCN2B = rbend('IASWC', 0.0061,  IASWC2N(2), IASWC2N(2)/2, IASWC2N(2)/2, 0, 'BendLinearPass');
IASWCN2C = rbend('IASWC', 0.00305, IASWC2N(3), IASWC2N(3)/2, IASWC2N(3)/2, 0, 'BendLinearPass');
IASWCPA  = rbend('IASWC', 0.0061,  IASWCP(1),  IASWCP(1)/2,  IASWCP(1)/2,  0, 'BendLinearPass');
IASWCPB  = rbend('IASWC', 0.0061,  IASWCP(2),  IASWCP(2)/2,  IASWCP(2)/2,  0, 'BendLinearPass');
IASWCPC  = rbend('IASWC', 0.00305, IASWCP(3),  IASWCP(3)/2,  IASWCP(3)/2,  0, 'BendLinearPass');
IASWCNA  = rbend('IASWC', 0.0061,  IASWCN(1),  IASWCN(1)/2,  IASWCN(1)/2,  0, 'BendLinearPass');
IASWCNB  = rbend('IASWC', 0.0061,  IASWCN(2),  IASWCN(2)/2,  IASWCN(2)/2,  0, 'BendLinearPass');
IASWCNC  = rbend('IASWC', 0.00305, IASWCN(3),  IASWCN(3)/2,  IASWCN(3)/2,  0, 'BendLinearPass');
...
% Bending magnets
BEND = rbend('BEND',1.22, BBANGLE, BBANGLE/2, BBANGLE/2, -0.366596, 'BendLinearPass');
HDM = rbend('HDM', 0.61, BBANGLE/2, BBANGLE/4, BBANGLE/4, -0.366596, 'BendLinearPass');

%Standard Cell Quadrupoles
Q1 = quadrupole('QD1', 0.35, -1.5302, 'QuadLinearPass');
Q2 = quadrupole('QF1', 0.35, 2.8814, 'QuadLinearPass');
Q3 = quadrupole('QD2', 0.24, -1.1616, 'QuadLinearPass');
Q4 = quadrupole('QF2', 0.35, 2.7903, 'QuadLinearPass');
...
SF = sextupole('SF', 0.06, 35.7309, 'StrMPoleSymplectic4Pass'); 
SD = sextupole('SD', 0.06, -40.3794, 'StrMPoleSymplectic4Pass');
%
R1SF1 = SF;
R1SF2 = SF;
R2SF1 = SF;
...
% R4 In-Arc Super-conducting Wiggler IASWC 
IASWC = [IASWCPEA IASWCPEB IASWCPEC IASWCPEC IASWCPEB IASWCPEA ... %1
         IASWCN2A IASWCN2B IASWCN2C IASWCN2C IASWCN2B IASWCN2A ... %2
         IASWCPA IASWCPB IASWCPC IASWCPC IASWCPB IASWCPA ... %3
         IASWCNA IASWCNB IASWCNC IASWCNC IASWCNB IASWCNA ... %4
         IASWCPA IASWCPB IASWCPC IASWCPC IASWCPB IASWCPA ... %5
         IASWCNA IASWCNB IASWCNC IASWCNC IASWCNB IASWCNA ... %6
         IASWCPA IASWCPB IASWCPC IASWCPC IASWCPB IASWCPA ... %7
         IASWCNA IASWCNB IASWCNC IASWCNC IASWCNB IASWCNA ... %8
         IASWCPA IASWCPB IASWCPC IASWCPC IASWCPB IASWCPA ... %9
         IASWCNA IASWCNB IASWCNC IASWCNC IASWCNB IASWCNA ... %10
         IASWCPA IASWCPB IASWCPC IASWCPC IASWCPB IASWCPA ... %11
         IASWCNA IASWCNB IASWCNC IASWCNC IASWCNB IASWCNA ... %12
         IASWCPA IASWCPB IASWCPC IASWCPC IASWCPB IASWCPA ... %13
         IASWCNA IASWCNB IASWCNC IASWCNC IASWCNB IASWCNA ... %14
         IASWCP2A IASWCP2B IASWCP2C IASWCP2C IASWCP2B IASWCP2A ... %15
         IASWCNEA IASWCNEB IASWCNEC IASWCNEC IASWCNEB IASWCNEA]; %16
...
S34 = [R3D27 R3BPM8 R3D28 R3VC3 R3HC3 R3D29 R3D30A RSRFCAV R3D30B R3D31 ... % 01/15/2009
...
R4 = [R4QD1 RXD4 R4HC1 RXD5 R4QF2 R4D6A R4SQ1 R4D6B R4QD3 RXD7 R4BPM2 RXD8 BEND RXD9 ... % 
      R4SD1 R4VCSD1 R4SD1 RXD10 R4BPM3 RXD11 R4QF4 ...%
      R4D12A R4BPM4 R4D12B R4SF1 R4HCSF1 R4SF1 R4D13A R4D13B ... % 02/10/2010
      IASWC R4D14A R4VC2 R4D14B ...% 02/10/2010
      BEND RXD16 R4BPM5 RXD17 R4SF2 R4HCSF2 R4SF2 RXD18 R4QF5 ... % 
