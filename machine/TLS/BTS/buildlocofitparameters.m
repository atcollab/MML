...
try
    if isfield(LocoMeasData, 'MachineConfig')
        ...
        if all(LocoMeasData.MachineConfig.SF1.Setpoint.Data == 0)
            fprintf('   Turning SF1 family off in the LOCO model.\n');
            ATIndex = family2atindex('SF1');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        ...
        if all(LocoMeasData.MachineConfig.SD2.Setpoint.Data == 0)
            fprintf('   Turning SD2 family off in the LOCO model.\n');
            ATIndex = family2atindex('SD2');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        ...
        if all(LocoMeasData.MachineConfig.SF3.Setpoint.Data == 0)
            fprintf('   Turning SF3 family off in the LOCO model.\n');
            ATIndex = family2atindex('SF3');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        ...
        if all(LocoMeasData.MachineConfig.SD4.Setpoint.Data == 0)
            fprintf('   Turning SD4 family off in the LOCO model.\n');
            ATIndex = family2atindex('SD4');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        ...
        if all(LocoMeasData.MachineConfig.SF5.Setpoint.Data == 0)
            fprintf('   Turning SF5 family off in the LOCO model.\n');
            ATIndex = family2atindex('SF5');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        ...
        if all(LocoMeasData.MachineConfig.SD6.Setpoint.Data == 0)
            fprintf('   Turning SD6 family off in the LOCO model.\n');
            ATIndex = family2atindex('SD6');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
...
switch QuadFit
    ...
    case 1
        ...
        Families = {'QD1L','QF2L','QD3L','QD1S','QF2S','QD3S','QD4S','QF5S'};
...
switch SkewFit
    ...
    case 2  
        ...
        AT_Index = [family2atindex('SQSF'); family2atindex('SQSD');family2atindex('SQSF1');family2atindex('SQSD2');family2atindex('SQSF3');family2atindex('SQSD4');family2atindex('SQSF5');family2atindex('SQSD6');];
        for loop = 1:size(AT_Index,1)
            N = N + 1;
            FitParameters.Params{N} = mkparamgroup(RINGData.Lattice, AT_Index(loop,:),'s');
            FitParameters.Values = [FitParameters.Values; 0];
