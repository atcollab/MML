function SLKlocoprepfam
% Modified by SLK Feb.11,2005 for basic 4-quads and all sextupole as
% individual quad values 
...
dQ  = findcells(THERING, 'FamName', 'dQ');
...
%dQValues = getcellstruct(THERING, 'K', dQ(1:length(dQ)));
...
if 1
    % By family + all QD's
%        FitParameters.Values = [ QAValues; QBValues; QCValues; QDValues];
%     FitParameters.Values = [BENDValues ; QAValues; QBValues; QCValues; QDValues];
...
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1); dQValues];
%    FitParameters.Values = [dQValues];
%     FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1);...
%                             SFValues; SDValues];
%     FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1);...
%                             SFValues];
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1); SFValues(2)];
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1); SDValues(2)];
    FitParameters.Values = [BENDValues(1); QAValues(1); QBValues(1); QCValues(1); QDValues];
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues; SQValues];
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1);...
%                            SFValues; SDValues];
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1); SFValues];
%    FitParameters.Values = [QAValues(1); QBValues(1); QCValues(1); QDValues(1); SDValues];
...
% Add the sextupoles as quad gradients
%     for i = 1:length(SFI)
%         pos = pos + 1;
...
%     for i = 1:length(dQ)
%         pos = pos + 1;
%         FitParameters.Params{pos} = mkparamgroup(RINGData.Lattice, [dQ(i)], 'K');
%     end    
