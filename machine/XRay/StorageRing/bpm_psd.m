...
if PlotFlag
    ...
    subplot(3,1,1);
    loglog(f(m+1:N/2),1e12*Pdd(m+1:N/2),LineColor); grid on;
    title(['BPM POWER SPECTRUM (',num2str(N), ' points)']);
    xlabel('Frequency [Hz]');
    ylabel('PSD [\mum^2/Hz]');
    legend off;
    %aa=axis;
    %axis([10 2000 aa(3) aa(4)]);
    axis([10 2000 1e-4 1]);
    
    % Position spectrum
    subplot(3,1,2);
    semilogx(f(m:N/2),1e12*Pdd_int(m:N/2), LineColor); grid on;
    ...
    subplot(3,1,3);
    plot(0:deltaX:deltaX*(N-1),1e6*detrend(a),LineColor); grid on;
