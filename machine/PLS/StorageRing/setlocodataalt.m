...
switch lower(CommandInput)
    ...
    case 'symmetrize'
        ...
        A10=k2amp('Q1','Setpoint',-0.64178,[]);
        A20=k2amp('Q2','Setpoint',1.51773,[]);
        A30=k2amp('Q3','Setpoint',-1.69279,[]);
        A40=k2amp('Q4','Setpoint',-1.45440,[]);
        A50=k2amp('Q5','Setpoint',1.81956,[]);
        A60=k2amp('Q6','Setpoint',-0.79475,[]);

        for i=1:12
        Cu1=k2amp('Q1','Setpoint',Q1newfit(i),[]);
        Cur1(i)=Cu1(1);
        Cu2=k2amp('Q2','Setpoint',Q2newfit(i),[]);
        Cur2(i)=Cu2(1);
        Cu3=k2amp('Q3','Setpoint',Q3newfit(i),[]);
        Cur3(i)=Cu3(1);
        end

%        for i=1:12
%        Cu1=k2amp('Q1','Setpoint',Q1fit(i),[]);
%        Cur1(i)=Cu1(1);
%        Cu2=k2amp('Q2','Setpoint',Q2fit(i),[]);
%        Cur2(i)=Cu2(1);
%        Cu3=k2amp('Q3','Setpoint',Q3fit(i),[]);
%        Cur3(i)=Cu3(1);
%        end
        
        Cur4=k2amp('Q4','Setpoint',Q4fit(1),[]);
        Cur5=k2amp('Q5','Setpoint',Q5fit(1),[]);
        Cur6=k2amp('Q6','Setpoint',Q6fit(1),[]);
%        Cur1
%        Cur5
...
% Apply new quad k with N steps 
for j=1:N
fprintf('Do you want Quad. to be set?');
fprintf('\n');
fprintf('If you want, enter!');
fprintf('\n');
pause;
fprintf('%13.7f step is applied',j);
fprintf('\n');
    ...
    for i=1:12
        setsp('Q1',A1(i)-Nd1(i),[i 1]);
        setsp('Q2',A2(i)-Nd2(i),[i 1]);
        setsp('Q3',A3(i)-Nd3(i),[i 1]);
    end

    setsp('Q4',A4(12)-Nd4(12));
    setsp('Q5',A5(12)-Nd5(12));
    setsp('Q6',A6(12)-Nd6(12));
end

%        Family = 'Q1';
%        Dev = family2dev(Family);
%        setsp(Family, Q1newset1, Dev(1:2:end,:));
      
        
%        Family = 'Q2';
%        Dev = family2dev(Family);
%        setsp(Family, Q2newset1, Dev(1:2:end,:));

%        Family = 'Q3';
%        Dev = family2dev(Family);
%        setsp(Family, Q3newset1, Dev(1:2:end,:));
