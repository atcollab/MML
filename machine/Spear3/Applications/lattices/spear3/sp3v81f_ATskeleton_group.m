function sp3v81f_skeleton_grouped
...
%
%sp3v81f_skeleton_grouped
%to create this file 
%(1) add "Write .." comments to any AT deck
%(2) change strength fields for magnets accordingly
%(3) update HW2ATdeck as needed
...
QF		=	quadrupole('QF' , 0.3533895, KQF, 'StrMPoleSymplectic4Pass');
QD		=	quadrupole('QD' , 0.1634591, KQD, 'StrMPoleSymplectic4Pass');
QFC	    =	quadrupole('QFC', 0.5123803, KQFC,'StrMPoleSymplectic4Pass');

%Standard Cell Sextupoles
KSF=32.0477093/2;  %Nominal
KSD=-38.80153/2;   %Nominal
%Write Grouped Standard Cell Sextupoles - DO NOT REMOVE THIS LINE: command used to fill skeleton deck
SF		=	sextupole('SF' , 0.21, KSF,'StrMPoleSymplectic4Pass');
SD		=	sextupole('SD' , 0.25, KSD,'StrMPoleSymplectic4Pass');
...
SD DC4B BPM DC4A BND DC3B DC3A QD DC2C COR DC2D QF DC1B BPM DI1];
CEL2	=	[AP HCEL1 K1CEL2];

%Cell 3: (K1 & K2 magnets, Note QFC not split)
K1CEL3 =	[K1 DI2 BPM DC1B QF DC2A COR DC2B QD DC3A BPM DC3B BND DC4A BPM DC4B SD...
DC5A COR DC5B SF DC6A BPM DC6B QFC];
K2CEL3 =	[DC6B DC6A SF DC5C COR DC5D...
SD DC4B BPM DC4A BND DC3B DC3A QD DC2C COR DC2D QF DC1B BPM DI3 K2 DI4];
CEL3	=	[AP K1CEL3 K2CEL3];

%Cell 4: (Septum & K3 magnets, Note QFC not split)
SEPCEL4 =	[DI5 INJ DI6 BPM DC1B QF DC2A COR DC2B QD DC3A BPM DC3B BND DC4A BPM DC4B SD...
DC5A COR DC5B SF DC6A BPM DC6B QFC];
K3CEL4 =	[DC6B DC6A SF DC5C COR DC5D...
SD DC4B BPM DC4A BND DC3B DC3A QD DC2C COR DC2D QF DC1B BPM DI2 K3];
CEL4	=	[AP SEPCEL4 K3CEL4];

%Cell 5: (K5 magnets, Note QFC not split)
K3CEL5 =	[DI1 BPM DC1B QF DC2A COR DC2B QD DC3A BPM DC3B BND DC4A BPM DC4B SD...
DC5A COR DC5B SF DC6A BPM DC6B QFC];
...
QDX 	=	quadrupole('QDX' ,0.3533895,KQDX , 'StrMPoleSymplectic4Pass');
QFX 	=	quadrupole('QFX' ,0.6105311,KQFX , 'StrMPoleSymplectic4Pass');
QDY 	=	quadrupole('QDY' ,0.3533895,KQDY , 'StrMPoleSymplectic4Pass');
QFY 	=	quadrupole('QFY' ,0.5123803,KQFY , 'StrMPoleSymplectic4Pass');
QDZ 	=	quadrupole('QDZ' ,0.3533895,KQDZ , 'StrMPoleSymplectic4Pass');
QFZ 	=	quadrupole('QFZ' ,0.3533895,KQFZ , 'StrMPoleSymplectic4Pass');

%Matching Cell Sextupoles
KSFI= 7.5;  %Nominal
KSDI=-8.5;  %Nominal
%Write Grouped Match Cell Sextupoles - DO NOT REMOVE THIS LINE: command used to fill skeleton deck
SFI	=	sextupole('SFI'  , 0.21,KSFI,'StrMPoleSymplectic4Pass');
SDI	=	sextupole('SDI'  , 0.21,KSDI,'StrMPoleSymplectic4Pass');
...
% Begin Lattice
NORTH	=	[ACEL CEL2 CEL3 CEL4 CEL5 ACEL ACEL];
