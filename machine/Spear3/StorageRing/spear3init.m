...
AO.BPMx.Desired=AO.BPMx.Monitor;
...
AO.BPMy.Desired=AO.BPMy.Monitor;
...
name=bpm{ii,2};      AO.BPMx.Monitor.ChannelNames(ii,:)= name;
                     AO.BPMx.Desired.ChannelNames(ii,:)  = [deblank(name) 'Des']; 
...
name=bpm{ii,5};      AO.BPMy.Monitor.ChannelNames(ii,:)= name;
                     AO.BPMy.Desired.ChannelNames(ii,:)  = [deblank(name) 'Des']; 
...
val =bpm{ii,8};      AO.BPMx.ElementList(ii,:)         = val;   
                     ...
                     AO.BPMx.Desired.HW2PhysicsParams(ii,:) = 1e-3;
                     AO.BPMx.Desired.Physics2HWParams(ii,:) = 1000;
                     AO.BPMy.Desired.HW2PhysicsParams(ii,:) = 1e-3;
                     AO.BPMy.Desired.Physics2HWParams(ii,:) = 1000;
end
...
if 0 % change from 'if 1' to 'if 0' to invoke orbit, instead of u,v calls
    ...
    AO.BPMx.Desired.DataType = 'Scalar';
    AO.BPMy.Desired.DataType = 'Scalar';

    AO.BPMx.Desired = rmfield(AO.BPMx.Desired, 'DataTypeIndex');
    AO.BPMy.Desired = rmfield(AO.BPMy.Desired, 'DataTypeIndex');
    ...
    AO.BPMx.Desired.Mode            = 'Special';
    %AO.BPMx.Desired.SpecialFunctionGet = 'getxspear';  %will not return 'Desired'
    AO.BPMy.Desired.Mode            = 'Special';
    %AO.BPMy.Desired.SpecialFunctionGet = 'getyspear';
end
...
%***  LTBPMx/LTBPMy  ***
%=======================
families={'LTBBPMx' 'LTBBPMy'};
nltbpm=5;

for ii=1:2
AO.(families{ii}).FamilyName               = (families{ii});
AO.(families{ii}).FamilyType               = 'LTBPM';
AO.(families{ii}).MemberOf                 = {'PlotFamily'; 'LTBPM'; 'Diagnostics'};
AO.(families{ii}).Monitor.Mode             = Mode;
AO.(families{ii}).Monitor.DataType         = 'Scalar';
%AO.(families{ii}).Monitor.DataTypeIndex    = [1:nltbpm];
AO.(families{ii}).Monitor.Units            = 'Hardware';
AO.(families{ii}).Monitor.HWUnits          = 'mm';
AO.(families{ii}).Monitor.PhysicsUnits     = 'meter';
AO.(families{ii}).Desired                  = AO.(families{ii}).Monitor;
end

% x-name         x-chname   xstat y-name       y-chname     ystat DevList   Elem
ltbpm={
 'LTBBPM01'    'LTB-BPM01:X'  1  'LTBBPM1'    'LTB-BPM01:Y'  1  [1 ,1]    1; ...
 'LTBBPM02'    'LTB-BPM02:X'  1  'LTBBPM2'    'LTB-BPM02:Y'  1  [1 ,2]    2; ...
 'LTBBPM03'    'LTB-BPM03:X'  1  'LTBBPM3'    'LTB-BPM03:Y'  1  [1 ,3]    3; ...
 'LTBBPM04'    'LTB-BPM04:X'  1  'LTBBPM4'    'LTB-BPM04:Y'  1  [1 ,4]    4; ...
 'LTBBPM05'    'LTB-BPM05:X'  1  'LTBBPM5'    'LTB-BPM05:Y'  1  [1 ,5]    5; ...
};

%Load fields from data block
for ii=1:size(ltbpm,1)
name=ltbpm{ii,1};      AO.LTBBPMx.CommonNames(ii,:)         = name;
name=ltbpm{ii,2};      AO.LTBBPMx.Monitor.ChannelNames(ii,:)= name;
                       AO.LTBBPMx.Desired.ChannelNames(ii,:)= [deblank(name) 'Des']; 
val =ltbpm{ii,3};      AO.LTBBPMx.Status(ii,:)              = val;  
name=ltbpm{ii,4};      AO.LTBBPMy.CommonNames(ii,:)         = name;
name=ltbpm{ii,5};      AO.LTBBPMy.Monitor.ChannelNames(ii,:)= name;
                       AO.LTBBPMy.Desired.ChannelNames(ii,:)= [deblank(name) 'Des']; 
val =ltbpm{ii,6};      AO.LTBBPMy.Status(ii,:)              = val;  
val =ltbpm{ii,7};      AO.LTBBPMx.DeviceList(ii,:)          = val;   
                       AO.LTBBPMy.DeviceList(ii,:)          = val;
val =ltbpm{ii,8};      AO.LTBBPMx.ElementList(ii,:)         = val;   
                       AO.LTBBPMy.ElementList(ii,:)         = val;
                       AO.LTBBPMx.Monitor.HW2PhysicsParams(ii,:) = 1e-3;
                       AO.LTBBPMx.Monitor.Physics2HWParams(ii,:) = 1000;
                       AO.LTBBPMy.Monitor.HW2PhysicsParams(ii,:) = 1e-3;
                       AO.LTBBPMy.Monitor.Physics2HWParams(ii,:) = 1000;
                       AO.LTBBPMx.Desired.HW2PhysicsParams(ii,:) = 1e-3;
                       AO.LTBBPMx.Desired.Physics2HWParams(ii,:) = 1000;
                       AO.LTBBPMy.Desired.HW2PhysicsParams(ii,:) = 1e-3;
                       AO.LTBBPMy.Desired.Physics2HWParams(ii,:) = 1000;
end

AO.LTBBPMx.Status = AO.LTBBPMx.Status(:);
AO.LTBBPMy.Status = AO.LTBBPMy.Status(:);

%=======================
%***  BTSBPMx/BTSBPMy  ***
%=======================
families={'BTSBPMx' 'BTSBPMy'};
nbtsbpm=9;

for ii=1:2
AO.(families{ii}).FamilyName               = (families{ii});
AO.(families{ii}).FamilyType               = 'BTSBPM';
AO.(families{ii}).MemberOf                 = {'PlotFamily'; 'BTSBPM'; 'Diagnostics'};
AO.(families{ii}).Monitor.Mode             = Mode;
AO.(families{ii}).Monitor.DataType         = 'Scalar';
%AO.(families{ii}).Monitor.DataTypeIndex    = [1:nbtsbpm];
AO.(families{ii}).Monitor.Units            = 'Hardware';
AO.(families{ii}).Monitor.HWUnits          = 'mm';
AO.(families{ii}).Monitor.PhysicsUnits     = 'meter';
AO.(families{ii}).Desired                  =AO.(families{ii}).Monitor;
...
% x-name         x-chname   xstat y-name        y-chname     ystat DevList   Elem
btsbpm={
 'BTSBPM01'    'BTS-BPM01:X'  1  'BTSBPM01'    'BTS-BPM01:Y'  1  [1 ,1]    1; ...
 'BTSBPM02'    'BTS-BPM01:X'  1  'BTSBPM02'    'BTS-BPM02:Y'  1  [1 ,2]    2; ...
 'BTSBPM03'    'BTS-BPM01:X'  1  'BTSBPM03'    'BTS-BPM03:Y'  1  [1 ,3]    3; ...
 'BTSBPM04'    'BTS-BPM01:X'  1  'BTSBPM04'    'BTS-BPM04:Y'  1  [1 ,4]    4; ...
 'BTSBPM05'    'BTS-BPM01:X'  1  'BTSBPM05'    'BTS-BPM05:Y'  1  [1 ,5]    5; ...
 'BTSBPM06'    'BTS-BPM01:X'  1  'BTSBPM06'    'BTS-BPM06:Y'  1  [1 ,6]    6; ...
 'BTSBPM07'    'BTS-BPM01:X'  1  'BTSBPM07'    'BTS-BPM07:Y'  1  [1 ,7]    7; ...
 'BTSBPM08'    'BTS-BPM01:X'  1  'BTSBPM08'    'BTS-BPM08:Y'  1  [1 ,8]    8; ...
 'BTSBPM09'    'BTS-BPM01:X'  1  'BTSBPM09'    'BTS-BPM09:Y'  1  [1 ,9]    9; ...
};

%Load fields from data block
for ii=1:size(btsbpm,1)
name=btsbpm{ii,1};      AO.BTSBPMx.CommonNames(ii,:)         = name;
name=btsbpm{ii,2};      AO.BTSBPMx.Monitor.ChannelNames(ii,:)= name;
                        AO.BTSBPMx.Desired.ChannelNames(ii,:)= [deblank(name) 'Des']; 
val =btsbpm{ii,3};      AO.BTSBPMx.Status(ii,:)              = val;  
name=btsbpm{ii,4};      AO.BTSBPMy.CommonNames(ii,:)         = name;
name=btsbpm{ii,5};      AO.BTSBPMy.Monitor.ChannelNames(ii,:)= name;
                        AO.BTSBPMy.Desired.ChannelNames(ii,:)= [deblank(name) 'Des']; 
val =btsbpm{ii,6};      AO.BTSBPMy.Status(ii,:)              = val;  
val =btsbpm{ii,7};      AO.BTSBPMx.DeviceList(ii,:)          = val;   
                        AO.BTSBPMy.DeviceList(ii,:)          = val;
val =btsbpm{ii,8};      AO.BTSBPMx.ElementList(ii,:)         = val;   
                        AO.BTSBPMy.ElementList(ii,:)         = val;
                        AO.BTSBPMx.Monitor.HW2PhysicsParams(ii,:) = 1e-3;
                        AO.BTSBPMx.Monitor.Physics2HWParams(ii,:) = 1000;
                        AO.BTSBPMy.Monitor.HW2PhysicsParams(ii,:) = 1e-3;
                        AO.BTSBPMy.Monitor.Physics2HWParams(ii,:) = 1000;
                        AO.BTSBPMx.Desired.HW2PhysicsParams(ii,:) = 1e-3;
                        AO.BTSBPMx.Desired.Physics2HWParams(ii,:) = 1000;
                        AO.BTSBPMy.Desired.HW2PhysicsParams(ii,:) = 1e-3;
                        AO.BTSBPMy.Desired.Physics2HWParams(ii,:) = 1000;
end

AO.BTSBPMx.Status = AO.BTSBPMx.Status(:);
AO.BTSBPMy.Status = AO.BTSBPMy.Status(:);
...
cor={
 ...
 '9SCX1   ' '09S-COR1H:Curr1 '  '09S-COR1H:CurrSetpt '  1  '9SCX1   ' '                ' '                    '   0   [9 ,5]   37 [-30.0 +30.0]  0.101  1.5e-4    1.5e-4    0.5e-4   ;... 
 '9SCX2   ' '09S-COR2H:Curr1 '  '09S-COR2H:CurrSetpt '  1  '9SCX2   ' '                ' '                    '   0   [9 ,6]   38 [-30.0 +30.0]  0.101  1.5e-4    1.5e-4    0.5e-4   ;... 
 '9SCY1   ' '                '  '                    '  0  '9SCY1   ' '09S-COR1V:Curr1 ' '09S-COR1V:CurrSetpt '   1   [9 ,7]   39 [-30.0 +30.0]  0.101  1.5e-4    1.5e-4    0.5e-4   ;... 
 '9SCY2   ' '                '  '                    '  0  '9SCY2   ' '09S-COR2V:Curr1 ' '09S-COR2V:CurrSetpt '   1   [9 ,8]   40 [-30.0 +30.0]  0.101  1.5e-4    1.5e-4    0.5e-4   ;... 
 '9SCX3   ' '09S-COR3H:Curr1 '  '09S-COR3H:CurrSetpt '  1  '9SCX3   ' '                ' '                    '   0   [9 ,9]   41 [-30.0 +30.0]  0.101  1.5e-4    1.5e-4    0.5e-4   ;... 
 '9SCX4   ' '09S-COR4H:Curr1 '  '09S-COR4H:CurrSetpt '  1  '9SCX4   ' '                ' '                    '   0   [9 ,10]  42 [-30.0 +30.0]  0.101  1.5e-4    1.5e-4    0.5e-4   ;... 
...
'ID11_TH '  '15S-ID11TH:Curr1  '    '15S-ID11TH:CurrSetpt  '  '15S-ID11TH:CurrReference '      1  [15,1]  5
'ID11_TV '  '15S-ID11TV:Curr1  '    '15S-ID11TV:CurrSetpt  '  '15S-ID11TV:CurrReference '      1  [15,2]  6
'ID04_TH '  '16S-ID4TH:Curr1   '    '16S-ID4TH:CurrSetpt   '  '16S-ID4TH:CurrReference  '      1  [16,2]  7
'ID04_TV '  '16S-ID4TV:Curr1   '    '16S-ID4TV:CurrSetpt   '  '16S-ID4TV:CurrReference  '      1  [16,2]  8
};
...
AO.CD.Desired=AO.CD.Monitor;

%common           monitor              setpoint        stat devlist elem scalefactor range     tol   respkick
cd={ 
 '9CD1     '    '09S-CD1:CurrSetptDes' '09S-CD1:Curr'     '09S-CD1:CurrSetpt'  1   [9  ,1]  1     1.0       [0 200]  0.050   0.25      ; ...
 '9CD2     '    '09S-CD2:CurrSetptDes' '09S-CD2:Curr'     '09S-CD2:CurrSetpt'  1   [9  ,2]  2     1.0       [0 200]  0.050   0.25      ; ...
 '9CD3     '    '09S-CD3:CurrSetptDes' '09S-CD3:Curr'     '09S-CD3:CurrSetpt'  1   [9  ,3]  3     1.0       [0 200]  0.050   0.25      ; ...
 '9CD4     '    '09S-CD4:CurrSetptDes' '09S-CD4:Curr'     '09S-CD4:CurrSetpt'  1   [9  ,4]  4     1.0       [0 200]  0.050   0.25      ; ...
  }; 

MagnetCoreName={'CD1' 'CD2' 'CD3' 'CD4'};
for ii=1:size(cd,1)
name=cd{ii,1};     AO.CD.CommonNames(ii,:)          = name; 
name=cd{ii,2};     AO.CD.Desired.ChannelNames(ii,:) = name;
name=cd{ii,3};     AO.CD.Monitor.ChannelNames(ii,:) = name; 
name=cd{ii,4};     AO.CD.Setpoint.ChannelNames(ii,:)= name;     
val =cd{ii,5};     AO.CD.Status(ii,1)               = val;
val =cd{ii,6};     AO.CD.DeviceList(ii,:)           = val;
val =cd{ii,7};     AO.CD.ElementList(ii,1)          = val;
val =cd{ii,8};
...
AO.CD.Desired.HW2PhysicsParams{1}(ii,:)                 = AO.CD.Monitor.HW2PhysicsParams{1}(ii,:);
AO.CD.Desired.HW2PhysicsParams{2}(ii,:)                 = val;
...
AO.CD.Desired.Physics2HWParams{1}(ii,:)                 = AO.CD.Monitor.HW2PhysicsParams{1}(ii,:);
AO.CD.Desired.Physics2HWParams{2}(ii,:)                 = val;

val =cd{ii,9};     AO.CD.Setpoint.Range(ii,:)       = val;
val =cd{ii,10};    AO.CD.Setpoint.Tolerance(ii,1)   = val;
val =cd{ii,11};    AO.CD.Setpoint.DeltaRespMat(ii,1)        = val;
end
...
% %amplitude of signal out of SRS
...
%   };
%kicker amplitude in kV
%common        monitor                  setpoint                stat  devlist elem range    tol
kickeramp={ 
 'K1     '    '02S-K1:VoltReadback  '     '02S-K1:VoltSetpt  '  1   [2  ,1]  1  [0 9]  0.20 ; ...
 'K2     '    '03S-K2:VoltReadback  '     '03S-K2:VoltSetpt  '  1   [3  ,1]  2  [0 9]  0.20 ; ...
 'K3     '    '04S-K3:VoltReadback  '     '04S-K3:VoltSetpt  '  1   [4  ,1]  3  [0 9]  0.20 ; ...
  };
...
AO.TUNE.Monitor.SpecialFunctionGet     = 'gettune_spear';
...
'CH13'   'OrbInt:Ch13BpldState'  1       [1 13]      13       13 'OLD4  '      [13  6]   [14  1]  [ 13 4]    2.812   4.7   1.10    1.16  0.36     1.0    1.0; ...
...
'CH16'   'OrbInt:Ch16BpldState'  1       [1 16]      16       16 'BL4   '      [16  6]   [17  1]  [ 16 4]    2.812   4.7   1.10    1.16  0.36     1.0    1.0; ...
'CH17'   'OrbInt:Ch17BpldState'  1       [1 17]      17       17 'BL13  '      [17  6]   [18  1]  [ 17 4]    4.552   4.7   0.707   1.155 0.222    1.0    1.0; ...
