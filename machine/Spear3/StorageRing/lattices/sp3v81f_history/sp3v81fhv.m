...
HCOR =  corrector('HCOR',0.15/2,[0 0],'CorrectorPass');
VCOR =  corrector('VCOR',0.15/2,[0 0],'CorrectorPass');
...
HCEL1 =	[DC1A BPM DC1B QF DC2A HCOR VCOR DC2B QD DC3A BPM DC3B BEND DC4A BPM DC4B SD...
DC5A HCOR VCOR DC5B SF DC6A BPM DC6B QFC];
HCEL2 =	[DC6B DC6A SF DC5C HCOR VCOR DC5D...
SD DC4B BPM DC4A BEND DC3B DC3A QD DC2C HCOR VCOR DC2D QF DC1B BPM DC1A];
ACEL	=	[AP HCEL1 HCEL2];

%Cell 2: (K1 magnet, Note QFC not split)
K1CEL2 =	[DC6B DC6A SF DC5C HCOR VCOR DC5D...
SD DC4B BPM DC4A BEND DC3B DC3A QD DC2C HCOR VCOR DC2D QF DC1B BPM DI1];
CEL2	=	[AP HCEL1 K1CEL2];

%Cell 3: (K1 & K2 magnets, Note QFC not split)
K1CEL3 =	[K1 DI2 BPM DC1B QF DC2A HCOR VCOR DC2B QD DC3A BPM DC3B BEND DC4A BPM DC4B SD...
DC5A HCOR VCOR DC5B SF DC6A BPM DC6B QFC];
K2CEL3 =	[DC6B DC6A SF DC5C HCOR VCOR DC5D...
SD DC4B BPM DC4A BEND DC3B DC3A QD DC2C HCOR VCOR DC2D QF DC1B BPM DI3 K2 DI4];
CEL3	=	[AP K1CEL3 K2CEL3];

%Cell 4: (Septum & K3 magnets, Note QFC not split)
SEPCEL4 =	[DI5 INJ DI6 BPM DC1B QF DC2A HCOR VCOR DC2B QD DC3A BPM DC3B BEND DC4A BPM DC4B SD...
DC5A HCOR VCOR DC5B SF DC6A BPM DC6B QFC];
K3CEL4 =	[DC6B DC6A SF DC5C HCOR VCOR DC5D...
SD DC4B BPM DC4A BEND DC3B DC3A QD DC2C HCOR VCOR DC2D QF DC1B BPM DI2 K3];
CEL4	=	[AP SEPCEL4 K3CEL4];

%Cell 5: (K5 magnets, Note QFC not split)
K3CEL5 =	[DI1 BPM DC1B QF DC2A HCOR VCOR DC2B QD DC3A BPM DC3B BEND DC4A BPM DC4B SD...
DC5A HCOR VCOR DC5B SF DC6A BPM DC6B QFC];
...
MCA=[DA1A BPM DA1B QDX DA2A HCOR VCOR DA2B ...
QFX DA3A BPM DA3B QDY DM4 BDM DA5A BPM DA5B SDM DA6A HCOR VCOR DA6B SFM DA7A BPM DA7B QFY ...
DM7 SFM DA6C HCOR VCOR DA6D SDM DA5C BPM DA5D BDM...
DA8A BPM DA8B QDZ DA9A HCOR VCOR DA9B QFZ DA10A BPM DA10B];

%Matching Cell B (North East, South West)
MCB=[DB1A BPM DB1B QDX DB2A HCOR VCOR DB2B ...
QFX DB3A BPM DB3B QDY DM4 BDM DB5A BPM DB5B SDM DB6A HCOR VCOR DB6B SFM DM7 QFY DB7A BPM...
DB7B SFM DB6C HCOR VCOR DB6D SDM DB5C BPM DB5D BDM...
DB8A BPM DB8B QDZ DB9A HCOR VCOR DB9B QFZ DB10A BPM DB10B];
