...
if nargin < 1
    ...
    NStepsY = [0 .25 .4 .5 .6 .7 .8 .9 1];
    FileName = [];
    
    Sector = menu('Choose a sector?','Injection', 'BL 5','BL 7','BL 9','14 Straight','Exit');
...
% % Initialize lifetime measurement
% [LifeTimeFast, LifeTimeMed, LifeTimeSlow, LifeTimePeriodFast, LifeTimePeriodMed, LifeTimePeriodSlow] = getlifetime;
% if strcmpi(LifeTimeFlag,'Fast')
%     getlifetime(LifeTimePeriod);
% elseif strcmpi(LifeTimeFlag,'Med')
%     getlifetime(LifeTimePeriodFast, LifeTimePeriod);
% elseif strcmpi(LifeTimeFlag,'Med')
%     getlifetime(LifeTimePeriodFast, LifeTimePeriodMed, LifeTimePeriod);
% end
...
if any(NStepsX ~= 0)
    ...
    if exist([HorizontalBumpFile,'.mat'], 'file')
        ...
        Xgain = getfamilydata('BPMx', 'Gain', BPMList);
        ...
        [HOCS, RF, HOCS0] = setorbitbump('BPMx', BPMList, DeltaXBump/6, 'HCM', [-5 -4 -3 -2 -1 1 2 3 4 5], 1, SVDIndex, BPMWeight, 'Inc', 'NoDisplay');
        ...
        [HOCS, RF, HOCS0] = setorbitbump('BPMx', BPMList, DeltaXBump, 'HCM', [-5 4 -3 -2 -1 1 2 3 4 5], 1, SVDIndex, BPMWeight, 'Inc', 'NoDisplay');
...
if any(NStepsY ~= 0)
    ...
    if exist([VerticalBumpFile,'.mat'], 'file')
        ...
        % BPM gain (work in real coordinates)
        Ygain = getfamilydata('BPMy', 'Gain', BPMList);
        ...
        [VOCS, RF, VOCS0] = setorbitbump('BPMy', BPMList, DeltaYBump/6, 'VCM', [-4 -3 -2 -1 1 2 3 4], 1, SVDIndex, BPMWeight, 'Inc', 'NoDisplay');
        ...
        [VOCS, RF, VOCS0] = setorbitbump('BPMy', BPMList, DeltaYBump, 'VCM', [-4 -3 -2 -1 1 2 3 4], 5, SVDIndex, BPMWeight, 'Inc', 'NoDisplay');
...
for x = DeltaX
    ...
    for y = DeltaY
        ...
        pause(1);
        LifeTime(i,j) = measlifetime;  % Base on current drop
        %pause(2.2*LifeTimePeriod - 2*Tbpm);
        %pause(1.1*LifeTimePeriod);
        %[LifeTimeFast(i,j), LifeTimeMed(i,j), LifeTime(i,j)] = getlifetime;
        %LifeTime(i,j) = measlifetime(0:.5:LifeTimePeriod);
        %LifeTime(i,j) = measlifetimebuffer;
...
% Reset DCCT parameters
% getlifetime(LifeTimePeriodFast, LifeTimePeriodMed, LifeTimePeriodSlow);
...
% plot(squeeze(BPMy(1,:,BPMyIndex(1))), LifeTime(1,:),'b');
% hold on
% plot(squeeze(BPMy(1,:,BPMyIndex(1))), LifeTimeMed(1,:),'r')
% hold off
