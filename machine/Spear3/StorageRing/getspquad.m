function [AM, tout, DataTime, ErrorFlag] = getspquad(varargin);
%GETSPQUAD 
...
if iscell(Family)
    for i = 1:length(Family)
        if length(varargin) < 2
            [QuadSetpoint{i}, ErrorFlag{i}] = getspquad(Family{i}, UnitsFlag{:});
        else
            if iscell(Field) & iscell(DeviceList)
                [QuadSetpoint{i}, ErrorFlag{i}] = getspquad(Family{i}, Field{i}, DeviceList{i}, UnitsFlag{:});
            elseif ~iscell(Field) & iscell(DeviceList)
                [QuadSetpoint{i}, ErrorFlag{i}] = getspquad(Family{i}, Field, DeviceList{i}, UnitsFlag{:});
            elseif ~iscell(Field) & ~iscell(DeviceList)
                [QuadSetpoint{i}, ErrorFlag{i}] = getspquad(Family{i}, Field, DeviceList, UnitsFlag{:});
            else
                error('Family, Field, DeviceList input problem with cells.');
            end
        end
    end
    return
...
% Regular Family, Field, DeviceList inputs at this point
if isnumeric(Field)
    DeviceList = Field;
    Field = '';
end
...
elseif strcmpi(Machine,'SPEAR3') | strcmpi(Machine,'SPEAR')
    ...
    for i = 1:size(DeviceList,1)
        ...
        if isempty(TblRowMatch)
            ...
            elseif any(DeviceList(i,:) ~= Device)
                ...
                [tmp, t, DataTime, ErrorFlag] = getpv('118-QMS1:CurrSetpt');
                AM(i,1) = tmp(1);    
            end
        end
    end
    ...
    [AM, t, DataTime, ErrorFlag] = getam(Family, QuadDev);
...
if strcmpi(UnitsFlag,'Physics')
    ...
    AM = hw2physics(Family, 'Setpoint', AM, DeviceList);
end
