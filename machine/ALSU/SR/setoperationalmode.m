...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin < 1 || isempty(ModeNumber)
   %ModeNumber = [];
    ModeNumber = 1;
end
% if ModeNumber == 1
%     ModeCell = {'ALS-U Operations', 'ALS-U Model'};
%     [ModeNumber, OKFlag] = listdlg('Name','ALS-U','PromptString','Select the Operational Mode:', 'SelectionMode','single', 'ListString', ModeCell, 'ListSize', [450 200]);
%     if OKFlag ~= 1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
AD = getad;
AD.Machine = 'ALSU';              % Will already be defined if setpathmml was used
...
AD.Energy = 2.0;                  % make sure this is the same as bend2gev at the production lattice!
AD.InjectionEnergy = 2.0;
...
if ModeNumber == 1
    % User mode
    AD.OperationalMode = 'ALS-U 2.0 GeV';
    ModeName = 'User';
    OpsFileExtension = '_ALSU';

    % AT lattice
    AD.ATModel = 'alsulattice';
    %alsulattice;
    ...
    % This is a bit of a cluge to know if the user is on the ALS-U filer
    % If so, then the MML user probably wants to be online
    MMLROOT = getmmlroot;
    if isempty(findstr(lower(MMLROOT),'SomeDirectoryAtALSU'))
        switch2sim;
    else
        switch2online;
    end
...
elseif ModeNumber == 101
    % Model mode
    AD.OperationalMode = 'ALS-U 2.0 GeV - Model';
    ModeName = 'Model';
