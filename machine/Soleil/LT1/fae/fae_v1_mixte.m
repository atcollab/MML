function varargout = fae_v1(varargin)
% FAE_V1 M-file for fae_v1.fig
%      FAE_V1, by itself, creates a new FAE_V1 or raises the existing
%      singleton*.
%
%      H = FAE_V1 returns the handle to a new FAE_V1 or the handle to
%      the existing singleton*.
%
%      FAE_V1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FAE_V1.M with the given input arguments.
%
%      FAE_V1('Property','Value',...) creates a new FAE_V1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before fae_v1_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to fae_v1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help fae_v1

% Last Modified by GUIDE v2.5 11-Mar-2005 12:32:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @fae_v1_OpeningFcn, ...
                   'gui_OutputFcn',  @fae_v1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before fae_v1 is made visible.
function fae_v1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to fae_v1 (see VARARGIN)

% Choose default command line output for fae_v1
...
% % so window can get raised using fae_v1.
% if strcmp(get(hObject,'Visible'),'off')
%     plot(rand(5));
% end

% UIWAIT makes fae_v1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = fae_v1_OutputFcn(hObject, eventdata, handles)
...
% % lecture du premier mesureur de charge
% vecty1 = tango_read_attribute(dev,'Sensor1Data');
% ydatamc1 = vecty1.value;
% % lecture du deuxieme  mesureur de charge
% vecty2 = tango_read_attribute(dev,'Sensor2Data');
% ydatamc2 = vecty2.value;
% 
...
% horodatage du scan :lorsque sera install� le server scan :
dev = device_name.scan;
...
% on �crase les vecteurs donn�es re - calcul�es  (liss�es, �nergie)
nul = [];
...
if isequal(log1,1)
    ...
    % calcul de l'�nergie du faisceau / dispersion � la fente d'analyse
    LT1init;
    global THERING;
    AO = getao;
...
Ec = 100    
Idip = 170
    % en attendant la reconnexion des �quipements, simulation !
    xdata = [-25:0.25:25];
    xmin = min(xdata);
    xmax = max(xdata);
    sizex = size(xdata,2);
    ...
    % graphe avec labels et valeurs ligne de transfert d�duits des courants
    % sur la machine
    %______________________________________________________________________
    axes(handles.axes1);
    cla
  
    ResultsStr = {['I dip�le [A] = ',num2str(Idip)],...
    ...
    plot(xdata,ydatab,'r')    
    xlim([xmin xmax]);
    xlabel('d�placement du mors en mm');
    ylabel('charge normalis�e');
    title('int�grale du spectre','FontAngle','italic','FontSize',12,'FontWeight','bold');
    %axis tight
    
    legend('donn�es brutes',2);
    legend('boxoff');
    drawnow;
    ...
    axes(handles.axes2);
    cla
    % on d�rive les donn�es bruit�es (Dbruit) 
...
%     position = structure.value(1);
    gap = 12 *0.001;
    valgap=sprintf('%2.1f',gap*1000);
    resolution = gap*100/dispersion;
    valresolution = sprintf('%2.1f',abs(gap*100/dispersion));
    position = -5.3 * 0.001;
    valposition=sprintf('%2.1f',position*1000);
    ...
    % trouver le courant donnant la charge max
...
% % %____________________________________
    ...
    % � l'axe th�orique (valeur de l'attribut position)
%             B0 = getappdata(handles.figure1,'B0');
%             B1 = getappdata(handles.figure1,'B1');
    [C, Leff, MagnetType, A] = magnetcoefficients('BEND');
    B0 = A(9);
    ...
    xdata2 = xdata2 * (1 + position/dispersion);
    xmin = min(xdata2);
    xmax = max(xdata2);
    sizex2 = size(xdata2,2);
    ...
    xlabel('Energie en MeV');
    ylabel('charge normalis�e');
    title('spectre corrig�','FontAngle','italic','FontSize',12,'FontWeight','bold');
    
    
    %____________________________________________________________________
    % calcul de largeur du spectre non liss�
    %Itot = max(yfiltre1)
    ...
    LargeurAf = Nbpxf * pasAf;
     ...
     ResultsStr = {ResultsStr{:},...
         [],['Largeur corrig�e du spectre non liss� : (Epeack =' ,sprintf('%2.3f',xdata2(I)),' MeV)'],...
        ['68 % des e- dans ',val2,' % de Epeack']}';
     
%      ResultsStr = {ResultsStr{:}, ['Largeur non corr � 68 % =  ',val1,' %'],...
%         ['Largeur corr � 68 % =  ',val2,' %'] }'
    set(handles.listbox1,'String',ResultsStr);
%     
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
setappdata(handles.figure1,'ydata',ydatab);
...
legend('donn�es brutes','donn�es liss�es',2);
legend('boxoff');
...
if isequal(log2,1)
    ...
    Largeurmmf = Nbpxf * pasmmf;
     ...
     ResultsStr = get(handles.listbox1,'String');
     ResultsStr = {ResultsStr{:}, ...
         [],['Largeur corrig�e du spectre liss� : (Epeack =' ,sprintf('%2.3f',xdata2(I)),' MeV)'],...
         [' 68 % des e- dans ',val2,' % de Epeack'],...
         }';
    set(handles.listbox1,'String',ResultsStr);
%     
...
try
    ...
    if isequal(log1,1)
       ...
       save(FileName, 'xdata', 'ydata','xdata2','ydata2',...
            'yfiltre','Dfiltre','ResultsStr','log1','horodatage')

    else 
       save(FileName, 'xdata', 'ydata','xdata2','yfiltre',...
           'ResultsStr','log1','horodatage')     
    end
...
if isequal(log1,1)
    ...
    if isequal(Dfiltre,[]) & isequal(ydata2,[])
        errordlg('vous n''avez pas rentr� de donn�es !','Attention');
        return
...
E0 = 0.51099906;
angle = 15 * pi / 180;
%         Bro = 0.001 * (E0 / 0.29979) * sqrt((energie+E0)*(energie+E0)/(E0*E0) - 1)
Bro = getbrho((energie + E0)*1e-3);

Integrale = Bro * angle;
courant = (Integrale - B0)/ B1;
device_name = getappdata(handles.figure1,'device_name');
tango_write_attribute(device_name.dipole,'current',courant);
%tango_write_attribute('LT1/AEsim/D.1','current',courant);    
