function varargout = fae_v3(varargin)
% FAE_V3 M-file for fae_v3.fig
%      FAE_V3, by itself, creates a new FAE_V3 or raises the existing
%      singleton*.
%
%      H = FAE_V3 returns the handle to a new FAE_V3 or the handle to
%      the existing singleton*.
%
%      FAE_V3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FAE_V3.M with the given input arguments.
%
%      FAE_V3('Property','Value',...) creates a new FAE_V3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before fae_v3_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to fae_v3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help fae_v3

% Last Modified by GUIDE v2.5 20-Jul-2005 15:06:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @fae_v3_OpeningFcn, ...
                   'gui_OutputFcn',  @fae_v3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before fae_v3 is made visible.
function fae_v3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to fae_v3 (see VARARGIN)

% Choose default command line output for fae_v3
...
% % so window can get raised using fae_v3.
% if strcmp(get(hObject,'Visible'),'off')
%     plot(rand(5));
% end

% UIWAIT makes fae_v3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = fae_v3_OutputFcn(hObject, eventdata, handles)
...
try
    ...
    elseif  isequal(log2,1)
       save(FileName, 'xdata', 'ydata','xdata2','yfiltre','ydatamc1',...
           'ResultsStr','log1','log2','log3','horodatage','mc1_charge','mc1_calibre')   
