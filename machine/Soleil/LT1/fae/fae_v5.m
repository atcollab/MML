function varargout = fae_v5(varargin)
% FAE_V5 M-file for fae_v5.fig
%      FAE_V5, by itself, creates a new FAE_V5 or raises the existing
%      singleton*.
%
%      H = FAE_V5 returns the handle to a new FAE_V5 or the handle to
%      the existing singleton*.
%
%      FAE_V5('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FAE_V5.M with the given input arguments.
%
%      FAE_V5('Property','Value',...) creates a new FAE_V5 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before fae_v5_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to fae_v5_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help fae_v5
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @fae_v5_OpeningFcn, ...
                   'gui_OutputFcn',  @fae_v5_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before fae_v5 is made visible.
function fae_v5_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to fae_v5 (see VARARGIN)

% Choose default command line output for fae_v5
...
% % so window can get raised using fae_v5.
% if strcmp(get(hObject,'Visible'),'off')
%     plot(rand(5));
% end

% UIWAIT makes fae_v5 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = fae_v5_OutputFcn(hObject, eventdata, handles)
...
String2 = double(res.value + 1*60*60)/(60*60*24) + String1;
...
%%  calcul de largeur du spectre non lissé
    ...
    % option où on n'écrit que les résultats pour le spectre lissé
     set(handles.listbox1,'String',ResultsStr);
...
%%%%%%%%%%%%%%%%%%%%%
y2filtre=filter(b,a,ydata2);
% y2filtre1 repositionne y2filtre au milieu de la fen�tre windowsize
y2filtre1=y2filtre(ceil(windowSize/2):sizex);
...
if isequal(log3,1)
    ...
    plot(...
    xdata2(1:size(y2filtre1,2)),y2filtre1(1:size(y2filtre1,2)));

%%  calcul de largeur du spectre lissé
    pourcentage_1sigma = 0.3173;
    pourcentage_80pc = 0.2;
    pourcentage_90pc = 0.1;

    x = xdata2(1:size(y2filtre1,2));
    y = y2filtre1;
    [Y,I] = max(y);
    largeur_MeV_1sigma = largeur_pourcentage(x,y,pourcentage_1sigma)
    largeur_MeV_80pc = largeur_pourcentage(x,y,pourcentage_80pc)
    largeur_MeV_90pc = largeur_pourcentage(x,y,pourcentage_90pc)
    dpsurp_1sigma = 100 * largeur_MeV_1sigma / x(I);
    largeur_MeV_80pc = largeur_pourcentage(x,y,pourcentage_80pc)
    dpsurp_80pc = 100 * largeur_MeV_80pc / x(I);
    largeur_MeV_90pc = largeur_pourcentage(x,y,pourcentage_90pc)
    dpsurp_90pc = 100 * largeur_MeV_90pc / x(I);
    ...
    ResultsStr = get(handles.listbox1,'String');
    ResultsStr = {ResultsStr{:},...
    [],['Largeur du spectre lissé : (Epeack = ',sprintf('%2.3f',xdata2(I)),' MeV)'],...
    ['68 % des e- dans ',valdpsurp_1sigma,' % de Epeack'],...
    ['80 % des e- dans ',valdpsurp_80pc,' % de Epeack'],...
    ['90 % des e- dans ',valdpsurp_90pc,' % de Epeack']}';
    set(handles.listbox1,'String',ResultsStr);
...
setappdata(handles.figure1,'y2filtre',y2filtre1);
...
y2filtre = getappdata(handles.figure1,'y2filtre');
...
try
    ...
    elseif  isequal(log2,1)
       save(FileName, 'xdata', 'ydata','xdata2','ydata2','yfiltre','y2filtre','ydatamc1',...
...
try
    ...
    if isequal(exist('y2filtre'),1)
        setappdata(handles.figure1,'y2filtre',y2filtre);
    end
    set(handles.nom_fichier_edit7,'String',FileName);
    % _____________________________________________________________________
    axes(handles.axes1);
...
% %     else
        ...
        if isequal(exist('y2filtre'),1)
        plot(xdata2,ydata2,'r',...
             xdata2(1:size(y2filtre,2)),y2filtre(1:size(y2filtre,2)))
        else 
            plot(xdata2,ydata2,'r')
        end
 %%  end
