function varargout = ellipse_v15(varargin)  % EN COURS !!!!!!
% ellipse_v15 M-file for ellipse_v15.fig
%      ellipse_v15, by itself, creates a new ellipse_v15 or raises the existing
%      singleton*.
%
%      H = ellipse_v15 returns the handle to a new ellipse_v15 or the handle to
%      the existing singleton*.
%
%      ellipse_v15('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ellipse_v15.M with the given input arguments.
%
%      ellipse_v15('Property','Value',...) creates a new ellipse_v15 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ellipse_v15_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ellipse_v15_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ellipse_v15
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @ellipse_v15_OpeningFcn, ...
                   'gui_OutputFcn',  @ellipse_v15_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ellipse_v15 is made visible.
function ellipse_v15_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ellipse_v15 (see VARARGIN)

% Choose default command line output for ellipse_v15
...
if iscell(varargin) && ~isempty(varargin)
   ...
   %ù IQ = getam('QP');
    ...
    energie = str2double(get(handles.energie_edit38,'String'))
    setenergymodel(energie/1000);
    ao = getao;
    valQ = getam('QP');
    % � introduire quand OK
    setsp('QP',valQ,'model');
    ...
    % ici erreur : on prend la date de construction de l'ellipse et non pas
    % le moment de la prise des données... bon..
    set(handles.date_edit32,'String',datestr(clock,29));
    set(handles.energie_edit33,'String',Str2num(get(handles.energie_edit38,'String')));
    set(handles.emitH_edit34,'String',num2str(epsx*1e6));
    set(handles.emitV_edit35,'String',num2str(epsy*1e6));
...
% UIWAIT makes ellipse_v15 wait for user response (see UIRESUME)
% uiwait(handles.main);


% 
% --- Outputs from this function are returned to the command line.
function varargout = ellipse_v15_OutputFcn(hObject, eventdata, handles)
