function varargout = orbitx_AT(varargin)
% ORBITX_AT M-file for orbitx_AT.fig
%      ORBITX_AT, by itself, creates a new ORBITX_AT or raises the existing
%      singleton*.
%
%      H = ORBITX_AT returns the handle to a new ORBITX_AT or the handle to
%      the existing singleton*.
%
%      ORBITX_AT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ORBITX_AT.M with the given input arguments.
%
%      ORBITX_AT('Property','Value',...) creates a new ORBITX_AT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before orbitx_AT_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to orbitx_AT_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help orbitx_AT
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @orbitx_AT_OpeningFcn, ...
                   'gui_OutputFcn',  @orbitx_AT_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before orbitx_AT is made visible.
function orbitx_AT_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to orbitx_AT (see VARARGIN)

% Choose default command line output for orbitx_AT
...
% UIWAIT makes orbitx_AT wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = orbitx_AT_OutputFcn(hObject, eventdata, handles) 
...
axes(handles.axes_lattice)
   drawlattice_alex
   set(handles.axes_lattice,'YTick',[]);
   set(handles.axes_lattice,'XTick',[]);
...
K2=getam('HCOR');save('xcoortemp','K2')
K3=K2+K1';
...
K2=getam('HCOR','simulator'); save('xcoortemp','K2')
for i=1:ncorrx,
    if (K1(i)<1.5)
       %stepsp('HCOR',K1(i),[i 1]);
    end    
...
axes(handles.axes_orbit)
    ...
    plot(s_bx,Xc,'-or','MarkerFaceColor','r'); hold off
    xlim([0 156]); ylim([-4 4]);
    ylabel('Orbite');
    grid on
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles   reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
[betcX, betcY, Scx, Scy, Tune] = modeltwiss('beta','HCOR');
[phacX, phacY, Scx, Scy, Tune] = modeltwiss('phase','HCOR');
[betbX, betbY, Sbx, Sby, Tune] = modeltwiss('beta','BPMx');
[phabX, phabY, Sbx, Sby, Tune] = modeltwiss('phase','BPMx');
nux=Tune(1)*2*pi;
nuz=Tune(2)*2*pi;
