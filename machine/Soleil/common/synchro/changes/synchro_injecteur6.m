...
% Edit the above text to modify the response to help synchro_injecteur6

% Last Modified by GUIDE v2.5 05-Dec-2006 14:53:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @synchro_injecteur6_OpeningFcn, ...
                   'gui_OutputFcn',  @synchro_injecteur6_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before synchro_injecteur6 is made visible.
function synchro_injecteur6_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to synchro_injecteur6 (see VARARGIN)

% Choose default command line output for synchro_injecteur6
...
h = uibuttongroup('visible','off','Position',[0.1405 0.32 0.11 0.14],...
...
% UIWAIT makes synchro_injecteur6 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = synchro_injecteur6_OutputFcn(hObject, eventdata, handles) 
...
save('synchro_offset_lin', 'inj_offset' , 'ext_offset', 'lin_fin');
...
if strcmp(modeinj,'togglebutton_soft')
    ...
    elseif (modefill>4)&&(modefill<=7) %LPM
        for k=1:boucle
            for i=1:(modefill-3)
                clk1=handles.clk_pc(i);
                clk2=handles.clk_soft(i);
    ...
    elseif (modefill>=9) %SPM   
        clk1=handles.clk_pc(1);
        clk2=handles.clk_soft(1);
        table=handles.table;
        nb=table(1);
        dtour=table(2:1+nb);
        paq  =table(2+nb:1+2*nb);
        clk_pc  =int32(dtour) +  int32(clk1);
        clk_soft=int32(dtour)  + int32(clk2);    
        for k=1:boucle
            for i=1:nb  
                table=int32([1 0 paq(i)]);
                tango_command_inout('ANS/SY/CENTRAL','SetTables',table); pause(0.2)  ;             
                tango_write_attribute2('ANS/SY/CENTRAL', 'TPcStepDelay',clk_pc(i));
                tango_write_attribute2('ANS/SY/CENTRAL', 'TSoftStepDelay',clk_soft(i));
                tango_command_inout('ANS/SY/CENTRAL','FireSoftEvent');
                pause(dt)
...
switch mode  % Get Tag of selected object
    ...
    % switch to soft
        ...
        tango_write_attribute2('LIN/SY/LOCAL.LPM.1', 'lpmEvent',eventlpm); pause(tout);
        tango_write_attribute2('LIN/SY/LOCAL.SPM.1', 'spmLinacEvent',eventspm);
    ...
    % switch to 3Hz
        ...
        tango_write_attribute2('ANS/SY/LOCAL.SDC.1', 'oscEvent',event); pause(tout);
        tango_write_attribute2('LIN/SY/LOCAL.LPM.1', 'lpmEvent',int32(0)); pause(tout);
        ...
        tango_write_attribute2('BOO/SY/LOCAL.Binj.1', 'sep-p.trigEvent',event); pause(tout);
        tango_write_attribute2('BOO/SY/LOCAL.Binj.1', 'k.trigEvent',event);
        tango_command_inout2('BOO/SY/LOCAL.Binj.1',  'Reset');  % stop les trigs d'injection boo
        ...
        tango_write_attribute2('BOO/SY/LOCAL.Bext.1', 'sep-a.trigEvent',event);pause(tout);
        tango_write_attribute2('BOO/SY/LOCAL.Bext.1', 'k.trigEvent',event);pause(tout);
        tango_command_inout2('BOO/SY/LOCAL.Bext.1',  'Reset'); % stop les trigs d'extraction boo
        tango_write_attribute2('LT2/SY/LOCAL.DG.2', 'bpm.trigEvent',event);pause(tout);
        tango_write_attribute2('LT2/SY/LOCAL.DG.1', 'osc-fctEvent',event);pause(tout);
        tango_write_attribute2('LT2/SY/LOCAL.DG.1', 'mrsvEvent',event);pause(tout);
        tango_write_attribute2('BOO/SY/LOCAL.DG.3', 'emittanceEvent',event);pause(tout);
        ...
        tango_write_attribute2('ANS-C01/SY/LOCAL.Ainj.2', 'sep-p.trigEvent',event);pause(tout);
        tango_write_attribute2('ANS-C01/SY/LOCAL.Ainj.2', 'sep-a.trigEvent',event);pause(tout);
        tango_command_inout2('ANS-C01/SY/LOCAL.Ainj.2',  'Reset'); % stop les trigs d'injection anneau
...
switch mode  % Get Tag of selected object
    ...
    case 'lpm_mode'
        tango_write_attribute2('LIN/SY/LOCAL.LPM.1', 'lpmEvent',event32); 
        tango_write_attribute2('LIN/SY/LOCAL.SPM.1', 'spmLinacEvent',int16(0));     
        
        tango_write_attribute('LT1/AE/CV.2', 'current',  0.088 );
        tango_write_attribute('LT1/AE/CV.3', 'current', -0.34 );

    case 'spm_mode'
        tango_write_attribute2('LIN/SY/LOCAL.LPM.1', 'lpmEvent',int32(0)); 
        tango_write_attribute2('LIN/SY/LOCAL.SPM.1', 'spmLinacEvent',event16);   
