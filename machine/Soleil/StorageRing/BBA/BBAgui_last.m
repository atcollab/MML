...
if nargin<1 %Créer l'IHM
    ...
    delete(findobj(allchild(0),'Tag','Ola')); % detruit interface si existe deja
    for i=1:10
    ...
    RepRes = find_directory;
    setappdata(findobj(allchild(0),'Tag','Ola'),'RepRes',RepRes);
    ...
    for w=1:length(family)
        ...
        for loop=1:length(PlaneList0)
            ...
            elseif etat==0
                ...
                OffCor=0;
                StepCor=1.2;
                Delais=2.5; % modification si nécessaire
                StepQp=2.5;
            end
            ...
            for t=1:maxloop
                ...
                drawnow
                RepRes = getappdata(findobj(allchild(0),'Tag','Ola'),'RepRes');
                [merit,CorCons,BpmRes,offs,OffsCor,center,ze,ord1,ord2,BpmDevRes,CorDevRes,iniCor,rmsFit,NameFile,NameFilePNG] = ...
                    bba_last(RepRes,family(w),QpDevList(w,:), PlaneList0(loop),char(MetList),nbDeltaCor,StepCor,OffCor,Delais,StepQp,CorNumber);
                %center
...
function RepRes = find_directory
%% Mémorisation du répertoire de travail
Rep    = getfamilydata('Directory','BBA');

Announcement = strcat('  ***        BBA  Registration Directory                             ***');
disp('  ****************************************************************************');
disp(Announcement);
disp('  **                                                                        **');
disp('  ****************************************************************************');
...
button = questdlg('Is the BBA Registration Directory already existing ?','BBA Registration Directory','yes','no','yes') ;
