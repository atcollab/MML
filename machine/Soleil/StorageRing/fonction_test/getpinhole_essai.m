...
S = tango_read_attributes(devAna ,{'PixelSizeX','PixelSizeY','GammaCorrection',...
    'XProfileSigma','XProfileMag','YProfileSigma','YProfileMag','XProfile','YProfile',...
    'XProfileFitted','YProfileFitted','GaussianFitTilt','XProfileBG','YProfileBG',...
    'XProfileCenter','YProfileCenter','OpticalMagnification'...
    'AutoROIOriginX','AutoROIOriginY','AutoROIWidth','AutoROIHeight'})

rep.pixelsizex = S(1).value(1)%readattribute([devAna '/PixelSizeX']);
rep.pixelsizez = S(2).value(1)%readattribute([devAna '/PixelSizeY']);
rep.gamma = S(3).value(1)%readattribute([devAna '/GammaCorrection']);
rep.sigmax = S(4).value%readattribute([devAna '/XProfileSigma']);
rep.magnitudex = S(5).value%readattribute([devAna '/XProfileMag']);
rep.sigmaz =S(6).value %readattribute([devAna '/YProfileSigma']);
rep.magnitudez =S(7).value %readattribute([devAna '/YProfileMag']);
rep.X = S(8).value%tango_read_attribute2(devAna,'XProfile');
rep.Z =S(9).value %tango_read_attribute2(devAna,'YProfile');
rep.fitX = S(10).value%tango_read_attribute2(devAna,'XProfileFitted');
rep.fitZ = S(11).value%tango_read_attribute2(devAna,'YProfileFitted');
rep.GaussianFitTilt = S(12).value%readattribute([devAna '/GaussianFitTilt']);
rep.growth = S(17).value(1)%readattribute([devAna '/OpticalMagnification']);
...
rep.XProfileBG = S(13).value;
rep.YProfileBG = S(14).value;
rep.XProfileCenter = S(15).value;
rep.YProfileCenter = S(16).value;
rep.AutoROIOriginX = S(18).value;
rep.AutoROIOriginY = S(19).value;
rep.AutoROIWidth = S(20).value;
rep.AutoROIHeight = S(21).value;
...
if DisplayFlag
    ...
    %figure(105);
    figure
    plot(rep.fitX,'k') ; hold on ; plot(rep.X,'r') ;
    xlabel('numero de pixel')
    plot(rep.fitZ,'p') ; hold on ; plot(rep.Z,'b') ; legend('fit H de l''ImageAnalyser','Données brutes H','fit V de l''ImageAnalyser','Données brutes V')
    xlabel('numero de pixel')
    title('Profils H et V projetés dans la région d''interet (ROI)');
    % Gaussienne du fit de l'image analyser :
    for j=rep.AutoROIOriginY:rep.AutoROIOriginY + rep.AutoROIHeight
        Yfit(j) = rep.magnitudez *  exp(-(double(j)*rep.pixelsizez/rep.growth-rep.YProfileCenter)*(-rep.YProfileCenter)/(2*rep.sigmaz*rep.sigmaz)) + rep.YProfileBG;
    end
    hold on ; plot(Yfit,'sb')
    hold off
    %%%%%%%%%%%%%%% WARNING
