function [emittance taille] = getpinhole(varargin)
% GETPINHOLE enregistre les images de la caméra pinhole avec les paramètres
% environnants
%
%  INPUTS
%  1. 'Display' affiche l'image enregistrée (par défaut)
%  2. 'NoDisplay' le contraire..
%  3. 'Archive' sauvegarde la structure matlab (par défaut)
%  4. 'NoArchive' le contraire..
%
% OUTPUTS
% 
...
if DisplayFlag
    ...
    plot(rep.fitZ.value,'k') ; hold on ; plot(rep.Z.value,'b') ; legend('fit H de l''ImageAnalyser','Données brutes H','fit V de l''ImageAnalyser','Données brutes V') 
    xlabel('numero de pixel')
    ...
    EkeV_ref = 8.5e3;
    TV2 = 7.4e-6/2.60 ;                       % TV2  en µm/pixel sur le plan convertisseur   7.5e-6/2.60
    TH2 = 7.4e-6/2.60 ;                       %TH2 = 11.4e-6 % 13.38e-6 ;  % Grandissement H camera 2
    ...
    [eps_z rms_z_conv rms_z_source] = fun(EkeV,sigma_z,TV2,wV,yp_siph_ref,EkeV_ref,d,D,NBPIXEL,NBCONVCRENEAU,sigmae,eta,etap,beta,alpha,gammaPM)
    ...
    alpha = 0.0825 ;
    gammaPM = (1 + alpha)/beta;
    [eps_x rms_x_conv rms_x_source] = fun(EkeV,sigma_x,TH2,wH,yp_siph_ref,EkeV_ref,d,D,NBPIXEL,NBCONVCRENEAU,sigmae,eta,etap,beta,alpha,gammaPM)
...
if ArchiveFlag
    ...
    fprintf('Energie pic des photons X (keV)  %11.2f \n',EkeV*1e-3);
...
function [emittance S SV] = fun(EkeV,Nbpixelsmes,T2,w,yp_siph_ref,EkeV_ref,d,D,NBPIXEL,NBCONVCRENEAU,sigmae,eta,etap,beta,alpha,gammaPM)
% calcul d'émittance
