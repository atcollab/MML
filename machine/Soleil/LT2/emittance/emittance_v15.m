...
% Last Modified by GUIDE v2.5 20-May-2005 09:14:01
...
setappdata(handles.figure1,'nbenregistremen99ts',st);
...
device_name.videograbber = 'lt2/dg/emit-vg';
device_name.iris = 'lt2/dg/emit-iris';
device_name.lentille_haute = 'lt2/dg/emit-l.haut';
device_name.lentille_basse = 'lt2/dg/emit-l.bas';
device_name.miroir = 'lt2/dg/emit-mc';
device_name.backlight = 'lt2/dg/emit-bl';
device_name.ecran = 'lt2/dg/emit-ecr';
device_name.Q1 = 'lt2/AE/Q.1';
device_name.Q2 = 'lt2/AE/Q.2';
device_name.Q3 = 'lt2/AE/Q.3';
device_name.Q4 = 'lt2/AE/Q.4';
device_name.H1 = 'lt2/AE/CH.1';
device_name.H2 = 'lt2/AE/CH.2';
device_name.V1 = 'lt2/AE/CV.1';
...
%DirectoryName = '/home/PM/nadolski/controlroom/measdata/lt2data/';
...
% 
% valH1= (read_magnet(device_name.H1)); 
...
% tango_command_inout('lt2/emit/mirror','Off');
% tango_command_inout('lt2/emit/backlight','SetBrightness',uint16(0));
% miroir LT2 : d�faut c�blage. Provisoirement on �change commande dans
% MATLAB et non pas dans le DServer
...
setappdata(handles.figure1,'posl2Gmax',34600);
setappdata(handles.figure1,'posl1Gmoy',8000);
setappdata(handles.figure1,'posl2Gmoy',19500);
setappdata(handles.figure1,'posl1Gmin',0);
setappdata(handles.figure1,'posl2Gmin',3046);
...
% % %         % on inscrit en dur le grandissement
% % %         %toto = 14.4
%         toto = 38.
%         val=sprintf('%2.1f',toto)
...
figure(3);
tx1=handles.matrx(:,1)
tx2=handles.matrx(:,2).*handles.matrx(:,2)

ty1=handles.matry(:,1)
ty2=handles.matry(:,2).*handles.matry(:,2)
...
Z = [ones(size(T))  T  T.^2]*a;
hold on
%plot(T,Z,'r-',tx1,tx2,'ro'), grid on
plot(tx1,tx2,'ro',T,Z,'r-'), grid on

Y = [ones(size(ty1))  ty1  ty1.^2];
a=Y\ty2;
Tmin=min(ty1);
Tmax=max(ty1);
T=[Tmin:0.1:Tmax]';
Z = [ones(size(T))  T  T.^2]*a;
hold on
%plot(T,Z,'b-',ty1,ty2,'b+'), grid on
plot(ty1,ty2,'b+',T,Z,'b-'), grid on
...
legend('Mesures Horizontales','Fit Horizontal','Mesures verticales','Fit vertical','Location','North')
...
% else
    ...
    tango_write_attribute(device_name.(aimant),'currentPM',res);
% end
...
%idem pb c�blage miroir LT2 provisoirement modif dans MATLAB
%tango_command_inout(device_name.miroir,'On');
tango_command_inout(device_name.miroir,'Off');
...
% positionner le miroir HORS, eteindre le backlight
% provisoirement pb de c�blage du miroir LT2 et modif dans MATLAB
%tango_command_inout(device_name.miroir,'Off');
tango_command_inout(device_name.miroir,'On');
tango_command_inout(device_name.backlight,'SetBrightness',uint16(0));
...
% Attention à l'étalonnage du gradient qui n'est valable que pour les
% valeurs absolues des courants. Mais OK pour Q4 qui est conv Horiz
...
% --- Executes on button press in ellipse_pushbutton13.
function ellipse_pushbutton13_Callback(hObject, eventdata, handles)
% hObject    handle to ellipse_pushbutton13 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%         name='backlight';
%         numlines=1;
%         lumiere=tango_command_inout('lt2/emit/backlight','GetBrightness');
%         %- check error
%         if (tango_error == -1)
%               %- handle error
%               tango_print_error_stack;
...
%             tango_command_inout('lt2/emit/backlight','SetBrightness',uint16(str2num(intensite{:})));
%         end
% 
