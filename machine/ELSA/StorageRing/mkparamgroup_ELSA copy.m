...
%
%  2009-01-27 Oliver Preisner
%     Added shifts in x and y direction ('SHIFTX','SHIFTY') as one possible
%     parameter
...
switch lower(PARAMSTR)
    ...
    case 'shiftx'
        if ~isfield(LATTICE{INDEX(KIDNUM(1))},'T1')
            error('Element ',int2str(KIDNUM(1)),' does not have field ''T1''');
        end
        if ~isfield(LATTICE{INDEX(KIDNUM(1))},'T2')
            error('Element ',int2str(KIDNUM(1)),' does not have field ''T2''');
        end

        P1 = struct('ElemIndex',num2cell(INDEX(KIDNUM)),'FieldName','T1','Function',inline('x'));
        [P1.FieldIndex]=deal({1,1});
        [P1.Args]=deal({});

        P2 = struct('ElemIndex',num2cell(INDEX(KIDNUM)),'FieldName','T2','Function',inline('-x'));
        [P2.FieldIndex]=deal({1,1});
        [P2.Args]=deal({});

        for k = 1:length(KIDNUM)
            P1(k).SavedValue = LATTICE{INDEX(KIDNUM(k))}.T1(1);
            P2(k).SavedValue = LATTICE{INDEX(KIDNUM(k))}.T2(1);
        end
        P = reshape([P1;P2],1,2*length(P1));
    ...
    case 'shifty'
        ...
        P1 = struct('ElemIndex',num2cell(INDEX(KIDNUM)),'FieldName','T1','Function',inline('x'));
        [P1.FieldIndex]=deal({1,3});
        [P1.Args]=deal({});

        P2 = struct('ElemIndex',num2cell(INDEX(KIDNUM)),'FieldName','T2','Function',inline('-x'));
        [P2.FieldIndex]=deal({1,3});
        [P2.Args]=deal({});

        for k = 1:length(KIDNUM)
            P1(k).SavedValue = LATTICE{INDEX(KIDNUM(k))}.T1(3);
            P2(k).SavedValue = LATTICE{INDEX(KIDNUM(k))}.T2(3);
        end
        P = reshape([P1;P2],1,2*length(P1));
end
