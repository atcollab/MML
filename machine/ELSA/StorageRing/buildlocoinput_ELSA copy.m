...
% ADDITIONAL STUFF BY Oliver Preisner
% l. 595 f. Questions for fitting of energy shifts in corrector magnets
% (2009-01-21)
...
% Fitting of energy shifts at corrector magnets (by O. Preisner)
choice=menu('Fit energy shifts at corrector magnets?','Yes!', 'No!');

if choice==1;
    CMData.FitHCMEnergyShift = 'Yes';
    CMData.FitVCMEnergyShift = 'Yes';
elseif choice==2;
...
if ~GuessIt
    if exist('buildlocofitparameters_ELSA', 'file')
        buildlocofitparameters_ELSA(OutputFileName);
        fprintf('   It is done: a LOCO input file for ELSA has just been created!\n');        
    else
        fprintf('   buildlocofitparameters_ELSA.m was not found so the FitParameters variable still needs to be created.\n');
    end
end
