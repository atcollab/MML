...
if Orbit.PilotTone
    ...
    xlabel('Turns [~656 ns / turn]', 'FontSize', FontSize);
    ylabel('RMS [Counts]', 'FontSize', FontSize);
    title(sprintf('Channel RMS at the Pilot Tone Freq = %.3f MHz  (RF = %.6f MHz)', Orbit.f(Orbit.nHarmPT(1)), ENV.Machine.Clock), 'FontSize', FontSize);
    ...
    title(sprintf('Ratio of the Pilot Tone RMS at %.3f MHz  (RF = %.6f MHz)', Orbit.f(Orbit.nHarmPT(1)), ENV.Machine.Clock), 'FontSize', FontSize);
    ylabel('B/A', 'FontSize', FontSize);
    axis tight
    subplot(3,1,2);
