...
h1(length(h1)+1,1) = subplot(2,1,2);
...
figure(FigNum+1);
clf reset
h1(length(h1)+1,1) = subplot(2,1,1);
...
clf reset
h1(length(h1)+1,1) = subplot(2,1,1);
loglog(f(3:end), 1e6*T1*Pss(3:end));
title(sprintf('TBT Sum PSD (%d points)', N), 'FontSize', FontSize);
ylabel('Sum', 'FontSize', FontSize);
axis tight;

h1(length(h1)+1,1) = subplot(2,1,2);
semilogx(f(2:end), 1e6*Pss_int(2:end));
xlabel('Frequency [Hz]', 'FontSize', FontSize);
ylabel('Sum {^2}', 'FontSize', FontSize);
title(sprintf('\\fontsize{%d}Cumulative  \\fontsize{16}\\int \\fontsize{12}PSD df  (RMS: %.3f)', FontSize+2, 1000*sqrt(Pss_int(end))), 'FontSize', FontSize);
axis tight;
...
h2(length(h2)+1,1) = subplot(2,1,2);
