...
if~isempty(strfind(Prefix,'Test')) || ~isempty(strfind(Prefix,'SR')) || ~isempty(strfind(Prefix,'bpmTest'))
    ...
    ENV.PT.AutoTrimControl    = getpvonline([Prefix,':autotrim:control']);
...
elseif ~isempty(strfind(Prefix,'GTL')) ||  ~isempty(strfind(Prefix,'LN')) ||  ~isempty(strfind(Prefix,'LTB')) || ~isempty(strfind(Prefix,'LTB')) || ~isempty(strfind(Prefix,'BTS'))
    ENV.Machine.Type = 'TransportLine';
    ENV.Calc.Type = 'Transport';
    ENV.Calc.SelfTrigger = 1;
    ENV.Calc.N = 77;  %100;   % ???  Number of points in an orbit calculation  Actually 77 with an offset from the trigger
    ENV.PT = [];
else
