function varargout = graphit_aa(varargin)
...
% Edit the above text to modify the response to help graphit_aa
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @graphit_aa_OpeningFcn, ...
    'gui_OutputFcn',  @graphit_aa_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before graphit_aa is made visible.
function graphit_aa_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to graphit_aa (see VARARGIN)

% Choose default command line output for graphit_aa
...
% UIWAIT makes graphit_aa wait for user response (see UIRESUME)
...
ZoomCallBack = @(hObject,eventdata)graphit_aa('AxesButtonDown_Callback',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'ZoomCallBack', ZoomCallBack);

IdentifyLine_CallBack = @(hObject,eventdata)graphit_aa('IDLine_Callback',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'IDLineCallBack', IdentifyLine_CallBack);


YZoomCallBack = @(hObject,eventdata)graphit_aa('YAxesButtonDown_Callback',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'YZoomCallBack', YZoomCallBack);



YLabelButtonDown =  @(hObject, eventdata)graphit_aa('Label_ButtonDownFcn',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'YLabelButtonDown', YLabelButtonDown);

DYGrid_CallBack = @(hObject,eventdata)graphit_aa('YGrid_Callback',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'YGridCallBack', DYGrid_CallBack);


DXGrid_CallBack = @(hObject,eventdata)graphit_aa('XGrid_Callback',hObject,eventdata,guidata(hObject));
...
jToolbar = [];  % GJP get(get(hToolbar,'JavaContainer'),'ComponentPeer');
...
function varargout = graphit_aa_OutputFcn(hObject, eventdata, handles)
...
for p = [1:5 7:ngraph]
    ...
    try
        ...
        tic
        if p == 6
            pvnames = GR{p-1}.Data.ChannelNames;
        else
            pvnames = GR{p}.Data.ChannelNames;
        end
    ...
    fprintf('   Data retreived from %s to %s\n', datestr(datacell{1}.data.datenum(1),31), datestr(datacell{1}.data.datenum(end),31));
    ...
    % if no data, i.e. empty data check if no error: should never happen as cell array initializedto empty now
...
% ignore future time
if(T1Time > now)
    fprintf('%s',strcat('[T1 Not Valid Date:' , datestr(T1Time),']'));
    return;
elseif(T2Time > now)
...
if strcmpi(getappdata(handles.figure1, 'WriteToWebPageFlag'),'on')
    ...
    if(rem(get(handles.thetimer, 'TasksExecuted'),n) == 0)
        try
            ...
            fprintf('EXCEPTION at WriteToWebPage please report error to Paola Pace. %s \n',ME.message);
        end
    end
end
