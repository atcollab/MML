function varargout = graphit_bpm_aa(varargin)
% GRAPHIT_BPM M-file for graphit_bpm_aa.fig
...
%      applied to the GUI before graphit_bpm_aa_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to graphit_bpm_aa_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help graphit_bpm_aa

% Last Modified by GUIDE v2.5 17-Jan-2016 11:45:58

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @graphit_bpm_aa_OpeningFcn, ...
    'gui_OutputFcn',  @graphit_bpm_aa_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before graphit_bpm_aa is made visible.
function graphit_bpm_aa_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to graphit_bpm_aa (see VARARGIN)


RemoveBPMDeviceList = [];

% Choose default command line output for graphit_bpm_aa
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes graphit_bpm_aa wait for user response (see UIRESUME)
...
ZoomCallBack = @(hObject,eventdata)graphit_bpm_aa('AxesButtonDown_Callback',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'ZoomCallBack', ZoomCallBack);

IdentifyLine_CallBack = @(hObject,eventdata)graphit_bpm_aa('IDLine_Callback',hObject,eventdata,guidata(hObject));
setappdata(handles.figure1, 'IDLineCallBack', IdentifyLine_CallBack);

YZoomCallBack = @(hObject,eventdata)graphit_bpm_aa('YAxesButtonDown_Callback',hObject,eventdata,guidata(hObject));
...
if strcmpi(PresentMarker, '.')
    ...
    fprintf('%s.%s(%d,%d)  %s\n', Family, Field,DeviceList(iLine,:), deblank(ChannelNames(iLine,:)));
...
function varargout = graphit_bpm_aa_OutputFcn(hObject, eventdata, handles)
...
% Check for Daylight savings time and convert to UTC time
if isdaylightsavings(EndTime)
    end_tf = EndTime + 7/24;
else
    end_tf = EndTime + 8/24;
end
...
for p=1:ngraph
    ...
    end_t = end_tf;
    % Sometimes TimeBack is negative??
    if TimeBack < 0
        start_t = addtodate(end_t,(round(TimeBack)),'second');
    else
        start_t = addtodate(end_t,-1*(round(TimeBack)),'second');
    end
    % new archiver needs date in this format.
    if ~ischar(start_t)
        start_t = datestr(start_t, 'yyyy-mm-ddTHH:MM:SS.FFFZ');
    end
    if ~ischar(end_t)
        end_t = datestr(end_t, 'yyyy-mm-ddTHH:MM:SS.FFFZ');
    end
    
    fprintf('\n\n +++++++++++++++Graph n=%d (%s to %s)\n', p, start_t, end_t);
    fprintf('Retrieving data..graph n=%d',p);
    ...
    fprintf('Retieving data ... graph n=%d/%d\n', p,(ngraph));
    %drawnow;
    ...
    try
        % Grab PV names for graph and put into cell array
        charList = ArchiverGetStringSet{p};
        charList = ['|' charList '|']; % add spacing at the ends to make transformation easier
        separatorIndx = find(charList == '|');
        for j = 1:length(separatorIndx)-1
            pvnames{j,1} = charList(separatorIndx(j)+1:separatorIndx(j+1)-1);
        end
        
        
        % Grab data from AA archiver
        tic
        data = client.getData(pvnames, start_t, end_t);
        fprintf('   client.getData time = %.3f seconds\n', toc);
    ...
    datacell = aaepoch2datenum(data);
    
    clear NPoints t a
    for l = 1:length(pvnames)
        NPoints(l,1) = length(datacell{l}.data.values);
        
        for j = 1:NPoints(l)
            % temporary solution here.
            datetemp = datacell{l}.data.datenum(j);
            %datetemp = addtodate(datetemp,1,'hour');
            t{l}(j) = datetemp;
            a{l}(j) = datacell{l}.data.values(j);
        end
    end
    ...
    % Convert time back from UTC/Archiver time to MATLAB time (PacificTime)
    % Get Time difference between UTC time and Pacific Time, this changes for DST
    ...
    if(isempty(aoldSet{p})) % No old data in graph
        ...
        %timer call (nargin <2)
        if nargin < 2
            % first, drop data out of Time Window set in timer menu ... empty not ok otherwise
            temp=told;
            temp(cellfun(@isempty,temp)) = {NaN};
    ...
    if p == 3
        ...
        last30SecRef = EndTime - 4*30/60/60/24; % 4*30 sec back in time - in Matlab time
...
% ignore future time
if(T1Time > now)
    fprintf('%s\n',strcat('[T1 Not Valid Date:' , datestr(T1Time),']'));
    return;
elseif(T2Time > now)
    fprintf('%s\n',strcat('[T2 Not Valid Date:' , datestr(T2Time),']'));
    return;
end
...
if(~isempty(stringT1) && ~isempty(stringT2))
    fprintf('%s\n',strcat('[T1=',stringT1,'/', 'T2=' , stringT2,']'));
...
elseif(T1Time < T2Time)
    ...
    fprintf('%s\n','Please set T1 T2 for Time Interval');
    return;
end
...
if(~isempty(T1string))
    fprintf('%s\n',strcat('[T1=',T1string,'-', 'T2=' , datestr(T2),']'));
else
    fprintf('%s\n',strcat('[T2=' , datestr(T2),']'));
end
drawnow;
...
if(~isempty(T2string))
    fprintf('%s\n',strcat('[T1=',datestr(T1),'-', 'T2=' , T2string,']'));
else
    fprintf('%s\n',strcat('[T1=' , datestr(T1),']'));
end
