...
set(gca,'fontsize',24);
x = [0,          0, 0,0;
    Cam.Cols , Cam.Cols,Cam.Cols,Cam.Cols];
y = [200,400,600,800;
     200,400,600,800;];
hline = line(x,y);
set(hline,'LineStyle','--','Color',[1,1,1]*0.6);

y = [0,          0, 0,0,0;
    Cam.Rows , Cam.Rows,Cam.Rows,Cam.Rows,Cam.Rows];
...
while 1 
    ...
    SigmaX = getpv('beamline31:XRMSAve');
    SigmaY = getpv('beamline31:YRMSAve');
    OffsetX =getpv('beamline31:XCentAve');
    OffsetY =getpv('beamline31:YCentAve');
   % set(htitle,'string', sprintf('Image Number %d   %s',Cam.ImageNumber, datestr(Cam.DataTime, 'HH:MM:SS.FFF')));
   % set(htitle,'string', sprintf('Image Number %d    \\sigma_x=%8.1f    \\sigma_y=%8.1f     Xoffset=%8.1f    Yoffset=%8.1f  %s',Cam.ImageNumber, Cam.SigmaX*CaliFactorX, Cam.SigmaY*CaliFactorY, Cam.OffsetX, Cam.OffsetY, datestr(Cam.DataTime, 'HH:MM:SS.FFF')));
   set(htitle,'string', sprintf('\\sigma_x=%8.1f,  \\sigma_y=%8.1f,  x_c=%8.1f,  y_c=%8.1f ',SigmaX, SigmaY, OffsetX, OffsetY));
   drawnow;
    
%     
%     ii = ii+1;
%     AveSigmaX = AveSigmaX+Cam.SigmaX;
%     AveSigmaY = AveSigmaY+Cam.SigmaY;
%     AveCentX  = AveCentX+Cam.OffsetX;
%     AveCentY = AveCentY+Cam.OffsetY;
%     if ii>=AveNo
%         AveSigmaX = AveSigmaX/ii;
%         AveSigmaY = AveSigmaY/ii;
%         AveCentX =AveCentX/ii;
%         AveCentY = AveCentY/ii;
%       %  setpvonline('bl31:XRMSAve',AveSigmaX*CaliFactorX);
%       %  setpvonline('bl31:YRMSAve',AveSigmaY*CaliFactorY);
%       %  setpvonline('bl31:XCentAve',AveCentX);
%       %  setpvonline('bl31:YCentAve',AveCentY);
%      
%       %  setpvonline('beamline31:XRMSAve',AveSigmaX*CaliFactorX);
%       %  setpvonline('beamline31:YRMSAve',AveSigmaY*CaliFactorY);
%       %  setpvonline('beamline31:XCentAve',AveCentX);
%       %  setpvonline('beamline31:YCentAve',AveCentY);
%       
%         AveSigmaX= 0;
%         AveSigmaY= 0;
%         AveCentX = 0;
