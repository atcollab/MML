function varargout = alslat_MultipleRings(NRings)
% Lattice definition file
% 1. Superbend (SB)& BEND family
% 2. QF, QD, QDA quadrupole families
% 3. SF, SD sextupole families
% 4. EtaX straight sections = 6 cm
% 5. BENDs split at the beamline source points
% 6. 122 BPMs
% 7. Fast kicker is sector 2
% 8. Multiple rings concatenated
...
% I'm not sure why I need this line here (GJP)
%AP = aperture('AP',  [-0.1, 0.1, -0.1, 0.1], 'AperturePass');


L0 = NRings * 196.8054150;     % design length [m]
C0 = 299792458;       % speed of light [m/s]


% RF SYSTEM
HarmNumber = 328*NRings;
...
% BEND = rbend('BEND', 0.86514, 0.1745329, 0.0872664, 0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');
% B1a  = rbend('B1a',  6.3/10*0.86514, 6.3/10*0.1745329, 0.0872664, 0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
% B1b  = rbend('B1b',  3.7/10*0.86514, 3.7/10*0.1745329, 0.0,       0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');
% B2a  = rbend('B2a',  2.6/10*0.86514, 2.6/10*0.1745329, 0.0872664, 0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
% B2b  = rbend('B2b',  4.8/10*0.86514, 4.8/10*0.1745329, 0.0,       0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
% B2c  = rbend('B2c',  2.6/10*0.86514, 2.6/10*0.1745329, 0.0,       0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');
% B3a  = rbend('B3a',  2.6/10*0.86514, 2.6/10*0.1745329, 0.0872664, 0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
% B3b  = rbend('B3b',  7.4/10*0.86514, 7.4/10*0.1745329, 0.0,       0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');

B1a  = rbend('BEND', 6.3/10*0.86514, 6.3/10*0.1745329, 0.0872664, 0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
B1b  = rbend('BEND', 3.7/10*0.86514, 3.7/10*0.1745329, 0.0,       0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');
B2a  = rbend('BEND', 2.6/10*0.86514, 2.6/10*0.1745329, 0.0872664, 0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
B2b  = rbend('BEND', 4.8/10*0.86514, 4.8/10*0.1745329, 0.0,       0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
B2c  = rbend('BEND', 2.6/10*0.86514, 2.6/10*0.1745329, 0.0,       0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');
B3a  = rbend('BEND', 2.6/10*0.86514, 2.6/10*0.1745329, 0.0872664, 0.0,       -0.7782,'BndMPoleSymplectic4RadPass');
B3b  = rbend('BEND', 7.4/10*0.86514, 7.4/10*0.1745329, 0.0,       0.0872664, -0.7782,'BndMPoleSymplectic4RadPass');
...
BSa =  rbend('BS', .26*0.255, .26*0.1745329, 0.0872665, 0.0,       0,'BndMPoleSymplectic4RadPass');
BSb =  rbend('BS', .48*0.255, .48*0.1745329, 0.0,       0.0,       0,'BndMPoleSymplectic4RadPass');
BSc =  rbend('BS', .26*0.255, .26*0.1745329, 0.0,       0.0872665, 0,'BndMPoleSymplectic4RadPass');
...
% SECT1  =  marker('StraightCenter', 'IdentityPass');
% SECT2  =  marker('StraightCenter', 'IdentityPass');
% SECT3  =  marker('StraightCenter', 'IdentityPass');
% SECT4  =  marker('StraightCenter', 'IdentityPass');
% SECT5  =  marker('StraightCenter', 'IdentityPass');
% SECT6  =  marker('StraightCenter', 'IdentityPass');
% SECT7  =  marker('StraightCenter', 'IdentityPass');
% SECT8  =  marker('StraightCenter', 'IdentityPass');
% SECT9  =  marker('StraightCenter', 'IdentityPass');
% SECT10 =  marker('StraightCenter', 'IdentityPass');
% SECT11 =  marker('StraightCenter', 'IdentityPass');
% SECT12 =  marker('StraightCenter', 'IdentityPass');
...
SUP1  = [ L1 L2 BPM L3 QF L4 COR L5 QD  ...
    L6 BPM L7 L8 B1a BL B1b L9 SDD COR SDD L10 ...
    BPM L11 QFA L12  SFF COR SFF L13 BPM  ...
    L14 B2a BL B2b BL B2c L15 BPM L16  SFF COR SFF  L17  ...
    QFA L18 BPM L19  SDD COR SDD L20 B3a BL B3b L21 ...
...
SUP4  = [ L1s4a IDBPM L1s4b CHIC COR L1s4c IDBPM L1s4d IDBPM L1s4e CHIC L1s4f COR L2 BPM L3 QF L4 COR L5 QD  ...
    ...
    LS3 BSa BL BSb BL BSc LS3 BPM LS2 QDA LS1  SFF COR SFF  L17  ...
    QFA L18 BPM L19  SDD COR SDD L20 B3a BL B3b L21 ...
    BPM L22 QD L23 COR L24 QF L25  ...
...
for i = 1:NRings
    THERING = setcellstruct(THERING, 'K',        QFI([7:8 15:16 23:24]) + length(ELIST1)*(i-1), 2.219784);
    THERING = setcellstruct(THERING, 'PolynomB', QFI([7:8 15:16 23:24]) + length(ELIST1)*(i-1), 2.219784, 2);
end

QDI = findcells(THERING, 'FamName', 'QD');
for i = 1:NRings
    THERING = setcellstruct(THERING, 'K',        QDI([7:8 15:16 23:24]) + length(ELIST1)*(i-1), -2.483259);
    THERING = setcellstruct(THERING, 'PolynomB', QDI([7:8 15:16 23:24]) + length(ELIST1)*(i-1), -2.483259,2);
end

QFAI = findcells(THERING,'FamName','QFA');
for i = 1:NRings
    THERING = setcellstruct(THERING, 'K',        QFAI([7:8 15:16 23:24]) + length(ELIST1)*(i-1), 3.120815);
    THERING = setcellstruct(THERING, 'PolynomB', QFAI([7:8 15:16 23:24]) + length(ELIST1)*(i-1), 3.120815,2);
end
...
% Add an injection bump
if 0
    HCMI = findcells(THERING, 'FamName', 'COR');
    % Radians for a -4.5 mm injection bump
    DeltaHCM = [
        -0.000464865899849
        -0.001152895137263
        -0.001159649017556
    ...
    for i = 1:NRings
        RingStart = length(ELIST1)*(i-1);
        THERING{RingStart+HCMI(1)}.KickAngle  = [DeltaHCM(1) 0];
        THERING{RingStart+HCMI(2)}.KickAngle  = [DeltaHCM(2) 0];
        THERING{RingStart+HCMI(96)}.KickAngle = [DeltaHCM(3) 0];
        THERING{RingStart+HCMI(97)}.KickAngle = [DeltaHCM(4) 0];
    end
end

if NRings == 5
    % Tune change
    fprintf('   Detuning the lattice by a small amount.\n');
    % m=gettuneresp('Physics')
    m = [
        +8.7939   +1.3458
        -5.6226   -7.8935];
    deltaQ = inv(m) * [0; -.03];
    %deltaQ = inv(m) * [.005; -.03];  % Injection bump seems to require a horizontal tune shift to get off the .25 resonance
    deltaQ(1) = -0.00026032064286/2;
    deltaQ(2) =  0.00170144962480/2;

    %[a,b]=modeltune,t=(a+b)/5
    %  71.275
    %  45.853
    %Divide by 5 equals,
    %  14.255074346506678
    ...
    QFI = findcells(THERING, 'FamName', 'QF');
    for i = 1:length(QFI)
        K = THERING{QFI(i)}.K + deltaQ(1);
        THERING = setcellstruct(THERING, 'K',        QFI(i), K);
        THERING = setcellstruct(THERING, 'PolynomB', QFI(i), K, 2);
    end

    QDI = findcells(THERING, 'FamName', 'QD');
    for i = 1:length(QDI)
        K = THERING{QDI(i)}.K + deltaQ(2);
        THERING = setcellstruct(THERING, 'K',        QDI(i), K);
        THERING = setcellstruct(THERING, 'PolynomB', QDI(i), K, 2);
    end
end
