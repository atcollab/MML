...
% two sextupole families, BPMs as markers, no correctors, QFA
% magnets as one chain.

global FAMLIST THERING GLOBVAL
GLOBVAL.E0 = 1.9e9;
GLOBVAL.LatticeFile = 'als_symp_1sb_old';
FAMLIST = cell(0);
disp(' ');
disp('** Loading ALS lattice in alslat_symp_1sb_old.m **');
...
QF		=	quadrupole('QF'  , 0.344, 2.259603 ,'StrMPoleSymplectic4RadPass');
QD		=	quadrupole('QD'  , 0.187, -2.303794 ,'StrMPoleSymplectic4RadPass');
QFA		=	quadrupole('QFA'  , 0.448, 2.888348 ,'StrMPoleSymplectic4RadPass');
QDA		=	quadrupole('QDA'  , 0.2, -1.046695 ,'StrMPoleSymplectic4RadPass');
...
THERING = setcellstruct(THERING,'K',QFI([15:16]),2.162669);
THERING = setcellstruct(THERING,'PolynomB',QFI([15:16]),2.162669,2);
QDI = findcells(THERING,'FamName','QD');
THERING = setcellstruct(THERING,'K',QDI([15:16]),-2.129039);
THERING = setcellstruct(THERING,'PolynomB',QDI([15:16]),-2.129039,2);
