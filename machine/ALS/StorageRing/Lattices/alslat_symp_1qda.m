function alslat_symp_1qda
% Lattice definition file (alslat_symp_1qda)
...
% two sextupole families, BPMs as markers, no correctors, one
% sector with QDA magnets (broken symmetry).

global FAMLIST THERING GLOBVAL
GLOBVAL.E0 = 1.9e9;
GLOBVAL.LatticeFile = 'als_symp_1qda';
FAMLIST = cell(0);
disp(' ');
disp('** Loading ALS lattice in alslat_symp_1qda.m **');
...
QDA		=	quadrupole('QDA'  , 0.2, -0.4350 ,'StrMPoleSymplectic4RadPass');
...
ELIST = [INJ SECT1 SUP SECT2 SUP SECT3 CAV SUP SECT4 SUP SECT5 SUP SECT6 SUP ...
	SECT7 SUP SECT8 SUPS SECT9 SUP SECT10 SUP SECT11 SUP SECT12 SUP FIN];
...
THERING = setcellstruct(THERING,'K',QFI([15:16]),2.2808);
THERING = setcellstruct(THERING,'PolynomB',QFI([15:16]),2.2808,2);
QDI = findcells(THERING,'FamName','QD');
THERING = setcellstruct(THERING,'K',QDI([15:16]),-2.3257);
THERING = setcellstruct(THERING,'PolynomB',QDI([15:16]),-2.3257,2);
QFAI = findcells(THERING,'FamName','QFA');
THERING = setcellstruct(THERING,'K',QFAI([15:16]),2.9897);
THERING = setcellstruct(THERING,'PolynomB',QFAI([15:16]),2.9897,2);
