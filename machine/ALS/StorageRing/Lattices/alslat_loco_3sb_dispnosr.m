function varargout = als_loco_3sb_disp_nosr(varargin)
...
GLOBVAL.E0 = 1.9e9;
GLOBVAL.LatticeFile = 'als_loco_3sb_dispnosr';
FAMLIST = cell(0);
disp(' ');
disp('** Loading ALS lattice in alslat_loco_3sb_dispnosr.m **');
...
QF		=	quadrupole('QF'  , 0.344, 2.213483 ,'QuadLinearPass');
QD		=	quadrupole('QD'  , 0.187, -2.356502 ,'QuadLinearPass');
QFA		=	quadrupole('QFA'  , 0.448, 2.954337 ,'QuadLinearPass');
QDA		=	quadrupole('QDA'  , 0.187, -1.783113 ,'QuadLinearPass');

% SEXTUPOLES for xix=0.4 and xi_y=1.4
% -52.246310 74.801936 w/o sextu in superbend
% -52.03     75.05     w/  sextu in superbend
SDD   =  sextupole('SDD', 0.2030/2, -52.0333, 'StrMPoleSymplectic4Pass');
SFF   =  sextupole('SFF', 0.2030/2,  75.0479, 'StrMPoleSymplectic4Pass');
...
BEND  =  rbend('BEND'  , 0.86514,  ...
            0.1745329, 0.0872665, 0.0872665, -0.7782,'BendLinearPass');

% Super bend L=255 mm for SBM1 and 254 mm for SBM3 and 4 (cf. field clamps) 
BS    =  rbend('BS', 0.255, ...
            0.1745329, 0.0872665, 0.0872665, 0,'BendLinearPass');
