function alslat_loco_3sb_disp_nuy9_122bpms(varargin)
...
% QUADRUPOLES
QF    =  quadrupole('QF'  , 0.344,  2.664467, 'StrMPoleSymplectic4RadPass');
QD    =  quadrupole('QD'  , 0.187, -2.780814, 'StrMPoleSymplectic4RadPass');
QFA   =  quadrupole('QFA' , 0.448,  2.88625, 'StrMPoleSymplectic4RadPass');
QDA   =  quadrupole('QDA' , 0.187, -1.79739, 'StrMPoleSymplectic4RadPass');


% SEXTUPOLES for xix=0.4 and xi_y=1.4
% -52.246310 74.801936 w/o sextu in superbend
% -52.03     75.05     w/  sextu in superbend
SDD   =  sextupole('SDD', 0.2030/2, -43.216, 'StrMPoleSymplectic4RadPass');
SFF   =  sextupole('SFF', 0.2030/2,  54.7181, 'StrMPoleSymplectic4RadPass');
...
L1s6f  = drift('L1s6f',  CM2CL - ID6CL2Chicane3,        'DriftPass');
...
SUP6  = [L1s6a IDBPM L1s6b CHIC COR L1s6c IDBPM L1s6d IDBPM L1s6e CHIC L1s6f COR L2 BPM L3 QF L4 COR L5 QD  ...
    L6 BPM L7 L8 BEND L9 SDD COR SDD L10 ...
    BPM L11 QFA L12  SFF COR SFF L13 BBPM  ...
    L14 BEND L15 BBPM L16  SFF COR SFF  L17  ...
    QFA L18 BPM L19  SDD COR SDD L20 BEND L21 ...
...
THERING = setcellstruct(THERING, 'K',        QFI([7:8 15:16 23:24]), 2.648276);
THERING = setcellstruct(THERING, 'PolynomB', QFI([7:8 15:16 23:24]), 2.648276,2);

QDI = findcells(THERING,'FamName','QD');
THERING = setcellstruct(THERING, 'K',        QDI([7:8 15:16 23:24]), -2.759828);
THERING = setcellstruct(THERING, 'PolynomB', QDI([7:8 15:16 23:24]), -2.759828,2);

QFAI = findcells(THERING,'FamName','QFA');
THERING = setcellstruct(THERING, 'K',        QFAI([7:8 15:16 23:24]), 3.051577);
THERING = setcellstruct(THERING, 'PolynomB', QFAI([7:8 15:16 23:24]), 3.051577,2);
