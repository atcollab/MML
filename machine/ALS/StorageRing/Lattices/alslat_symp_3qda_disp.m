function alslat_symp_3qda_disp
% Lattice definition file (alslat_symp_3qda_disp)
...
% two sextupole families, BPMs as markers, no correctors, three
% sectors with QDA magnets (broken symmetry), eta_x = 6 cm (straights).

global FAMLIST THERING GLOBVAL
GLOBVAL.E0 = 1.9e9;
GLOBVAL.LatticeFile = 'als_symp_3qda_disp';
FAMLIST = cell(0);
disp(' ');
disp('** Loading ALS lattice in alslat_symp_3qda_disp.m **');
...
QF		=	quadrupole('QF'  , 0.344, 2.19716 ,'StrMPoleSymplectic4RadPass');
QD		=	quadrupole('QD'  , 0.187, -2.24752 ,'StrMPoleSymplectic4RadPass');
QFA		=	quadrupole('QFA'  , 0.448, 2.95601 ,'StrMPoleSymplectic4RadPass');
QDA		=	quadrupole('QDA'  , 0.2, -0.4300 ,'StrMPoleSymplectic4RadPass');
...
SUPS  = [   L1 L2 BPM L3 QF L4 L5 QD  ...
           ...
           L14 BEND L15 BPM LS5 QDA LS1  SF  L17  ...
           QFA L18 BPM L19  SD L20 BEND L21 ... 
           BPM L22 QD L23  L24 QF L25  ...
           BPM L26  L27 ];
...
THERING = setcellstruct(THERING,'K',QFI([7:8 15:16 23:24]),2.25600);
THERING = setcellstruct(THERING,'PolynomB',QFI([7:8 15:16 23:24]),2.25600,2);
QDI = findcells(THERING,'FamName','QD');
THERING = setcellstruct(THERING,'K',QDI([7:8 15:16 23:24]),-2.33285);
THERING = setcellstruct(THERING,'PolynomB',QDI([7:8 15:16 23:24]),-2.33285,2);
QFAI = findcells(THERING,'FamName','QFA');
THERING = setcellstruct(THERING,'K',QFAI([7:8 15:16 23:24]),3.0788);
THERING = setcellstruct(THERING,'PolynomB',QFAI([7:8 15:16 23:24]),3.0788,2);
