function varargout = bpm_psd_plot_newbpm_web(varargin)
% function varargout = bpm_psd_plot_newbpm_web(varargin)
%
...
% BPM data gets scaled and low pass filtered to be comparable to old data
% (IDBPMs have different beta functions than BPMs 2 and 9 and also have 60
% Hz low pass filters.
BPMlist2 = [5 10;7 1];  
IDxnames2 = family2channel('BPMx', BPMlist2);
IDynames2 = family2channel('BPMy', BPMlist2);
...
while 1
    ...
    if state == 5 && ((ffbon == 1) || (ffbon == 2))
        ...
        for loop=1:size(IDxnames,1)
            bpmx(loop,:)=getpvonline([IDxnames(loop,1:end-8) 'wfr:FA:c0'])/1e6*sqrt(22/23);   % scaled to IDBPM location
            bpmy(loop,:)=getpvonline([IDynames(loop,1:end-8) 'wfr:FA:c1'])/1e6*sqrt(5/8);   % scaled to IDBPM location
        end
     ...
     bpmx=filter(b,a,bpmx')';
     bpmy=filter(b,a,bpmy')';
        
     
        loop2 = 1; suppress_inj = 1;
        for loop = 1:size(bpmx,1)
            labelstr=IDxnames2(loop,:);
            ...
            if ~suppress_inj % reject injection transients
                ...
                if loop2==1
                    setpvonline('SR09S_IBPM1X_RMS',sqrt((1e6*Drms).^2-(0.2*sqrt(60/10000))^2));
                elseif loop2==2
                    setpvonline('SR09S_IBPM2X_RMS',sqrt((1e6*Drms).^2-(0.2*sqrt(60/10000))^2));
                end
            end
            loop2=loop2+1;
        end
        ...
        for loop = 1:size(bpmy,1)
            ...
            if ~suppress_inj % reject injection transients
                ...
                if loop2 == 3
                    scaput('SR09S_IBPM1Y_RMS', sqrt((1e6*Drms).^2-0.2^2));
                elseif loop2==4
                    scaput('SR09S_IBPM2Y_RMS', sqrt((1e6*Drms).^2-0.2^2));
                end
            end
    ...
    % Pause for a bit
    fprintf('   Pausing %.1f seconds ... Using new digital BPMs, not FOFB with Bergoz BPMs', PauseBetweenUpdates);
