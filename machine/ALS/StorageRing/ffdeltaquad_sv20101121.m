function [DelQF, DelQD] = ffdeltaquad(Sector, Gap, GeV)
    ...
    if strcmp(SR_Mode, '1.9 GeV, High Tune') ||  strcmp(SR_Mode, '1.9 GeV, Inject at 1.23') || strcmp(SR_Mode, '1.9 GeV, Two-Bunch')
        ...
        elseif strcmp(FFTypeFlag,'Global')
            ...
            if (Sector(i,1) == 4) | (Sector(i,1) == 11)
                % Remove sector 4 and 11 tune correction
                DeltaNuY = 0;
            end
            % if (Sector(i,1) == 4) | (Sector(i,1) == 11)
            %     if Sector(i,:) == [11 1]
            %         modenamestr = sprintf('SR%02dU___ODS1M__DC00',Sector(i,1));
            %         longshift=3;
            %     elseif Sector(i,:) == [11 2]
            %         modenamestr = sprintf('SR%02dU___ODS2M__DC00',Sector(i,1));
            %         longshift=3;
            %     else
            %         modenamestr = sprintf('SR%02dU___ODS1M__DC00',Sector(i,1));
            %         longshift=0;
            %     end
            %     if getsp(modenamestr)==0
            %         % DeltaNuX = shift2tune(Sector,Gap(i,:),getepu(Sector)+longshift)+0.005;
            %         DeltaNuX = 0;   % problem with beam stability (instability/resonance?) with nu_x = 14.25 for EPU shift at 25 mm
            %         DeltaNuY = 0;   % correct vertical tune shift of EPUs is very small
            %     else
            %         DeltaNuX = 0;
            %         DeltaNuY = 0;
            %     end
            ...
            DeltaAmps = inv(TuneResponseMat) * [(fraccorr*6.23e-4)-DeltaNuX; fraccorr*(-0.05301)];    %  DelAmps =  [QF; QD];
%             if Sector(i,1)==5
%                 DeltaAmps
%             end           
            for j = 1:size(Gap,2)
            ...
            DeltaAmpsLocal = 12*inv(TuneResponseMat) * [-DeltaNuX;0];                       %  DelAmps =  [QF; QD];
