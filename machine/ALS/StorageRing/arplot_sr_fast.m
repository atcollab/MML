function arplot_sr_fast(monthStr, days, year1, month2Str, days2, year2)
...
%
% 2007      C.Steier
%        Did some code speedup - Caution: routine has problems if channels
%        get added to or deleted from archiver during the time period
%        plotted
...
for day = days
    ...
    if day==days(1)
        ...
        [B040_PSS002_RSS_TRLY_OPN, iB040_PSS002_RSS_TRLY_OPN] = arselect('B040_PSS002_RSS_TRLY_OPN');
        [B040_PSS001_RSS_TRLY_OPN, iB040_PSS001_RSS_TRLY_OPN] = arselect('B040_PSS001_RSS_TRLY_OPN');
        ...
        [B050_PSS101_OPN, iB050_PSS101_OPN] = arselect('B050_PSS101_OPN');
        [B050_PSS201_OPN, iB050_PSS201_OPN] = arselect('B050_PSS201_OPN');
        [B050_PSS301_OPN, iB050_PSS301_OPN] = arselect('B050_PSS301_OPN');
        [B050_PSS111_OPN, iB050_PSS111_OPN] = arselect('B050_PSS111_OPN');
        [B050_PSS211_OPN, iB050_PSS211_OPN] = arselect('B050_PSS211_OPN');
        [B050_PSS311_OPN, iB050_PSS311_OPN] = arselect('B050_PSS311_OPN');
        ...
        dcct = [dcct ARData(idcct,:)];
        lcw  = [lcw  ARData(ilcw,:)];
        i_cam = [i_cam ARData(ii_cam,:)];
        gev = [gev ARData(igev,:)];
        lifetime = [lifetime ARData(ilifetime,:)];

        % EPU
        EPU =[EPU  ARData(iEPU,:)];

        % ID vertical gap
        IDgap = [IDgap  ARData(iIDgap,:)];

        if FFFlag
            GapEnable = [GapEnable ARData(iGapEnable,:)];
            FF7Enable = [FF7Enable ARData(iFF7Enable,:)];
        end

        TC7_1=[TC7_1 ARData(iTC7_1,:)];
        TC7_2=[TC7_2 ARData(iTC7_2,:)];
        TC7_3=[TC7_3 ARData(iTC7_3,:)];
        TC7_4=[TC7_4 ARData(iTC7_4,:)];

        TC8_1=[TC8_1 ARData(iTC8_1,:)];
        TC8_2=[TC8_2 ARData(iTC8_2,:)];
        TC8_3=[TC8_3 ARData(iTC8_3,:)];
        TC8_4=[TC8_4 ARData(iTC8_4,:)];

        TC9_1=[TC9_1 ARData(iTC9_1,:)];
        TC9_2=[TC9_2 ARData(iTC9_2,:)];
        TC9_3=[TC9_3 ARData(iTC9_3,:)];
        TC9_4=[TC9_4 ARData(iTC9_4,:)];

        TC10_1=[TC10_1 ARData(iTC10_1,:)];
        TC10_2=[TC10_2 ARData(iTC10_2,:)];
        TC10_3=[TC10_3 ARData(iTC10_3,:)];
        TC10_4=[TC10_4 ARData(iTC10_4,:)];

        TC12_1=[TC12_1 ARData(iTC12_1,:)];
        TC12_2=[TC12_2 ARData(iTC12_2,:)];
        TC12_3=[TC12_3 ARData(iTC12_3,:)];
        TC12_4=[TC12_4 ARData(iTC12_4,:)];

        TCSR01=[TCSR01 ARData(iTCSR01,:)];
        TCSR02=[TCSR02 ARData(iTCSR02,:)];
        TCSR03=[TCSR03 ARData(iTCSR03,:)];
        TCSR04=[TCSR04 ARData(iTCSR04,:)];
        TCSR05=[TCSR05 ARData(iTCSR05,:)];
        TCSR06=[TCSR06 ARData(iTCSR06,:)];
        TCSR07=[TCSR07 ARData(iTCSR07,:)];
        TCSR08=[TCSR08 ARData(iTCSR08,:)];
        TCSR09=[TCSR09 ARData(iTCSR09,:)];
        TCSR11=[TCSR11 ARData(iTCSR11,:)];
        TCSR12=[TCSR12 ARData(iTCSR12,:)];
        TCSRAVG=[TCSRAVG ARData(iTCSRAVG,:)];

        AHUT01=[AHUT01 ARData(iAHUT01,:)];
        AHUT03=[AHUT03 ARData(iAHUT03,:)];
        AHUT04=[AHUT04 ARData(iAHUT04,:)];
        AHUT06=[AHUT06 ARData(iAHUT06,:)];
        AHUT07=[AHUT07 ARData(iAHUT07,:)];
        AHUT08=[AHUT08 ARData(iAHUT08,:)];
        AHUT09=[AHUT09 ARData(iAHUT09,:)];
        AHUT11=[AHUT11 ARData(iAHUT11,:)];

        WVSRM=[WVSRM ARData(iWVSRM,:)];

        CWTemp=[CWTemp ARData(iCWTemp,:)];
        CWValve=[CWValve ARData(iCWValve,:)];

        RingLCWSupPx=[RingLCWSupPx ARData(iRingLCWSupPx,:)];
        RingLCWRetPx=[RingLCWRetPx ARData(iRingLCWRetPx,:)];
        LCWSupPx=[LCWSupPx ARData(iLCWSupPx,:)];
        LCWRetPx=[LCWRetPx ARData(iLCWRetPx,:)];
        LCWSupTemp=[LCWSupTemp ARData(iLCWSupTemp,:)];
        LCWRetTemp=[LCWRetTemp ARData(iLCWRetTemp,:)];
        WBasinTemp=[WBasinTemp ARData(iWBasinTemp,:)];
        EBasinTemp=[EBasinTemp ARData(iEBasinTemp,:)];

        % All Bergoz BPMs
        BPMx = [BPMx ARData(iBPMx,:)];
        BPMy = [BPMy ARData(iBPMy,:)];

        ID9BPM1xRMS = [ID9BPM1xRMS ARData(iID9BPM1xRMS,:)];
        ID9BPM1yRMS = [ID9BPM1yRMS ARData(iID9BPM1yRMS,:)];
        ID9BPM2xRMS = [ID9BPM2xRMS ARData(iID9BPM2xRMS,:)];
        ID9BPM2yRMS = [ID9BPM2yRMS ARData(iID9BPM2yRMS,:)];

        C1TEMP=[C1TEMP ARData(iC1TEMP,:)];
        C2TEMP=[C2TEMP ARData(iC2TEMP,:)];

        SigX=[SigX ARData(iSigX,:)];
        SigY=[SigY ARData(iSigY,:)];

        if ~isempty(iSigX31)
            SigX31=[SigX31 ARData(iSigX31,:)];
        else
            SigX31 = [SigX31 NaN * ones(1,length(ARt))];
        end

        if ~isempty(iSigY31)
            SigY31=[SigY31 ARData(iSigY31,:)];
        else
            SigY31 = [SigY31 NaN * ones(1,length(ARt))];
        end

        if ~isempty(iSigX72)
            SigX72=[SigX72 ARData(iSigX72,:)];
        else
            SigX72 = [SigX72 NaN * ones(1,length(ARt))];
        end

        if ~isempty(iSigY72)
            SigY72=[SigY72 ARData(iSigY72,:)];
        else
            SigY72 = [SigY72 NaN * ones(1,length(ARt))];
        end

        SpiroX=[SpiroX ARData(iSpiroX,:)];
        SpiroY=[SpiroY ARData(iSpiroY,:)];

        HPcounter=[HPcounter ARData(iHPcounter,:)];
        HPsyn=[HPsyn ARData(iHPsyn,:)];
        hqmofm=[hqmofm 4.988e3*ARData(ihqmofm,:)];

        QFAam=[QFAam ARData(iQFAam,:)];

        % BR RF parameters
        Power_RF_fwd=[Power_RF_fwd ARData(iPower_RF_fwd,:)];
        Power_RF_ref=[Power_RF_ref ARData(iPower_RF_ref,:)];
        Power_drive=[Power_drive ARData(iPower_drive,:)];
        HV_setpoint=[HV_setpoint ARData(iHV_setpoint,:)];
        Beam_volts=[Beam_volts ARData(iBeam_volts,:)];
        Beam_current=[Beam_current ARData(iBeam_current,:)];
        Bias_volts=[Bias_volts ARData(iBias_volts,:)];
        Bias_current=[Bias_current ARData(iBias_current,:)];
        Heater_Voltage=[Heater_Voltage ARData(iHeater_Voltage,:)];
        Heater_current=[Heater_current ARData(iHeater_current,:)];
        Focus_voltage=[Focus_voltage ARData(iFocus_voltage,:)];
        Focus_current=[Focus_current ARData(iFocus_current,:)];
        VacIon_voltage=[VacIon_voltage ARData(iVacIon_voltage,:)];
        VacIon_current=[VacIon_current ARData(iVacIon_current,:)];
        Thy_Grid1_volts=[Thy_Grid1_volts ARData(iThy_Grid1_volts,:)];
        Thy_Grid2_volts=[Thy_Grid2_volts ARData(iThy_Grid2_volts,:)];
        Thy_heater_volts=[Thy_heater_volts ARData(iThy_heater_volts,:)];
        Thy_heater_current=[Thy_heater_current ARData(iThy_heater_current,:)];
        Body_Current=[Body_Current ARData(iBody_Current,:)];
        Temp_HV_enclose=[Temp_HV_enclose ARData(iTemp_HV_enclose,:)];
        Temp_IOT_enclose=[Temp_IOT_enclose ARData(iTemp_IOT_enclose,:)];
        Temp_IOT_cooling=[Temp_IOT_cooling ARData(iTemp_IOT_cooling,:)];
        Temp_ceramic_output=[Temp_ceramic_output ARData(iTemp_ceramic_output,:)];
        BR_CLDFWD=[BR_CLDFWD ARData(iBR_CLDFWD,:)];
        BR_WGREV=[BR_WGREV ARData(iBR_WGREV,:)];
        BR_WGFWD=[BR_WGFWD ARData(iBR_WGFWD,:)];
        BR_RFCONT=[BR_RFCONT ARData(iBR_RFCONT,:)];
        BR_PHSCON_AM=[BR_PHSCON_AM ARData(iBR_PHSCON_AM,:)];
        BR_PHSCON_AC=[BR_PHSCON_AC ARData(iBR_PHSCON_AC,:)];
        BR_TUNPOS=[BR_TUNPOS ARData(iBR_TUNPOS,:)];
        BR_TUNERR=[BR_TUNERR ARData(iBR_TUNERR,:)];
        BR_CAVTMP=[BR_CAVTMP ARData(iBR_CAVTMP,:)];
        BR_LCWTMP=[BR_LCWTMP ARData(iBR_LCWTMP,:)];
        BR_CLDFLW=[BR_CLDFLW ARData(iBR_CLDFLW,:)];
        BR_CIRFLW=[BR_CIRFLW ARData(iBR_CIRFLW,:)];
        BR_SLDTMP=[BR_SLDTMP ARData(iBR_SLDTMP,:)];
        BR_CIRTMP=[BR_CIRTMP ARData(iBR_CIRTMP,:)];
        BR_SLDFLW=[BR_SLDFLW ARData(iBR_SLDFLW,:)];
        BR_CLDTMP=[BR_CLDTMP ARData(iBR_CLDTMP,:)];
        BR_CAVFLW=[BR_CAVFLW ARData(iBR_CAVFLW,:)];
        BR_WINFLW=[BR_WINFLW ARData(iBR_WINFLW,:)];
        BR_TUNFLW=[BR_TUNFLW ARData(iBR_TUNFLW,:)];

        % Beamline shutters
        if ~isempty(iB021_PSS201_OPN)
            B021_PSS201_OPN=[B021_PSS201_OPN ARData(iB021_PSS201_OPN,:)];
        else
            B021_PSS201_OPN = [B021_PSS201_OPN NaN * ones(1,length(ARt))];
        end
        B03_11_PSS1_RSS_TRLY_OPN=[B03_11_PSS1_RSS_TRLY_OPN ARData(iB03_11_PSS1_RSS_TRLY_OPN,:)];
        B03_2_PSS101_RSS_TRLY_OPN=[B03_2_PSS101_RSS_TRLY_OPN ARData(iB03_2_PSS101_RSS_TRLY_OPN,:)];
        B03_2_PSS201_RSS_TRLY_OPN=[B03_2_PSS201_RSS_TRLY_OPN ARData(iB03_2_PSS201_RSS_TRLY_OPN,:)];
        B03_3_PSS101_RSS_TRLY_OPN=[B03_3_PSS101_RSS_TRLY_OPN ARData(iB03_3_PSS101_RSS_TRLY_OPN,:)];
        B03_3_PSS201_RSS_TRLY_OPN=[B03_3_PSS201_RSS_TRLY_OPN ARData(iB03_3_PSS201_RSS_TRLY_OPN,:)];
        B040_PSS002_RSS_TRLY_OPN=[B040_PSS002_RSS_TRLY_OPN ARData(iB040_PSS002_RSS_TRLY_OPN,:)];
        B040_PSS001_RSS_TRLY_OPN=[B040_PSS001_RSS_TRLY_OPN ARData(iB040_PSS001_RSS_TRLY_OPN,:)];
        ICS_BL042_PSS101_RSS_TRLY_OP=[ICS_BL042_PSS101_RSS_TRLY_OP ARData(iICS_BL042_PSS101_RSS_TRLY_OP,:)];
        ICS_BL042_PSS201_RSS_TRLY_OP=[ICS_BL042_PSS201_RSS_TRLY_OP ARData(iICS_BL042_PSS201_RSS_TRLY_OP,:)];
        ICS_BL043_PSS101_RSS_TRLY_OP=[ICS_BL043_PSS101_RSS_TRLY_OP ARData(iICS_BL043_PSS101_RSS_TRLY_OP,:)];
        ICS_BL043_PSS201_RSS_TRLY_OP=[ICS_BL043_PSS201_RSS_TRLY_OP ARData(iICS_BL043_PSS201_RSS_TRLY_OP,:)];
        B050_PSS001_RSS_TRLY_OPN=[B050_PSS001_RSS_TRLY_OPN ARData(iB050_PSS001_RSS_TRLY_OPN,:)];
        B050_PSS101_OPN=[B050_PSS101_OPN ARData(iB050_PSS101_OPN,:)];
        B050_PSS201_OPN=[B050_PSS201_OPN ARData(iB050_PSS201_OPN,:)];
        B050_PSS301_OPN=[B050_PSS301_OPN ARData(iB050_PSS301_OPN,:)];
        B050_PSS111_OPN=[B050_PSS111_OPN ARData(iB050_PSS111_OPN,:)];
        B050_PSS211_OPN=[B050_PSS211_OPN ARData(iB050_PSS211_OPN,:)];
        B050_PSS311_OPN=[B050_PSS311_OPN ARData(iB050_PSS311_OPN,:)];
        B053_PSS101_RSS_TRLY_OPN=[B053_PSS101_RSS_TRLY_OPN ARData(iB053_PSS101_RSS_TRLY_OPN,:)];
        B053_PSS201_RSS_TRLY_OPN=[B053_PSS201_RSS_TRLY_OPN ARData(iB053_PSS201_RSS_TRLY_OPN,:)];
        B060_PSS101_OPN=[B060_PSS101_OPN ARData(iB060_PSS101_OPN,:)];
        B0612_PSS1_OPN=[B0612_PSS1_OPN ARData(iB0612_PSS1_OPN,:)];
        B0631_PSS1_OPN=[B0631_PSS1_OPN ARData(iB0631_PSS1_OPN,:)];
        B0632_PSS1_OPN=[B0632_PSS1_OPN ARData(iB0632_PSS1_OPN,:)];
        B0701_PSS1_RSS_TRLY_OPN=[B0701_PSS1_RSS_TRLY_OPN ARData(iB0701_PSS1_RSS_TRLY_OPN,:)];
        B073_PSS1_RSS_TRLY_OPN=[B073_PSS1_RSS_TRLY_OPN ARData(iB073_PSS1_RSS_TRLY_OPN,:)];
        B0733_PSS1_OPN=[B0733_PSS1_OPN ARData(iB0733_PSS1_OPN,:)];
        ICS_B0801_PSS1_RSS_TRLY_OPN=[ICS_B0801_PSS1_RSS_TRLY_OPN ARData(iICS_B0801_PSS1_RSS_TRLY_OPN,:)];
        ICS_B082_PSS101_RSS_TRLY_OP=[ICS_B082_PSS101_RSS_TRLY_OP ARData(iICS_B082_PSS101_RSS_TRLY_OP,:)];
        ICS_B082_PSS201_RSS_TRLY_OP=[ICS_B082_PSS201_RSS_TRLY_OP ARData(iICS_B082_PSS201_RSS_TRLY_OP,:)];
        ICS_B083_PSS101_RSS_TRLY_OP=[ICS_B083_PSS101_RSS_TRLY_OP ARData(iICS_B083_PSS101_RSS_TRLY_OP,:)];
        ICS_B083_PSS201_RSS_TRLY_OP=[ICS_B083_PSS201_RSS_TRLY_OP ARData(iICS_B083_PSS201_RSS_TRLY_OP,:)];
        B090_PSS1_RSS_TRLY_OPN=[B090_PSS1_RSS_TRLY_OPN ARData(iB090_PSS1_RSS_TRLY_OPN,:)];
        B09_31_PSS1_RSS_TRLY_OPN=[B09_31_PSS1_RSS_TRLY_OPN ARData(iB09_31_PSS1_RSS_TRLY_OPN,:)];
        B09_3_PSS1_RSS_TRLY_OPN=[B09_3_PSS1_RSS_TRLY_OPN ARData(iB09_3_PSS1_RSS_TRLY_OPN,:)];
        B10_0_PSS001_RSS_TRLY_OPN=[B10_0_PSS001_RSS_TRLY_OPN ARData(iB10_0_PSS001_RSS_TRLY_OPN,:)];
        B10_31_PSS1_RSS_TRLY_OPN=[B10_31_PSS1_RSS_TRLY_OPN ARData(iB10_31_PSS1_RSS_TRLY_OPN,:)];
        B110_PSS101_RSS_TRLY_OPN=[B110_PSS101_RSS_TRLY_OPN ARData(iB110_PSS101_RSS_TRLY_OPN,:)];
        B110_PSS201_RSS_TRLY_OPN=[B110_PSS201_RSS_TRLY_OPN ARData(iB110_PSS201_RSS_TRLY_OPN,:)];
        BL113_PSS101_RSS_TRLY_OPN=[BL113_PSS101_RSS_TRLY_OPN ARData(iBL113_PSS101_RSS_TRLY_OPN,:)];
        BL113_PSS201_RSS_TRLY_OPN=[BL113_PSS201_RSS_TRLY_OPN ARData(iBL113_PSS201_RSS_TRLY_OPN,:)];
        ICS_BL12_0_PSS1_RSS_TRLY_OPN=[ICS_BL12_0_PSS1_RSS_TRLY_OPN ARData(iICS_BL12_0_PSS1_RSS_TRLY_OPN,:)];
        ICS_BL12_2_PSS101_RSS_TRLY_O=[ICS_BL12_2_PSS101_RSS_TRLY_O ARData(iICS_BL12_2_PSS101_RSS_TRLY_O,:)];
        ICS_BL12_2_PSS201_RSS_TRLY_O=[ICS_BL12_2_PSS201_RSS_TRLY_O ARData(iICS_BL12_2_PSS201_RSS_TRLY_O,:)];
        ICS_BL12_3_PSS101_RSS_TRLY_O=[ICS_BL12_3_PSS101_RSS_TRLY_O ARData(iICS_BL12_3_PSS101_RSS_TRLY_O,:)];
        ICS_BL12_3_PSS201_RSS_TRLY_O=[ICS_BL12_3_PSS201_RSS_TRLY_O ARData(iICS_BL12_3_PSS201_RSS_TRLY_O,:)];
        B122_PSS111_OPN=[B122_PSS111_OPN ARData(iB122_PSS111_OPN,:)];
        B122_PSS211_OPN=[B122_PSS211_OPN ARData(iB122_PSS211_OPN,:)];
        B123_PSS111_OPN=[B123_PSS111_OPN ARData(iB123_PSS111_OPN,:)];
        B123_PSS211_OPN=[B123_PSS211_OPN ARData(iB123_PSS211_OPN,:)];
...
FigNum = 0;
FigNum = FigNum + 2; %+2 just to protect srcontrol window if it is open
h = figure(FigNum);
subfig(1,1,1, h);
%%p = get(h, 'Position');
...
axes(ax(1));
set(ax(1),'YTick',[0 100 200 300 400]);
axis([0 xmax 0 450]);
axes(ax(2));
axis([0 xmax 0 20]);
grid on;
...
YLim1 = get(ax(1),'YLim');
YLim2 = get(ax(2),'YLim');
set(ax(2),'YTick',round([0 100 200 300 400]*YLim2(2)/YLim1(2)*10)/10);
...
subplot(7,1,3);
[ax, h1, h2] = plotyy(t,lifetime, t(8:(end-8)),lifetime2);
grid on
...
subplot(7,1,5);
plot(t, dcct .* lifetime ./ (SigX31) ./ (SigY31), 'b');
grid on;
...
subplot(2,1,2);
[ax, h1, h2] = plotyy(t, HPcounter, t, hqmofm);
