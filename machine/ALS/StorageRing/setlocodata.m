...
switch lower(CommandInput)
    ...
    case 'setmodel'
        ...
        if abs(FitParameters(end).Values(59)+0.7782)<0.01
    ...
    case 'symmetrize'
        ...
        elseif length(FitParameters(end).Values)==82
            ...
            QFAloco  = FitParameters(end).Values([49 50 51 52]);
            QDAloco  = FitParameters(end).Values([53 54 55 56 57 58]);
            BENDloco = getpvmodel('BEND','K',[1 1],'Physics');
            
            QFloco0   = FitParameters(1).Values(1:24);
            QDloco0   = FitParameters(1).Values(25:48);
            QFAloco0  = FitParameters(1).Values([49 50 51 52]);
            QDAloco0  = FitParameters(1).Values([53 54 55 56 57 58]);
            BENDloco0 = getpvmodel('BEND','K',[1 1],'Physics');
            SQSFloco = FitParameters(end).Values(59:69);
            SQSDloco = FitParameters(end).Values(70:82);

            SQSFloco0 = FitParameters(1).Values(59:69);
            SQSDloco0 = FitParameters(1).Values(70:82);
        elseif length(FitParameters(end).Values)==103
            ...
            QDAloco0  = FitParameters(1).Values([53 54 55 56 57 58]);
            BENDloco0 = getpvmodel('BEND','K',[1 1],'Physics');
            if length(FitParameters(end).Values) >= 103
                SQSFloco = FitParameters(end).Values(59:68);
                SQSFloco = [0;SQSFloco];
                SQSDloco = FitParameters(end).Values(69:81);
                SQSHFloco = FitParameters(end).Values(82:103);

                SQSFloco0 = FitParameters(1).Values(59:68);
                SQSFloco0 = [0;SQSFloco0];
                SQSDloco0 = FitParameters(1).Values(69:81);
                SQSHFloco0 = FitParameters(1).Values(82:103);
            end
        elseif length(FitParameters(end).Values)==104
            ...
            if length(FitParameters(end).Values) >= 103
                SQSFloco = FitParameters(end).Values(59:69);
                SQSDloco = FitParameters(end).Values(70:82);
                SQSHFloco = FitParameters(end).Values(83:104);

                SQSFloco0 = FitParameters(1).Values(59:69);
                SQSDloco0 = FitParameters(1).Values(70:82);
                SQSHFloco0 = FitParameters(1).Values(83:104);
            end
        elseif length(FitParameters(end).Values)==114
            % Magnet fits
            QFloco   = FitParameters(end).Values(1:24);
            QDloco   = FitParameters(end).Values(25:48);
            QFAloco  = FitParameters(end).Values([49 50 51 52]);
            ...
            QDAloco0  = FitParameters(1).Values([53 54 55 56 57 58]);
            BENDloco0 = getpvmodel('BEND','K',[1 1],'Physics');
            if length(FitParameters(end).Values) == 114
                SQSFloco = FitParameters(end).Values(59:74);
                SQSDloco = FitParameters(end).Values(75:90);
                SQSHFloco = FitParameters(end).Values(91:114);

                SQSFloco0 = FitParameters(1).Values(59:74);
                SQSDloco0 = FitParameters(1).Values(75:90);
                SQSHFloco0 = FitParameters(1).Values(91:114);
            end
        else
            % Magnet fits
            QFloco   = FitParameters(end).Values(1:24);
        ...
        switch ModeNumber
            ...
            case 2       % Use table lookup of fit values (Christoph Method)
                ModeCell = {'Pseudo-Single Bunch (0.18,0.25)','6.0 cm dispersion (distributed)','nux=16.25 (Low Emittance)'};
                [DispFlag, OKFlag] = listdlg('Name','ALS - setlocodata','PromptString','Which Lattice (all are nuy=9.2)?', ...
                    'SelectionMode','single', 'ListString', ModeCell, 'ListSize', [250 80], 'InitialValue', 3);
                if OKFlag ~= 1
                    fprintf('   setlocodata canceled\n');
                    return
                end
               ...
               if DispFlag == 3
                    disp('   Using k-values for lowe emittance lattice, nux = 16.25');
                    kQF=	[2.311431		2.310217	 2.309003];
                    kQFS=	[2.322313    2.321090	 2.319867];
                    kQD=	[-2.690886	   -2.681751	 -2.67261];
                    kQDS=	[-2.570421	   -2.561542	-2.567594];
                    kQFA=	[3.223592	    3.223830	3.224068];
                    kQFAS=	[3.2495653	 3.250126	3.2506867];
                    kQDA=	[-0.3627096	   -0.3656653	-0.368621];
                    kBEND=	[-0.7765	-0.7782		-0.7799];
               ...
               elseif DispFlag == 2
                ...
                elseif DispFlag == 1
                    disp('   Using k-values for Pseudo-Single Bunch (0.18,0.25)');
                    kQDS = -2.575871E+00 ;
                    kQFS =  2.321139E+00 ;
                    kQDA = -3.966009E-01 ;
                    kQFA =  3.249987E+00 ;
                    ...
                    kQFA =  3.217611E+00 ;
                    kBEND= -0.778200E+00 ;
                    QFloco0 = kQF*ones(24,1);
                    QFloco0([7 8 15 16 23 24])=kQFS;
                    QDloco0 = kQD*ones(24,1);
                    QDloco0([7 8 15 16 23 24])=kQDS;
                    QFAloco0 = kQFA*ones(24,1);
                    QFAloco0([7 8 15 16 23 24])=kQFAS;
                    QDAloco0 = kQDA*ones(6,1);
                else
               ...
               if DispFlag ~= 1
                   ...
                   QFgoal([1:6 9:14 17:22],1)  = kQF;
                   QFgoal([7:8 15:16 23:24],1) = kQFS;
                   QDgoal([1:6 9:14 17:22],1)  = kQD;
                   QDgoal([7:8 15:16 23:24],1) = kQDS;
                   QDAgoal([1:6],1)            = kQDA;
                   QFAgoal(1,1)                = kQFA;
                   if length(FitParameters(end).Values) == 81
                       QFAgoal([2],1)          = kQFAS;
                   else
                       QFAgoal([2 3 4],1)          = kQFAS;
                   end
               end
            ...
            case 1      % Compute a New AT Fit
                ...
                elseif strcmpi(getfamilydata('OperationalMode'), '1.9 GeV, Low Tune')
                    ...
                    MeritFun = @alsfitnuy9;
                elseif strcmpi(getfamilydata('OperationalMode'), '1.9 GeV, Low Emittance Mode')
                    YGoalInput = {'16.235/4-0.035';'16.235/3';'9.20/3';'0.0';'0.0';'0.150';'0.0'};
                    MeritFun = @alsfitnuy9;
                elseif strcmpi(getfamilydata('OperationalMode'), 'Pseudo-Single Bunch (0.18,0.25)')
                    YGoalInput = {'16.155/4-0.0362';'16.155/3';'9.25/3';'0.0';'0.0';'0.150';'0.0'};
                    MeritFun = @alsfitnuy9;
                elseif strcmpi(getfamilydata('OperationalMode'), '1.9 GeV, Two-Bunch')
                    YGoalInput = {'16.165/4-0.0362';'16.165/3';'9.25/3';'0.0';'0.0';'0.150';'0.0'};
                    MeritFun = @alsfitnuy9;
                else
                ...
                if length(FitParameters(end).Values) > 114
                    ...
                    if any(SQSHFloco0~=0)
                        fprintf('   Warning: the start point in the model for SQSHF is not zero.\n');
                    end

                    disp('Setting SQSF, SQSD, SQSHF to fitted values');
                    setsp('SQSF', SQSFloco0, 'Physics', 'Model');
                    setsp('SQSD', SQSDloco0, 'Physics', 'Model');
                    setsp('SQSHF', SQSHFloco0, 'Physics', 'Model');
                else
                    setsp('SQSF', 0, 'Physics', 'Model');
                    setsp('SQSD', 0, 'Physics', 'Model');
                    setsp('SQSHF', 0, 'Physics', 'Model');
                end
...
%                 QDAnew = QDAnew * x(7) / x0(7);
    ...
    case 'correctcoupling'
        ...
        elseif length(FitParameters(end).Values) > 83
            fprintf('   There are more than 83 fit parameters, hopefully the skew quads fits are 59:114\n');
            fprintf('   This functions might not work for all other combinations of skew quad numbers!\n');
        ...
        elseif length(FitParameters(end).Values) ==114
            SQSFcoupling = FitParameters(end).Values(59:74);
            SQSDcoupling = FitParameters(end).Values(75:90);
            SQSHFcoupling = [ FitParameters(end).Values(113);FitParameters(end).Values(91:112);FitParameters(end).Values(114)];
        else
