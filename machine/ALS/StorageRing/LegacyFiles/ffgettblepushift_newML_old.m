function ffgettbl_newML(Sector, BPMFlag)
...
disp('                    6.  Bumps off and BTS 3 and 4 set to nominal current.');
...
ModeIn = menu(str2mat(sprintf('%.1f GeV Longitudinal Table',GeV),sprintf('EPU, Sector %d %d            ',Sector(1), Sector(2)),' ','Start table generation?'),'parallel mode','anti-parallel mode','Cancel');   
if ModeIn == 1
    EPUmode = 0;
...
minute = tmp(5);
seconds= tmp(6);
matfn1  = sprintf('epu%02d%01dm%01de%.0f', Sector(1), Sector(2), EPUmode, 10*getenergy);
matfn2  = sprintf('epu%02d%01dm%01de%.0f_%4d-%02d-%02d', Sector(1), Sector(2), EPUmode, 10*getenergy, year, month, day);
textfn1 = sprintf('epu%02d%01dm%01de%.0f.txt',  Sector(1), Sector(2), EPUmode, 10*getenergy);
textfn2 = sprintf('epu%02d%01dm%01de%.0f_%4d-%02d-%02d.txt',  Sector(1), Sector(2), EPUmode, 10*getenergy, year, month, day);
...
if (Sector(1)==4 || Sector(1)==6 || Sector(1) ==11) && Sector(2)==1
...
elseif (Sector(1)==4 || Sector(1)==6 || Sector(1)==11) && Sector(2)==2
...
Gaps = [210 150 100 70 50 40 35 30 27 24 21 19 17 16 GAPmin];
...
for i = 1:length(Gaps)
    ...
    for j = 1:length(GapsLongitudinal)
        ...
        if GapsLongitudinal(j) == 0
            setsp('HCM',HCM0,HCMList);
            setsp('VCM',VCM0,VCMList);
            pause(5);
...
FileName = sprintf('epu%02dd%ie%.0f', Sector(1), Sector(2), 10*getenergy);
...
fprintf(fid,'# EPU[%i %i] %.1f GeV %02.0f-%02.0f-%.0f %02.0f:%02.0f:%02.0f\n', Sector(1), Sector(2), 10*getenergy, month, day, year, hour, minute, seconds);
...
% disp(['  below 10 microns (see Fig. 2), however it is probably OK to accept a table with up to 25 microns of error.']);
% disp(['           Figure ', num2str(FigureHandles(1)),'  ->  Corrector strength verses gap position.']);
% disp(['           Figure ', num2str(FigureHandles(2)),'  ->  RMS orbit distortion verses gap position.']);
% disp(['           Figure ', num2str(FigureHandles(3)),'  ->  Orbit drift during table generation verses BPM position.']);
