...
if nargin < 1
    SectorIn = menu(str2mat(sprintf('%.1f GeV Feed Forward Generation',IDFF.GeV),'Feed forward must be off!',' ','Which insertion device?'),'4-1 vertical','4-1 longitudinal','4-2 vertical','4-2 longitudinal','5','6','7','8','9','10','11-1 vertical','11-1 longitudinal','11-2 vertical','11-2 longitudinal','12','Cancel');
    if SectorIn == 1
        % Sector 4-1 vertical using new feedforward method
        Sector = [4 1];
    elseif SectorIn == 2
        % Sector 4-1 longitudinal using new feedforward method
        Sector = 4;
        fprintf('ffgettblepushift_newML not ready yet!  Exiting.\n\n');
        %ffgettblepushift_newML([Sector 1]);
        return
    elseif SectorIn == 3
        % Sector 4-2 vertical using new feedforward method
        Sector = [4 2];
    elseif SectorIn == 4
        % Sector 4-2 longitudinal using new feedforward method
        Sector = 4;
        fprintf('ffgettblepushift_newML not ready yet!  Exiting.\n\n');
        %ffgettblepushift_newML([Sector 2]);
        return
    elseif SectorIn == 5
        Sector = [5 1];
    elseif SectorIn == 6
