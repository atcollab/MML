function  [OrbitSTD, IterOut] = setbpm_physics(CMfamily, Ygoal, Xelem, Yelem, In5, In6, In7, In8, In9, In10)
%  [OrbitSTD, IterOut] = setbpm_physics(CMfamily, BPMgoal, CMlist, BPMlist, Iterations, Tolerance);
%     or
%  [OrbitSTD, IterOut] = setbpm+physics(CMfamily1, BPMgoal1, CMlist1, BPMlist1, ...
...
if CMfamily == 'HCM'
    S = getrespmat('BPMx',Yelem, CMfamily, Xelem,'Physics');
elseif CMfamily == 'VCM'
    S = getrespmat('BPMy',Yelem, CMfamily, Xelem,'Physics');
else
...
if BumpFlag == 2
    ...
    if CMfamily2 == 'HCM'
        S2 = getrespmat('BPMx',Yelem2, CMfamily2, Xelem2,'Physics');
    elseif CMfamily2 == 'VCM'
        S2 = getrespmat('BPMy',Yelem2, CMfamily2, Xelem2,'Physics');
    else
        error('SETBPM:  Magnet family must be VCM or HCM!');
    end
...
if BumpFlag == 1
    ...
    for i=1:Iter
        pause(ExtraDelay);
        if Dim==1
            Y = getpv('BPMx', Yelem,'Physics');
        else
            Y = getpv('BPMy', Yelem,'Physics');
        end

        % Check tolerance
        OrbitSTD = std(Ygoal-Y)*1e3;
        ...
        if Dim==1
            DeltaCM=physics2hw('HCM','Setpoint',DeltaCM,Xelem);
        else
            DeltaCM=physics2hw('VCM','Setpoint',DeltaCM,Xelem);
        end
    ...
    for i=1:Iter
        ...
        if Dim2==1
            Y2 = getpv('BPMx', Yelem2,'Physics');
        else
            Y2 = getpv('BPMy', Yelem2,'Physics');
        end

        % Check tolerance
        OrbitSTD(1,1) = std(Ygoal-Y)*1e3;
        %OrbitMAX=max(abs(Ygoal-Y)) ;
        OrbitSTD(2,1) = std(Ygoal2-Y2)*1e3;
        ...
        if Dim2==1
            DeltaCM2=physics2hw('HCM','Setpoint',DeltaCM2,Xelem2);
        else
            DeltaCM2=physics2hw('VCM','Setpoint',DeltaCM2,Xelem2);
        end
