function arplot_sbm_and_waterchem(monthStr, days, year1, month2Str, days2, year2)
% arplot_sbm_and_waterchem(Month1 String, Days1, Year1, Month2 String, Days2, Year2)
%
% Plots archived data about all 3 Superbends and LCW water chemistry.
%
% Example:  arplot_sbm_and_waterchem('January',22:24, 1998);
%           plots data from 1/22, 1/23, and 1 in 1998
%
% C. Steier, August 2001

% 2002-08-15 T.Scarvie - Added cryo-liquid pressure plots
% 2016-11-21 T.Scarvie - Added water chemistry plot so we can look at longterm plots at Ops.Critique meeting

FigNum = 33; %figure;  % Must be next figure after arplot_sr (if publishing to the web)
...
subplot(6,1,1)
plot(t,sbm1i);
legend('I power supply', 'Location','Best');
ylabel('I [A]');
grid;
...
legend('LN level','LHe level','Location','Best');
...
legend('LN pressure','LHe pressure','Location','Best');
...
legend('cryo pressure','Location','Best');
...
% Hall probes have been broken for some time and are not necessary. 2017-04-14, T.Scarvie
% FigNum = FigNum + 1;
% h = figure(FigNum);
% subfig(1,1,1, h);
% %%p = get(h, 'Position');
...
subplot(2,1,1)
plot(t,bl831_hall);
legend('TopOff Hall probe - Inboard [T]', 'TopOff Hall probe - Inboard Raw [T]', 'TopOff Hall probe - Outboard [T]', 'TopOff Hall probe - Outboard Raw [T]');
