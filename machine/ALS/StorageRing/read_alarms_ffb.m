function [hcmlow,hcmhigh,HCMlist,vcmlow,vcmhigh,VCMlist,bpmxlow,bpmxhigh,BPMlist,bpmylow,bpmyhigh]=read_alarms_ffb
% function [hcmlow,hcmhigh,HCMlist,vcmlow,vcmhigh,VCMlist,bpmxlow,bpmxhigh,BPMlist,bpmylow,bpmyhigh]=read_alarms_ffb
%
% This routine reads the corrector magnet and IDBPM safety trip points, which are used
...
for loop=1:size(HCMlist,1)
   ...
   hcmlow(loop)=getpv(paramname2);
   paramname2=sprintf('%s.HIGH',paramname);
   hcmhigh(loop)=getpv(paramname2);
end
for loop=1:size(VCMlist,1)
   paramname=getname('VCM','Trim',VCMlist(loop,:),1);
   paramname2=sprintf('%s.LOW',paramname);
   vcmlow(loop)=getpv(paramname2);
   paramname2=sprintf('%s.HIGH',paramname);
   vcmhigh(loop)=getpv(paramname2);
end
...
for loop=1:size(BPMlist,1)
   ...
   paramname2=sprintf('%s.LOW',paramname);
   bpmxlow(loop)=getpv(paramname2);
   paramname2=sprintf('%s.HIGH',paramname);
   bpmxhigh(loop)=getpv(paramname2);
end
for loop=1:size(BPMlist,1)
   ...
   paramname(changeindex+1)='R';
   paramname2=sprintf('%s.LOW',paramname);
   bpmylow(loop)=getpv(paramname2);
   paramname2=sprintf('%s.HIGH',paramname);
   bpmyhigh(loop)=getpv(paramname2);
end
