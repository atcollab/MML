...
TimeStart = clock;
for i = 1:Navg
    [x, y, T1, TimeStamp] = getbpmbuffer(DeviceList);
    ...
    for j = 1:size(DeviceList,1)
        if PlotFlag
            figure(h_fig1);
            [Pxx1, fx, x1rms, Pxx1_Int] = bpm_psd(x(j,:), T1, 'r');
            Pxx1 = Pxx1(:)';
        else
            [Pxx1, fx, x1rms, Pxx1_Int] = bpm_psd(x(j,:), T1);
            Pxx1 = Pxx1(:)';
        end
        if PlotFlag
            drawnow;
            figure(h_fig2);
            [Pyy1, fy, y1rms, Pyy1_Int] = bpm_psd(y(j,:), T1, 'r');
            Pyy1 = Pyy1(:)';
            drawnow;
        else
            [Pyy1, fy, y1rms, Pyy1_Int] = bpm_psd(y(j,:), T1);
            Pyy1 = Pyy1(:)';
        end
