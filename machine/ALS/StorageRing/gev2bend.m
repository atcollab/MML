...
for i = 1:size(GeV,1)   %length(iDevTotal) 
    for j = 1:size(GeV,2) 
        ...
        if GeV(i,j) == 0
            % Cluge to make zero amps, zero energy
            Amps(i,j) = 0;
        elseif RampTable.GeV(1) == GeV(i,j)
            % This avoids a table lookup error when in production and injection are same energy.
            Amps(i,j) = MagnetTable(i,1);
        elseif RampTable.GeV(end) == GeV(i,j)
            % This avoids a table lookup error when in production and injection are same energy.
            Amps(i,j) = MagnetTable(i,end);
        else
            Amps(i,j) = interp1(RampTable.GeV, MagnetTable(i,:), GeV(i,j));
        end
