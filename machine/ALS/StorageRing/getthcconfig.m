function [ConfigSetpoint, ConfigMonitor, FileName] = getthcconfig(varargin)
%GETTHCCONFIG - Returns or saves to file the present THC setpoints
%  [ConfigSetpoint, ConfigMonitor, FileName] = getthcconfig(FileName)
%  
%  INPUTS
%  1. FileName - File name to storage data (if necessary, include full path)
%                'Archive' will archive to the default <Directory\THCData\THCArchiveFile>
...
%
%  NOTE
%  1. Use getthcconfig('Golden') to store the default golden lattice
%  2. Use getthcconfig('Archive') to archive
%
%  See also setthcconfig, showthc, getmachineconfig, getproductionlattice, getinjectionlattice
...
if ArchiveFlag
    if isempty(FileName)
        FileName = appendtimestamp(getfamilydata('Default', 'THCArchiveFile'));
        DirectoryName = getfamilydata('Directory','THCData');
        if isempty(DirectoryName)
            DirectoryName = [getfamilydata('Directory','DataRoot') 'THC', filesep];
        else
        ...
        [FileName, DirectoryName] = uiputfile('*.mat', 'Save THC config to ...', [DirectoryName FileName]);
        if FileName == 0
            disp('   THC configuration not saved (getthcconfig).');
            return
        end
    ...
    elseif strcmpi(FileName, 'Golden') || strcmpi(FileName, 'Production')
        ...
        if ~strcmp(AnswerString,'Yes')
            disp('   Lattice configuration not saved (getthcconfig).');
            return;
        end
...
% Get the setpoint
ConfigSetpoint.Setpoint = getpv('THC', 'Setpoint', 'Struct');
ConfigSetpoint.Error    = getpv('THC', 'Error', 'Struct');

% Get the monitors
if nargout >= 2 || ArchiveFlag
    ConfigMonitor = getpv('THC', 'Monitor', 'Struct');
end
