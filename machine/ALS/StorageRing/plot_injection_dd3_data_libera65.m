...
if strfind(getpvonline('SR_mode'),'Two-Bunch')>0
    setpv('LIBERA_0AAD:ENV:ENV_GAIN_SP',-33);
else    
    setpv('LIBERA_0AAD:ENV:ENV_GAIN_SP',-33);   % libera is now connected through splitter, i.e. smaller signal
    % setpv('LIBERA_0AB3:ENV:ENV_GAIN_SP',-27);
end
setpv('LIBERA_0AAD:ENV:ENV_AGC_SP',1);
setpv('LIBERA_0AAD:ENV:ENV_DSC_SP',2);
setpv('LIBERA_0AAD:ENV:ENV_SWITCHES_SP',255);
%
pause(0.1);
%
...
while 1
    ...
    [am,sp,name]=getlibera('DD3',[6 5],1);
    pause(1.45);
