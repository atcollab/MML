...
% Current levels
GoalCurrent = 500;  %getpv('Topoff_goal_current_SP');
...
if ~isempty(Fill_Pattern)
    ...
    % Wait for the last booster cycle to complete, just to get in sync
    PeakFlag = wait_for_booster_peak_field;
    if PeakFlag == 0
        soundquestion;
        error('Timeout -- no booster peak field reached within 1.5 seconds.  Is the booster pulsing?\n');
    end
    ...
    for i = 1:length(Fill_Pattern)*100
        %try
            pause(.2); % May want to change the scan rate of 'BTS_____ICT01__AM00' & 'BTS_____ICT02__AM01'

            %StartCurr = getpv(DCCT_ChannelName{i});
            StartCurr = getdcct;
                        
            [BTSCharge, ExtractFlag, T] = wait_for_extraction;
            BTSCharge1 = getam('BTS_____ICT01__AM00');
            BTSCharge2 = getam('BTS_____ICT02__AM01');
            fprintf('   BTSCharge1=%.3f   BTSCharge2=%.3f   %.2f wait in wait_for_extraction\n', BTSCharge1, BTSCharge2, T);
            
            if ExtractFlag==0
                %soundquestion;
                fprintf('\n   Timeout, no extraction reached within 1.5s\n');
                pause(0.5);
            else
                LastBucket = getpv('SR01C___TIMING_AM08');
                
                % Set the target bucket for the next shot
                iBucket = mod(i, length(Fill_Pattern))+1;
                setpv('SR01C___TIMING_AC08', Fill_Pattern(iBucket));
                
                % Measure the current from the last shot
                pause(0.5);
                
                StopCurr = getdcct;
                eff = (StopCurr-(StartCurr-0.05)) / BTSCharge * 4 / 1.5;  % BTSCharge/4=nC (ICT calibration); nC/1.5 = mA in the SR; -.05 SR lifetime loss during measurement
                fprintf('   %s: Injection efficiency %g \n', datestr(now), eff);
                
                setpv('BTS_To_SR_Injection_Efficiency', eff);
                setpv('BTS_To_SR_Injection_Rate', (StopCurr-(StartCurr-0.05)));
                setpv('BR_To_BTS_Extracted_Charge', BTSCharge/4);
                
                fprintf('   %d.  %.1f sec (%.2f sec)  Bucket %d  BTS=%.3f  DCCT=%.3f (%.3f)\n\n', i, etime(clock,t0), etime(clock,t_last), LastBucket, BTSCharge, getdcct, (StopCurr-(StartCurr-0.05)));
                t_last = clock;
            end
