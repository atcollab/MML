...
FileName = 'bpm_main_gtb.edl';
TitleBar = 'GTB BPM Engineering Panel Launcher';
...
WriteEDMFile(fid, EDMStaticText('S',         x+200+100+445, 3, 'Width', 40, 'Height', Height, 'FontSize', FontSize, 'FontWeight', 'bold', 'FontName', 'helvetica', 'HorizontalAlignment', 'center'));
...
for i = 1:size(Dev,1)
   ...
   %WriteEDMFile(fid, EDMRelatedDisplay('BPM_engrControls.edl',      x,      y, 'Macro', sprintf('P=%s:,R=BPM%d:', AccType, Dev(i,2)), 'Width',  100, 'Height', Height, 'FontSize', FontSize, 'FontWeight', 'bold',   'FontName', 'helvetica', 'ButtonLabel', sprintf('BPM(%d,%d)', Dev(i,1),Dev(i,2)), 'CommandLabel', ''));
    WriteEDMFile(fid, EDMRelatedDisplay('BPM_engrDisplay.edl',       x,      y, 'Macro', sprintf('P=%s:,R=BPM%d:', AccType, Dev(i,2)), 'Width',  100, 'Height', Height, 'FontSize', FontSize, 'FontWeight', 'bold',   'FontName', 'helvetica', 'ButtonLabel', sprintf('BPM(%d,%d)', Dev(i,1),Dev(i,2)), 'CommandLabel', ''));
    WriteEDMFile(fid, EDMRelatedDisplay('bpm_sa_plot.edl',           x+105,  y, 'Macro', sprintf('P=%s:,R=BPM%d:', AccType, Dev(i,2)), 'Width',   50, 'Height', Height, 'FontSize', FontSize, 'FontWeight', 'bold',   'FontName', 'helvetica', 'ButtonLabel', 'SA', 'CommandLabel', ''));
    WriteEDMFile(fid, EDMRelatedDisplay('bpm_sa_plot_xy.edl',        x+160,  y, 'Macro', sprintf('P=%s:,R=BPM%d:', AccType, Dev(i,2)), 'Width',   50, 'Height', Height, 'FontSize', FontSize, 'FontWeight', 'bold',   'FontName', 'helvetica', 'ButtonLabel', 'xy', 'CommandLabel', ''));
    ...
    WriteEDMFile(fid, EDMTextMonitor(sprintf('%sSA:Y', deblank(Name(i,:))), x+200+ 80+320, y, 'Width', 45, 'Height', Height, 'Precision', 3, 'HorizontalAlignment', 'right'));
...
%     WriteEDMFile(fid, EDMButton(sprintf('%swfr:ADC:arm', deblank(Name(i,:))), x+200+130+550, y, sprintf('%swfr:ADC:arm', deblank(Name(i,:))), 'ButtonType', 'Push', 'Width', 15, 'Height', 18));
    ...
    WriteEDMFile(fid, EDMRelatedDisplay('BPM_recorderControls.edl', x+200+120+650, y, 'Macro', sprintf('P=%s:,R=BPM%d:', AccType, Dev(i,2)), 'Width', 85, 'Height', Height, 'FontSize', FontSize, 'FontWeight', 'bold',   'FontName', 'helvetica', 'ButtonLabel', 'Trigger', 'CommandLabel', ''));
    y = y + 24;
end
y1 = y;
