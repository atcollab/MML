...
%            1 - Fit at all sextupole magnets
%
%  EXAMPLES
...
% 2011-05-16: retirado o monitor AMP03C (Liu/Fernando/Ximenes)
...
% 2011-05-16: retirado o monitor AMP03C.
    ...
    case 1
        ...
        for iFamily = 1:length(ATFamilies)
            ...
            for jFamily = 1:length(MMLFamilies)
                ...
                [Names, j, k] = unique(family2channel(MMLFamily), 'rows');
                for loop = 1:length(j)
                    N = N + 1;
...
if isempty(LocoMeasData.DeltaRF)
    FitParameters.DeltaRF = 1000;  % It's good to have something here so that LOCO will compute a model dispersion
else
