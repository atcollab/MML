...
if QMS.QuadPlane==1        
    ...
    % Use all downstream BPMs in the minimization
    BPMDevList = family2dev(QMS.BPMFamily);
    BPMspos    = getspos(QMS.BPMFamily);
    QUADspos   = getspos(QMS.QuadFamily, QMS.QuadDev);
    i = find(BPMspos>QUADspos);
    QMS.BPMDevList = BPMDevList(i,:);

    % Find the BPM closest to the quadrupole
    [TmpFamily, QMS.BPMDev, SPosition, PhaseAdvance] = quad2bpm(QMS.QuadFamily, QMS.QuadDev, 'DownStream');
