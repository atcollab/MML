...
for i = StartFrom+2:(StartFrom+2+Iterations-1)
    ...
    if strcmpi((get(findobj(gcf,'Tag','Jacobian'),'Checked')),'on')
        LocoFlags(i).Jacobian = 'Yes';
    else
        LocoFlags(i).Jacobian = 'No';
    end
...
set(findobj(gcbf,'Tag','UserInput'),  'Checked','Off');
set(findobj(gcbf,'Tag','Threshold'),  'Checked','Off');
if ischar(LocoFlags.Jacobian)
    if strcmpi((LocoFlags.Jacobian),'yes')
        set(findobj(gcbf,'Tag','Jacobian'),'Checked','On');
    else
        set(findobj(gcbf,'Tag','Jacobian'),'Checked','Off');
    end
end
    ...
    switch PlotNumber
        case 1
            ...
            if exist('LocoFlags','var')
                if ~isempty(LocoFlags)
                    ...
                    if isfield(LocoFlags(i),'Jacobian')
                        if ischar(LocoFlags(i).Jacobian)
                           text(2,N,sprintf('Jacobian Calculate = %s', LocoFlags(i).Jacobian),'units','characters');
                           N = N + 1;
                        end
                    end             
                end
        ...
        case 7
            if ~isempty(CMData(i).VCMEnergyShift)
                ...
                if isempty(CMData(i).VCMEnergyShiftSTD)
        ...
        case 22
            ...
            if 1
                ...
                if ~isempty(j)
                    ...
                    legend('Measured','Model','Outliers',0);
                else
