...
ModeCell = {'Fit QL1 - QM2 Magnets by Power Supply','Fit QL1 - QM2 by Magnet','Fit QL1 - QM2 (K & Tilt) by Magnet',...
    'Fit QL1-QM2 by Magnet, all QUADs & SQ with skew s', 'Fit QL1-QM2 by Magnet, only SQ with skew s',...
    'Fit QH1-3 and QM1-2 by power supply, QL1-3 by parameters','No Parameter Setup'};
%    'Fit by power supply except QL2', 'Fit by power supply except QL3', 'Fit by power supply except QH1', ...
%    'Fit by power supply except QL3', 'Fit by power supply except QH1', ...    
...
switch ButtonName
    ...
    case 3 %'Fit by Magnet with tilt'
        ...
        for loop=1:length(iAT)
            N = N + 1;
            FitParameters.Params{N} = mkparamgroup(THERING,iAT(loop),'tilt');
        end
        FitParameters.Values = [FitParameters.Values; zeros(length(iAT),1)];
        FitParameters.Deltas = [FitParameters.Deltas; NaN * ones(length(iAT),1)];
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      case 4 %'Fit QL1-QM2 by Magnet , all QUADs & SQ with skew s'
        ...
        FitParameters.Deltas = [FitParameters.Deltas; NaN * ones(length(iAT),1)];        
         ...
         % SQ K-values
        iAT = findcells(THERING,'FamName','SQ');
        for loop=1:length(iAT)
            N = N + 1;
            FitParameters.Params{N} = mkparamgroup(THERING,iAT(loop),'s');
        end
        FitParameters.Values = [FitParameters.Values; zeros(length(iAT),1)];
        FitParameters.Deltas = [FitParameters.Deltas; NaN * ones(length(iAT),1)];
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      case 5 %'Fit QL1-QM2 by Magnet , only SQ with skew s'
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    case 6 %'Fit QH1-3 & QM1-2 by Power Supply, QL1-3 by magnet'

        fprintf('\n   Fit Quadupoles by Power Supply except QL1\n');
