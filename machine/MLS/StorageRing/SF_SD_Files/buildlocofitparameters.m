...
% Quads
if nargin < 2
    ModeCell = {'No parameter setup', 'Fit magnets by power supply','Fit QF, QD, QFA by magnet'};
...
try
    if isfield(LocoMeasData, 'MachineConfig')
        ...
        if all(LocoMeasData.MachineConfig.HSD.Setpoint.Data == 0)
            fprintf('   Turning HSD family off in the LOCO model.\n');
            ATIndex = findcells(RINGData.Lattice,'FamName','HSD')';
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        end
...
switch SkewFit
    ...
    case 2  
        ...
        AT_Index = [family2atindex('SF'); family2atindex('SD'); family2atindex('HSD');]';
        for loop = 1:size(AT_Index,1)
            N = N + 1;
            FitParameters.Params{N} = mkparamgroup(RINGData.Lattice, AT_Index(loop,:),'s');
            FitParameters.Values = [FitParameters.Values; 0];
