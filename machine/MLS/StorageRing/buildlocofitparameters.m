...
try
    if isfield(LocoMeasData, 'MachineConfig')
        % Sextupoles
        if all(LocoMeasData.MachineConfig.S1.Setpoint.Data == 0)
            fprintf('   Turning S1 family off in the LOCO model.\n');
            ATIndex = family2atindex('S1');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        end
        if all(LocoMeasData.MachineConfig.S2.Setpoint.Data == 0)
            fprintf('   Turning S2 family off in the LOCO model.\n');
            ATIndex = family2atindex('S2');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
        end
        if all(LocoMeasData.MachineConfig.S3.Setpoint.Data == 0)
            fprintf('   Turning S3 family off in the LOCO model.\n');
            ATIndex = family2atindex('S3');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
...
switch SkewFit
    ...
    case 2  
        ...
        AT_Index = [family2atindex('S1'); family2atindex('S2'); family2atindex('S3');]';
        for loop = 1:size(AT_Index,1)
            N = N + 1;
            FitParameters.Params{N} = mkparamgroup(RINGData.Lattice, AT_Index(loop,:),'s');
            FitParameters.Values = [FitParameters.Values; 0];
