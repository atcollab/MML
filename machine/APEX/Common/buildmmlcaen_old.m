function OutStruct = buildmmlcaen(Family, DeviceList, Range)
% Caen Power Supplies
%
...
for i = 1:N
    OutStruct.CommonNames{i,1} = sprintf('%s%d', Family, i);
end

for i = 1:length(Field)
    OutStruct.(Field{i}) = buildmmlsubfamily(FieldType{i});
    for j = 1:size(DeviceList, 1)
        OutStruct.(Field{i}).ChannelNames{j,1} = sprintf('%s%d:',Family,j,CaenName{i});
    end
end
