...
% Last Modified by GUIDE v2.5 04-Nov-2011 09:35:35
...
if length(varargin) >= 5
    TextString = varargin{5};
end
if length(varargin) >= 6
    ScalingFlag = varargin{6};
end
...
varargout{4} = get(handles.Subject, 'String');
if strcmpi(varargout{4},'Subject?')
    varargout{4} = 'Matlab Figure';
end

varargout{5} = get(handles.TextField, 'String');
if strcmpi(varargout{5},'Entry?')
    varargout{5} = ' ';
end
...
if strcmpi(s{i},'Physics') || strcmpi(s{i},'Operations') || strcmpi(s{i},'Maintenance')
    Categories = {
        'Bug report'
        'Computer'
        'Electrical'
