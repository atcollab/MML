...
for i = length(varargin):-1:1
    ...
    elseif strcmpi(varargin{i}, 'Numeric')
        %OutputStruct = 0;
        varargin(i) = [];
    elseif any(strcmpi(varargin{i}, {'Init APEX','InitAPEX','APEX'}))
        Caen.Command = 'Init APEX';
        varargin(i) = [];
...
switch Caen.Command
    ...
    case 'Init APEX'
        if length(varargin) >= 1 % && (ischar(varargin{1}) || isempty(varargin{1}))
            DevName = varargin{1};
            varargin(i) = [];
            Caen = caenparameters('APEX', DevName);
            if isempty(Caen)
                fprintf('   No magnet selected.\n');
                return
            end
        else
            Caen = caenparameters('APEX');
        end
    ...
    case 'SY3634Init'
        ...
        if ChangesNeeded == 0
            ...
            if any(strcmpi(Caen.Name, {'SOL1','SOL2','SOL3'}))
                setpv_local_interlock([Caen.Name,':EEP_Intlk1Name'], 'WATERFLOW');
            ...
            setpv_local([Caen.Name,':EEP_TmaxFET'],   55);
            setpv_local([Caen.Name,':EEP_TmaxSHUNT'], 55);
            
            setpv_local([Caen.Name,':EEP_VminLimit'], 11);
