function varargout = llrf_buncher(varargin)
% LLRF_BUNCHER MATLAB code for llrf_buncher.fig
%      LLRF_BUNCHER, by itself, creates a new LLRF_BUNCHER or raises the existing
%      singleton*.
%
%      H = LLRF_BUNCHER returns the handle to a new LLRF_BUNCHER or the handle to
%      the existing singleton*.
%
%      LLRF_BUNCHER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LLRF_BUNCHER.M with the given input arguments.
%
%      LLRF_BUNCHER('Property','Value',...) creates a new LLRF_BUNCHER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before llrf_buncher_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to llrf_buncher_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help llrf_buncher

% Last Modified by GUIDE v2.5 19-Sep-2015 07:46:30
% Feb. 7, 2012. Modified by Fernando. Added FPGA frequency alignment in function setllrf1power
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @llrf_buncher_OpeningFcn, ...
    'gui_OutputFcn',  @llrf_buncher_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = llrf_buncher_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before llrf_buncher is made visible.
function llrf_buncher_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to llrf_buncher (see VARARGIN)

% Choose default command line output for llrf_buncher
...
% UIWAIT makes llrf_buncher wait for user response (see UIRESUME)
...
%set(t, 'StartFcn', ['llrf_buncher(''Timer_Start'',',    sprintf('%.30f',handles.hMainFigure), ',',sprintf('%.30f',handles.hMainFigure), ', [])']);
...
% Get LLRF_BUNCHER error/warning conditions
try
    % Reset warning
    ...
    % Check the interlock state on each LLRF_BUNCHER board
    InterlockFlag = 0;
    for ii = 1:length(LLRFData)
...
if ~isempty(GraphMax)
    ...
    if get(handles.TimeScaleAll,'Value')
        % Set all the LLRF_BUNCHER board to the same scaling
        for ii = 1:length(LLRFData)
            LLRF_Prefix = LLRFData{ii}.Prefix;
...
% Save LLRF_BUNCHER Data to global and plot
