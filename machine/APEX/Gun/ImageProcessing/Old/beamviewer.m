...
% Last Modified by GUIDE v2.5 10-May-2012 12:02:05
...
Cam = getcam(CamName);
handles.imageplot = imagesc(reshape(Cam.Data, Cam.Cols, Cam.Rows)', [0 2^12-1]);
...
set(handles.axes_left,   'Position', [8 Pmain(2)+6 138 516-12]);
...
YProjection = sum(reshape(Cam.Data, Cam.Cols, Cam.Rows),1);    % clean up???
set(handles.profx, 'XData', 1:Cam.Cols, 'YData', XProjection);
set(handles.profy, 'XData', 1:1:Cam.Rows, 'YData', YProjection);
...
h = figure;
imagesc(reshape(Cam.Data, Cam.Cols, Cam.Rows)', [0 2^12-1]);
...
% --- Otherwise, executes on mouse press in 5 pixel border or over AcquireTime.
function AcquireTime_ButtonDownFcn(hObject, eventdata, handles)
