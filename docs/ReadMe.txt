...
many accelerators world wide, there are many authors of the new MML.  Many thanks to all the people 
who have made suggestions, written new functions, and fixed bugs.  I look forward  more 
collaborations and I'm curious to see how things morph and improve as we move forward.

If there isn't a zip file on my website send me an email and I'll update the MML and zip it.
Thanks,
Greg and Jeff
...
For EPICS users using LabCA you have to add to the system path before starting matlab:
Unix
  Add <MMLROOT>/links/labca/bin/<arch> to PATH
  Add <MMLROOT>/links/labca/lib/<arch> to LD_LIBRARY_PATH
PC
  Add <MMLROOT>\links\labca\bin\<arch> to PATH
(note: labca_3_5 is need for 2007b onward)

Changes to the Matlab path are handled in setpathmml
