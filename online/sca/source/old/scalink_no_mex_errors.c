...
int nrhs, const mxArray *prhs[] )
{
    ...
    double   *AM, *SectorNum, *TimeStamp, *ErrorOutput, ErrorFlag=0, EPICS_Max_Wait;
    ...
    if (FunctionType == 0) {
        ...
        if (nlhs >= 1) {
            ...
            ErrorOutput = mxGetPr(OUTPUT1);
            *ErrorOutput = (double) ErrorFlag;
        }
        ...
        if (EPICStype == SCA_STRING) {
            ...
            for (Device=0; Device<Rows; Device++) {
                ...
                if (sca_error(Status)) {
                    mexPrintf("   Channel Name: %s, status=%x (QUE_GET error)\n", NameBuffer, StatusArray[Device]);
                    print_sca_status(Status);
                    ErrorFlag = -11;
                    /* mexErrMsgTxt("QUE_GET error (scalink.c).\n"); */
                    if (nlhs >= 4) {
            ...
            for (i=0; i<tcols; i++) {
                ...
                for (AverageLoop=1; AverageLoop<=NumberOfAverages; AverageLoop++) {
                    ...
                    if (NumberOfErrors > 0) {
                        mexPrintf("   Number of errors = %d (DO_GET)\n", NumberOfErrors);
                        for (Device=0; Device<Rows; Device++) {
                            if (sca_error(StatusArray[Device])) {
                                print_sca_status(StatusArray[Device]);
                            }
        ...
        if (nlhs >= 4) {
            OUTPUT4 = mxCreateDoubleMatrix((unsigned int) 1, (unsigned int) 1, mxREAL);
            ErrorOutput = mxGetPr(OUTPUT4);
            *ErrorOutput = (double) ErrorFlag;  /* "Status" has an EPICS code */
        }
        ...
        mexPrintf("   Input #1: Action type unknown (scalink.c).\n");
        ErrorFlag = -101;
