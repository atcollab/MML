...
%  Check string/menu conversion
	if ( ~strcmp(lcaGet('lca:scl0',0,'c'),'123') )
		error('type STRING -> number read check FAILED')
	end
	if ( ~strcmp(lcaGet('lca:count.SCAN'),'1 second') )
...
if ( ~strcmp(lcaGet('lca:out',0,'c'),'0.12345') )
	error('lcaGetPrecision test (string 1) FAILED')
end

lcaPut('lca:out.PREC',2)

if ( ~strcmp(lcaGet('lca:out',0,'c'),'0.12') )
...
if ( find( isnan(got) ~= isnan([ 0; nan('double')]) ) )
...
if ( find( isnan(lcaGet(scls)) ~= isnan( [ nan('double'); 0] )) )
...
if ( find(~strcmp(lcaGetUnits({'lca:scl0';'lca:scl1'}),{'ABER';'XX'})) )
...
%  a Ctrl-C event under matlab
    ...
    disp('WARNING: Cannot catch CTRL-C under some versions of MATLAB from .m file -- ignore possible error message')
	lca_fail=0;
	try
		lcaDelay(10)
		lca_fail=1;
