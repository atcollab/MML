...
{   
    ...
    struct dbr_sts_double *myDataPtr;
    struct dbr_time_double *myTimeDataPtr;
    TS_STAMP  myStamp;
    ...
    {  case 0: 
       ...
       case 3: // MCAOPEN Return names of connected channels as cell array of strings
        ...
        case 40: // Get PV values by their MCA handles
            mexPrintf("case 40 - Get PV values\n");
            for(i=1;i<nrhs;i++)
            
            {   Hndl = (int)mxGetScalar(prhs[i]);
                if(Hndl<1 || Hndl>HandlesUsed)
                    mexErrMsgTxt("Invalid Handle");
                    
                FieldType = ca_field_type(CHNLS[Hndl-1].CHID);
                RequestType = dbf_type_to_DBR(FieldType);
                Cnt = ca_element_count(CHNLS[Hndl-1].CHID);
                BuffSize = dbr_size_n(RequestType,Cnt);
                ...
                mexPrintf("Simple Type\n");
                mexPrintf("dbr_size_n = %d\tdbr_size = %d\tdbr_value_size = %d\n", dbr_size_n(RequestType,Cnt),
                    dbr_size[RequestType],dbr_value_size[RequestType]);
                
                status = ca_array_get(DBR_DOUBLE,Cnt,CHNLS[Hndl-1].CHID,myDblPr);
                if(status!=ECA_NORMAL)
                    mexErrMsgTxt("Error caling ca_array_get");
                status = ca_pend_io(MCA_IO_TIMEOUT);
      ...
      /*
                ...
                mexPrintf("STS Type\n");
                RequestType = dbf_type_to_DBR_STS(FieldType);  
                  
                mexPrintf("dbr_size_n = %d\tdbr_size = %d\tdbr_value_size = %d\n", dbr_size_n(RequestType,Cnt),
                    dbr_size[RequestType],dbr_value_size[RequestType]);
                    
                 mexPrintf("TIME Type\n");
                RequestType = dbf_type_to_DBR_TIME(FieldType);  
        ...
        */
        ...
        case 41: // Get PV values as STRINGS by their MCA handles
            
            mexPrintf("case 41 - Get PV values as strings\n");
            
          
            plhs[0] = mxCreateString(StrBuffer);
            ...
            {   Hndl = (int)mxGetScalar(prhs[i]);
                ...
                status = ca_array_get(RequestType,1,CHNLS[Hndl-1].CHID,StrBuffer);
                if(status!=ECA_NORMAL)
                    mexErrMsgTxt("Error caling ca_array_get");
                status = ca_pend_io(MCA_IO_TIMEOUT);
        ...
        case 42: // Get PV values by their MCA handles
            mexPrintf("case 40 - Get PV values\n");
            ...
            {   Hndl = (int)mxGetScalar(prhs[i]);
                ...
                FieldType = ca_field_type(CHNLS[Hndl-1].CHID);
                RequestType = DBR_STS_DOUBLE;
                Cnt = ca_element_count(CHNLS[Hndl-1].CHID);
                BuffSize = dbr_size[RequestType];
                myDataPtr = (struct dbr_sts_double*)mxMalloc(BuffSize);
                status = ca_array_get(DBR_STS_DOUBLE,Cnt,CHNLS[Hndl-1].CHID,myDataPtr);
                status = ca_pend_io(MCA_IO_TIMEOUT);
                mexPrintf("Value:%f\tStatus:%s\tSeverity:%s\n",myDataPtr->value,alarmStatusString[myDataPtr->status],alarmSeverityString[myDataPtr->severity]);
            ...
            case 43: // Get PV values by their MCA handles
            mexPrintf("case 40 - Get PV values\n");
            ...
            {   Hndl = (int)mxGetScalar(prhs[i]);
                ...
                myTimeDataPtr = (struct dbr_time_double *)mxMalloc(BuffSize);
                
                status = ca_array_get(DBR_TIME_DOUBLE,Cnt,CHNLS[Hndl-1].CHID,myTimeDataPtr);
                status = ca_pend_io(MCA_IO_TIMEOUT);
                mexPrintf("Value:%f\tStatus:%s\tSeverity:%s\n",myTimeDataPtr->value,alarmStatusString[myTimeDataPtr->status],alarmSeverityString[myTimeDataPtr->severity]);
                myStamp = myTimeDataPtr->stamp;
                mexPrintf("sec%u\tnsec:%u\n",myStamp.secPastEpoch,myStamp.nsec);
        ...
        case 60: // MCAPUT - fast unconfirmed put for scalat numeric PV's
        ...
        case 1000: // print timeout settings
            ...
            mexPrintf("MCA timeout and settings\n:");
