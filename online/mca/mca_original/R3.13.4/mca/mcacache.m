...
%       
%   Notes: The cache value for a PV does not exist until the fist use of a
%   monitor on that PV
%   
%   If a monitor is set on a PV with MCAMON 
%   the use of MCACACHE resets the number of monitor events to 0
%   See also: MCAMON

if nargin>1 & ne(nargin,nargout)
    error('Number of outputs must match the number of inputs')
end
[varargout{1:nargin}] = mcamain(300,varargin{:});
